...
\input{./relpath-to-latexroot.ltx}  % ✅ Loads tex-paths.ltx + metadata automatically
```
...
```latex
\input{./relpath-to-latexroot.ltx}  % Loads everything needed
...
**Problem:** This shouldn't occur with modern files - `relpath-to-latexroot.ltx` loads it automatically
...
**📚 For detailed explanation**: See [`SUBFILE_CROSS_REFERENCE_ARCHITECTURE.md`](SUBFILE_CROSS_REFERENCE_ARCHITECTURE.md) for the full architecture of the `\whenintegrated{}` / `\whenstandalone{}` system and why labels are wrapped.
...
```latex
✅ \input{./relpath-to-latexroot.ltx}  % Loads paths automatically
...
\input{./relpath-to-latexroot.ltx}  % Loads tex-paths.ltx + metadata
\documentclass[\latexroot/\projectname]{subfiles}
\whenstandalone{\externaldocument{\latexroot/\projectname}}
