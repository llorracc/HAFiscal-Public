...
# Portable "last argument": the wrapper is invoked as `sh <wrapper> %O %B` (latexmkrc_using_bibtex_wrapper),
# and on Debian/Ubuntu `sh` is dash, where the bash-only indirect expansion "${!#}" expands to NOTHING (no
# error). BASENAME_ARG was then empty, so the .blg sanitization below never ran and latexmk's own .blg scan
# ("There were N error messages") failed the build on every "Repeated entry" (HAFiscal `make pdf-candidate`
# exit 2, 2026-08-25/09-07; fixed 2026-09-09). macOS `sh` is bash in POSIX mode, which is why it never bit there.
for BASENAME_ARG in "$@"; do :; done

# --- PRE-CHECK: SKIP BIBTEX IF .BBL EXISTS BUT .BIB IS MISSING ---
# This handles the QE submission case where:
# - .bbl (precompiled bibliography) is provided
# - .bib (source) is intentionally excluded from main branch
# In this case, we should use the existing .bbl and skip bibtex.
BBL_FILE="${BASENAME_ARG}.bbl"
BIB_FILE="${BASENAME_ARG}.bib"

if [ -f "$BBL_FILE" ] && [ ! -f "$BIB_FILE" ]; then
    echo "bibtex_wrapper: Using precompiled .bbl file (no .bib source present)"
    echo "  BBL file: $BBL_FILE (exists)"
    echo "  BIB file: $BIB_FILE (not found - expected for QE submission)"
    echo "  Skipping bibtex - precompiled bibliography will be used"
    exit 0
fi
