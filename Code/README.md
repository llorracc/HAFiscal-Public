...
> **Secondary navigation doc (2026-06-13).** The authoritative computational
> pipeline, runtime, output, and paper-exhibit map is
> [`HA-Models/README.md`](HA-Models/README.md). The repo-wide authority map is
> [`../CLAUDE.md`](../CLAUDE.md), and the human architecture overview is
> [`../ARCHITECTURE.md`](../ARCHITECTURE.md). If this file conflicts with those
> owner docs, the owner docs win.
...
- **Files** (two-phase workflow):
  - **5a** `FromPandemicCode/AggFiscalMAIN_reduced.py --baseline` — TM multipliers
  - **5b** `FromPandemicCode/run_welfare6_parallel.py --baseline` — MC welfare-6
...
- **Outputs**:
  ...
  - Table 6 (Multipliers) → `FromPandemicCode/Tables/CRRA2/Multiplier.ltx` (from 5a)
  - Table 7 (Welfare) → `FromPandemicCode/Tables/Baseline/welfare6.tex` (from 5b)
- **Note**: This is the longest-running step. The old monolithic `AggFiscalMAIN.py` entry point was retired at commit c7e566d9.
...
**Option 2: Enable only fast steps with environment flags**
...
**Recommended configuration for full replication** (this is the default for `./reproduce.sh --comp full`):

| Step | Default | Env var override |
|------|---------|------------------|
| 1 (splurge estimate) | ON  | `HAFISCAL_RUN_STEP_1=false` to disable |
| 2 (baseline model)   | ON  | `HAFISCAL_RUN_STEP_2=false` to disable |
| 3 (Splurge=0 robustness, ~21 h) | OFF | `HAFISCAL_RUN_STEP_3=true` to enable (this is what `--comp max` does) |
| 4 (HANK model)       | ON  | `HAFISCAL_RUN_STEP_4=false` to disable |
| 5 (policy comparison)| ON  | `HAFISCAL_RUN_STEP_5=false` to disable |

These env vars are read by `do_all.py`. Prefer the environment flags over
editing the script in place, especially when comparing runs or preserving
provenance.
...
2. Export step flags to enable/disable steps:

   ```bash
   export HAFISCAL_RUN_STEP_1=true
   export HAFISCAL_RUN_STEP_2=false
   export HAFISCAL_RUN_STEP_4=true
   export HAFISCAL_RUN_STEP_5=false
   ```
...
- **Table 6**: `FromPandemicCode/Tables/CRRA2/Multiplier.ltx` (from Step 5a, `AggFiscalMAIN_reduced.py --baseline`)
- **Table 7**: `FromPandemicCode/Tables/Baseline/welfare6.tex` (from Step 5b, `run_welfare6_parallel.py --baseline`)
...
Use the repository `pyproject.toml` with `uv`; it is the current dependency
source of truth. Python 3.10 or 3.11 is supported on this branch. The legacy
`environment.yml` remains a fallback/reference, not the preferred setup path.
...
```bash
# Solution: recreate or sync the repository environment
bash reproduce/reproduce_environment_comp_uv.sh
# or
make sync
```
...
```bash
# Solution: install the dependency set from pyproject.toml
make sync
```
...
```bash
# Solution: use the pinned Python and dependency set
make sync
```
...
cd Code/HA-Models
HAFISCAL_RUN_STEP_1=true HAFISCAL_RUN_STEP_4=true \
HAFISCAL_RUN_STEP_2=false HAFISCAL_RUN_STEP_5=false \
python do_all.py
```
...
# You can restart from a later step with environment flags.
...
└── FromPandemicCode/             # Steps 2-5: Main analysis
    ...
    ├── AggFiscalMAIN_reduced.py  # Step 5a: TM multipliers (--baseline)
    ├── run_welfare6_parallel.py  # Step 5b: MC welfare-6 (--baseline)
