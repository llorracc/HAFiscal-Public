...
Behavioral differences vs do_all.py:
  - Step 5 (policy comparison) runs on Reduced_Run (3 education types ×
    DiscFacCount=1 = 3 cohorts, N=5000 — see Parameters.py; the 7-atom
    β-distribution is Baseline) instead of Baseline (21 types = 3 × 7).
...
# HAFISCAL_RUN_STEP_{1,2,4,5} are honored since 2026-06-12 (finding k: they
# were previously hardcoded True here, silently ignoring a user's =false).
...
if run_step_4:
    ...
    substep("Computing household Jacobians (HA-Fiscal-HANK-SAM.py)", expected_duration_min=15)
    t0 = time()
    # BUG-069-class residual fixed 2026-08-09: os.system swallowed the
    # child's return code, so a crashed Step 4 looked green here.
    _rc = os.system("python HA-Fiscal-HANK-SAM.py")
    if _rc != 0:
        raise SystemExit(f"Step 4 (reduced): HA-Fiscal-HANK-SAM.py failed (rc={_rc})")
    progress(f"Jacobian computation completed in {(time()-t0)/60:.1f} minutes")

    substep("Running HANK-SAM policy experiments", expected_duration_min=1)
    t0 = time()
    _rc = os.system("python HA-Fiscal-HANK-SAM-to-python.py")
    if _rc != 0:
        raise SystemExit(f"Step 4 (reduced): HA-Fiscal-HANK-SAM-to-python.py failed (rc={_rc})")
...
try:
    ...
    _prov.emit(["Results", "FromPandemicCode/Tables/Baseline", "FromPandemicCode/Figures/Baseline"], command=" ".join(_prov_sys.argv), argv=_prov_sys.argv,
               label="do_all_reduced", register=False)
