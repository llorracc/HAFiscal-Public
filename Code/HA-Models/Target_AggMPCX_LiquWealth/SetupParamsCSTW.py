...
# Whether to use cubic spline interpolation. Default False = the byte-identical legacy
# path. Under HAFISCAL_SLICE_INTERP=hermite (set by HAFISCAL_SOLVE_GRID_PROFILE=hermite60,
# remedy F wiring 2026-08-21) Step-1 solves with HARK's cubic-Hermite cFunc (in-solver MPC
# derivatives; solve_one_period_ConsKinkedR CubicBool branch) — the S1 analog of the T2a
# slices. Measured motive: at the 60-basis grids count-only LINEAR drifted nabla +1.6%
# (outside scatter) while Hermite held accuracy on the S2 path; speed at stake: 19.05 ->
# 2.53 min per Step-1 seed (m5 A/B). NOTE: no powerlaw-decay above-top form on this path —
# HARK's decay-to-the-limiting-line extrapolation applies above the solve top.
import os as _os_cb
try:  # the profile->env mapping is grid_sizing's import-time SST; import it FIRST so
    # HAFISCAL_SOLVE_GRID_PROFILE=hermite60 has set HAFISCAL_SLICE_INTERP before we read it
    # (caught by the 2026-08-21 smoke: reading before that import silently missed the profile)
    import sys as _sys_cb
    _ha_cb = _os_cb.path.dirname(_os_cb.path.dirname(_os_cb.path.abspath(__file__)))
    if _ha_cb not in _sys_cb.path:
        _sys_cb.path.insert(0, _ha_cb)
    import grid_sizing as _gs_cb  # noqa: F401
except Exception:
    if _os_cb.environ.get("HAFISCAL_SOLVE_GRID_PROFILE", "full").strip().lower() == "hermite60":
        _os_cb.environ.setdefault("HAFISCAL_SLICE_INTERP", "hermite")  # fallback copy of the SST mapping
CubicBool = _os_cb.environ.get("HAFISCAL_SLICE_INTERP", "linear").strip().lower() == "hermite"
...
# NOTE: kLogInitMean/kLogInitStd are set to match HARK 0.14.1's default aNrmInitMean/aNrmInitStd
# values (0.0, 1.0). In HARK 0.14.1, the 'kLogInitMean' key was silently ignored, so HARK
# used its defaults. Now that HARK 0.17.0 recognizes kLogInitMean, we must explicitly set
# these to the 0.14.1 default values to preserve identical behavior.
# NOTE: Rboro and Rsave are explicitly set equal to Rfree to ensure consistent
# human wealth (hNrm) calculation throughout the solver. Without this, HARK's
# defaults (Rboro=1.20, Rsave=1.02) would be used, causing inconsistency.
...
init_infinite = {"CRRA":CRRA,
                ...
                'T_age': 400,
                'IndL': IndL,
                'kLogInitMean':np.log(0.00001),   # Matches 0.14.1's aNrmInitMean value
                'kLogInitStd':0.0,                 # Matches 0.14.1's aNrmInitStd value
                'pLogInitMean':0.0,
                'pLogInitStd':0.0,
