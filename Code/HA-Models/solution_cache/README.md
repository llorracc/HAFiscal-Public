...
The shared solved-policy store has its own CLI (`python -m solution_cache.policy_store
ls | prune`), and the **primitives fingerprint gate** rides on its key
(`policy_store.key_inputs`; plan item A2, 2026-08-28): freeze what a launch would
build, then diff a later build against it before spending compute —

```bash
cd Code/HA-Models
python -m solution_cache.fingerprint --parametrization Baseline --reference ref.json
python -m solution_cache.fingerprint --parametrization Baseline --check ref.json   # exit 1 on any difference
```

(solve-free construction, every agent in every scenario; the TM dist-grid top rides
along as a separate `extra` block; details in `fingerprint.py`'s docstring).
