...
_HAFISCAL_NUMERICAL_ENV_VARS = (
    ...
    # 2026-08-26 (Improvements A/B): the extension POLICY (HAFISCAL_UI_EXTENSION_POLICY and its
    ...
    # 2026-08-28: HAFISCAL_MC_WEIGHTED_TAIL / HAFISCAL_MC_PLVL_INIT were added here for the Econ-2 triple (a weighted own
    # AD loop must not HIT an equal-weight AD-cache entry) and REVERTED the same day (BUG-098): this list also keys the
    # SHARED EQUILIBRIUM store, and those two are consumer-only (battery) settings the 5a producer never sets -- every
    # sharing battery MISSed for 2.5 h and silently ran its own AD loop. Own-loop arms run with the AD cache OFF instead.
    ...
    "HAFISCAL_PF_DECAY_Q",
    # 2026-08-24 (shared solved-policy store, P2): the JAX EGM kernel
    # (HAFISCAL_USE_JAX_SOLVER=1 replaces solve_one_period) was the one solve-
    # PATH selector still outside the key — its docs excluded it as "same
    # converged cFunc to <1e-3", which is the kernel-parity class USE_JAX_2B is
    # keyed for; keyed conservatively now. Added together with the solver-
    # SOURCE hash in gather_solve_inputs — ONE deliberate rewrite of every key
    # (the gitignored dev caches re-solve once; see BUG-089: a solver edit at
    # unchanged flags must not serve stale entries). SLICE_INTERP / SOLVE_ACCEL
    # / NEWTON2D_* are keyed further down (since 2026-08-04/06).
    ...
    # 2026-08-06 (overnight lab; gap closed retroactively — CONFIRM landed
    # 2026-08-05 without a key entry): newton2d's EGM confirmation-pass count
    # and the a0=0-in-grid chord fix both change the returned cFunc at
    # identical hashed params (representation class). Only reachable when
    # SOLVE_ACCEL=newton2d is set, but keyed independently so k=1 vs k=5 (or
    # a0 on/off) entries never cross-load.
    "HAFISCAL_NEWTON2D_CONFIRM",
    "HAFISCAL_NEWTON2D_A0",
    "HAFISCAL_NEWTON2D_WARM_PAYLOAD",
    "HAFISCAL_NEWTON2D_KERNEL",
)
...
def _hafiscal_root():
    ...
    # Walk up until we find .git — a directory in a main checkout, a FILE in a git
    # worktree (found 2026-08-25: every store meta written from the m5 worktree said
    # hafiscal_sha 'unknown' because isdir() rejected the worktree's .git file).
    cur = here
    while cur and cur != "/":
...
def gather_solve_inputs(eco, shock_type, mc_method="hark_mc",
    ...
    return {
        ...
        "env": _env_dict(),
        # WHICH solver code produced the entry (content hash of the HAFiscal
        # solver functions, tail/grid helpers, FTI solver family, HARK
        # interpolation/core) — NOT a commit SHA: unrelated commits leave it
        # unchanged, a solver edit rewrites it (BUG-089; policy-store P2).
        "solver_source": _solver_source_hash(),
        "provenance": _provenance_dict(),
    }


def _solver_source_hash():
    try:
        from solution_cache import policy_store as _ps
        return _ps.solver_source_hash()
    except Exception as e:  # never block a key on the hash helper
