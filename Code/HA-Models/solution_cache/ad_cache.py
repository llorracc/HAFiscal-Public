...
def cached_solve_ad_recession(
    ...
    if (not force_solve) and os.path.exists(pkl_path):
        ...
        try:  # package mode (solution_cache.ad_cache) vs script/path mode
            from .cache import record_reuse_event as _rre_adh
        except ImportError:
            from cache import record_reuse_event as _rre_adh
        _rre_adh("ad_cache", "hit", "tolerance", engine="jax_ad",
                 shock_type=shock_type, entry=os.path.basename(pkl_path))
        t0 = time.time()
        try:
    ...
    try:  # package mode (solution_cache.ad_cache) vs script/path mode
        from .cache import record_reuse_event as _rre_ads
    except ImportError:
        from cache import record_reuse_event as _rre_ads
    _rre_ads("ad_cache", "saved", "none", engine="jax_ad",
             shock_type=shock_type, entry=os.path.basename(pkl_path))
    return result
...
def cached_solve_ad_recession_hark(
    ...
    if (not force_solve) and os.path.exists(pkl_path):
        ...
        try:  # package mode (solution_cache.ad_cache) vs script/path mode
            from .cache import record_reuse_event as _rre_hah
        except ImportError:
            from cache import record_reuse_event as _rre_hah
        _rre_hah("ad_cache", "hit", "tolerance", engine="hark_ad",
                 shock_type=shock_type, entry=os.path.basename(pkl_path))
        t0 = time.time()
    ...
    try:  # package mode (solution_cache.ad_cache) vs script/path mode
        from .cache import record_reuse_event as _rre_has
    except ImportError:
        from cache import record_reuse_event as _rre_has
    _rre_has("ad_cache", "saved", "none", engine="hark_ad",
             shock_type=shock_type, entry=os.path.basename(pkl_path))
