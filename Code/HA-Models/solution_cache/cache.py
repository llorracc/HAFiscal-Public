...
_SUPERSEDED_NOTED = set()


def _superseded_by_policy_store(layer):
    """P2 of the shared solved-policy store (plans_local/20260824-1530h): when the
    per-agent store is ON, the per-economy cold-solution layers below
    (`recession_init` — lossy knot extraction, BUG-067 class — and `policy_full`)
    are redundant: every solve they used to skip now goes through the economy /
    worker hooks, which load the same policies losslessly with the one-sweep
    guard. Return True (= this layer is a no-op) in that case, with a one-shot
    note. With the store OFF the layers behave exactly as before (the `hark`
    rollback engine's bytes are untouched)."""
    try:
        from solution_cache import policy_store as _ps
        on = _ps.enabled()
    except Exception:
        on = False
    if on and layer not in _SUPERSEDED_NOTED:
        _SUPERSEDED_NOTED.add(layer)
        print(f"[{layer}] superseded by the per-agent policy store "
              f"(HAFISCAL_POLICY_STORE on) — layer inactive", flush=True)
    return on


def _ati_routing_on():
    """True when the ConsumedATI-Markov router is enabled (``HAFISCAL_STEP5_ATI=1``).
    Since P5 of the shared-policy-store plan (2026-08-24) the welfare battery's own
    workers route the patient atoms through it, so an economy's cold solution may
    hold ATI-shaped consumption functions (a bare ``LinearInterpOnInterp1D`` per
    state) that the lossy knot extractor (``serialize.extract_eco_solution``) cannot
    walk — it raises "cFunc layout unexpected"."""
...
def _lossy_layer_inactive(layer):
    """The two knot-extraction layers (``recession_init``, the base-economy
    ``cached_eco_solve``) are inactive when the per-agent store supersedes them OR
    when ATI routing can put non-extractable policies into the economy (welfare
    battery with the store OFF + ``HAFISCAL_STEP5_ATI=1`` — the store-off arm of the
    2026-08-24 P5 gate crashed on exactly this). The wholesale-pickle layers
    (``policy_full``, ``ad_full``) are unaffected: they hold any object."""
    if _superseded_by_policy_store(layer):
        return True
    if _ati_routing_on():
        if layer not in _ATI_NOTED:
            _ATI_NOTED.add(layer)
            print(f"[{layer}] inactive under HAFISCAL_STEP5_ATI=1: ATI-solved policies "
                  f"are not knot-serializable (wholesale layers unaffected)", flush=True)
        return True
    return False


def _recession_init_cache_enabled():
    if _lossy_layer_inactive("recession-init-cache"):
        return False
...
def _policy_full_enabled():
    if _superseded_by_policy_store("policy-full"):
        return False
    return (os.environ.get(USE_CACHE_ENV_VAR, "0") == "1"
            and not byte_exact_forces_fresh_solve())
...
def cached_eco_solve(ctx, shock_type="recession",
    ...
    # Also a plain solve under the shared solved-policy store (2026-08-24): the
    # store serves every cold per-agent solve inside eco.solve(), and this layer's
    # lossy knot extractor (serialize.extract_eco_solution) cannot serialize the
    # ATI-produced consumption functions the store now installs from the
    # multiplier entry — the first cross-entry welfare run crashed here.
    if (os.environ.get(USE_CACHE_ENV_VAR, "0") != "1" or byte_exact_forces_fresh_solve()
            or _lossy_layer_inactive("base-eco-cache")):
        t0 = time.time()
        if do_switch:
