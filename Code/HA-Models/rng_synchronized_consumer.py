...
class RNGSyncKinkedRconsumerType(BaseKinkedR):
    ...
    def __init__(self, **kwds):
        ...
        # --- FAR-FIELD tail exponent for the power-law continuation (opt-in,
        # HAFISCAL_PF_DECAY_Q=farfield; remedy-F/S1 arc 2026-08-21). The stock KinkedR
        # solver's cFunc extrapolates above its top knot with the exponential decay to the
        # limiting line; at coarse (hermite60) grids that mis-shapes the region Step-1's
        # wealth moments integrate over (DIST_TOP_MULT=10). This wrap retrofits each
        # iteration's UNCONSTRAINED cFunc member with the certified power-law form, using
        # the Anderson-mini-solve far-field exponent (farfield_tail_q; V0 truth gate:
        # q̂ within 0.5% of a 50k-top deep solve, continuation c-error ≤ 5e-5 over the
        # used window). Amplitude stays pinned by top-knot LEVEL continuity. Default OFF
        # = byte-identical stock path.
        _pfq_mode = os.environ.get('HAFISCAL_PF_DECAY_Q', '').strip().lower()
        if _pfq_mode == 'farfield':
            self._install_farfield_tail()
        elif _pfq_mode == 'chart':
            # MoM log-gap chart (owner-charged A/B 2026-08-22; opt-in, NO default):
            # re-chart each iteration's solved cFunc in (log(m+h̄), log gap) — the
            # within-solve iterations must see the chart too, or the arm re-imports
            # the H2 within-solve channel. plans_local/20260822-0230h_mom-chart plan.
            self._install_mom_chart()

    def _install_mom_chart(self):
        import warnings as _warnings
        from mom_chart import chartify_chs as _chartify
        base_solver = self.solve_one_period
        announced = {}

        def _chart_solver(solution_next, IncShkDstn, LivPrb, DiscFac, CRRA,
                          Rboro, Rsave, PermGroFac, BoroCnstArt, aXtraGrid,
                          vFuncBool, CubicBool):
            # Stock 12-name signature (HARK introspects the names — the farfield
            # smoke finding); the wrapper chartifies IN PLACE so every captured
            # reference (vPfunc holds the same object) sees the chart.
            sol = base_solver(solution_next, IncShkDstn, LivPrb, DiscFac, CRRA,
                              Rboro, Rsave, PermGroFac, BoroCnstArt, aXtraGrid,
                              vFuncBool, CubicBool)
            try:
                cf = sol.cFunc
                unc = cf.functions[0] if hasattr(cf, 'functions') else cf
                ok, reason = _chartify(unc, float(sol.MPCmin), float(sol.hNrm))
                if ok and 'first' not in announced:
                    announced['first'] = True
                    print(f"[mom-chart] DiscFac={DiscFac:.6f} chartified in place "
                          f"(top-slope q={getattr(unc, 'decay_extrap_Q', float('nan')):.4f})",
                          flush=True)
                elif not ok and 'warned' not in announced:
                    announced['warned'] = True
                    _warnings.warn(f"[mom-chart] refused ({reason}); stock tail kept.")
            except Exception as _e:  # never break the solve on a representation layer
                if 'failed' not in announced:
                    announced['failed'] = True
                    _warnings.warn(f"[mom-chart] chartify failed ({_e!r}); stock tail kept.")
            return sol

        self.solve_one_period = _chart_solver

    def _install_farfield_tail(self):
        import numpy as _np
        import warnings as _warnings
        from farfield_tail_q import farfield_q as _ffq
        from powerlaw_decay import retrofit_powerlaw as _retro
        base_solver = self.solve_one_period
        announced = {}

        def _farfield_solver(solution_next, IncShkDstn, LivPrb, DiscFac, CRRA,
                             Rboro, Rsave, PermGroFac, BoroCnstArt, aXtraGrid,
                             vFuncBool, CubicBool):
            # NOTE: HARK's solve machinery builds this call from the solver SIGNATURE, so
            # the wrapper must present the stock 12-name signature (a **kw shim presents
            # zero names and gets called empty — caught by the first smoke).
            sol = base_solver(solution_next, IncShkDstn, LivPrb, DiscFac, CRRA,
                              Rboro, Rsave, PermGroFac, BoroCnstArt, aXtraGrid,
                              vFuncBool, CubicBool)
            try:
                psi = _np.asarray(IncShkDstn.atoms[0])
                th = _np.asarray(IncShkDstn.atoms[1])
                pmv = _np.asarray(IncShkDstn.pmv)
                unc = sol.cFunc.functions[0] if hasattr(sol.cFunc, 'functions') else sol.cFunc
                # x_top for the far-field WINDOW: the asset-grid top (iteration-INVARIANT,
                # so the memo hits on every backward iteration; the m-grid top wobbles with
                # the converging consumption and would bust the memo — smoke finding).
                x_top = float(_np.max(aXtraGrid))
                q, diag = _ffq(float(Rsave), float(DiscFac) * float(LivPrb),
                               float(CRRA), float(PermGroFac),
                               psi, th, pmv, float(f"{x_top:.6g}"))
                if q is not None:
                    ok = _retro(unc, q, diag)
                    tag = 'first'   # once per agent instance; DiscFac moves every eval
                    if ok and tag not in announced:
                        announced[tag] = True
                        print(f"[farfield-tail] DiscFac={DiscFac:.6f} x_top={x_top:.1f} "
                              f"q̂={q:.4f} (curv {diag['curvature']:+.3f}, "
                              f"AA {diag['anderson_iters']} iters)", flush=True)
                elif 'warned' not in announced:
                    announced['warned'] = True
                    _warnings.warn(f"[farfield-tail] estimator unhealthy "
                                   f"({diag.get('reason')}); stock tail kept.")
            except Exception as _e:  # never break the solve on a diagnostic layer
                if 'failed' not in announced:
                    announced['failed'] = True
                    _warnings.warn(f"[farfield-tail] retrofit failed ({_e!r}); stock tail kept.")
            return sol

        self.solve_one_period = _farfield_solver
