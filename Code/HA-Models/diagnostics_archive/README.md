...
`grep` dangling-check confirmed 0 broken references. (Correction, 2026-06-11/B11:
`diag_welfare6_se` was kept in `FromPandemicCode/` — but NOT because anything imports
it. The earlier "`compute_welfare6_se_table.py` imports it" claim is FALSE: that file's
only mention is a docstring line ("the pooled-bootstrap SE from diag_welfare6_se.py …"),
a STRING reference, not a Python `import` — verified no `import diag_welfare6_se` /
`from diag_welfare6_se import` exists anywhere live. `diag_welfare6_se.py` itself stays
in `FromPandemicCode/` for now; it is keep-for-now pending a future archival round.)
