...
def test_solve_grid_count_preserves_density():
    ...
    assert 56 <= c240 <= 57
    # Monotone in aXtraMax (more reach -> more points to hold density). Dropout/HS/College = 56/57/57.
...
def test_tm_grid_recovers_production():
    # phi=6.5 (College binding) -> 1300 = production_dist_aGrid_max after round-100.
    assert gs.tm_dist_aGrid_max(H_EMP[2]) == 1300.0
    # conservative phi=7.5 over-sizes (never undersizes), still finite.
    assert gs.tm_dist_aGrid_max(H_EMP[2], phi=gs.TM_PHI_CONSERVATIVE) >= 1300.0


def test_tm_grid_deprecated_alias_warns_and_matches():
    # Owner ruling 2026-07-25: `tm_grid_aMax` survives one deprecation cycle as a
    # warning alias of `tm_dist_aGrid_max` — must warn on call AND return identically.
    import warnings as _w
    with _w.catch_warnings(record=True) as rec:
        _w.simplefilter("always")
        legacy = gs.tm_grid_aMax(H_EMP[2])
    assert legacy == gs.tm_dist_aGrid_max(H_EMP[2])
...
def test_fhwc_nonfinite_falls_back_negative_raises():
    ...
    with _w.catch_warnings():
        ...
        assert gs.tm_dist_aGrid_max(float("inf")) == gs.FHWC_FALLBACK_AMAX
        assert gs.tm_dist_aGrid_max(float("nan")) == gs.FHWC_FALLBACK_AMAX
