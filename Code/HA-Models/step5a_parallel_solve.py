...
modified here):
  ...
  * The stock solve's special kernels are NOT all reproduced by the fork
    worker: if HAFISCAL_USE_JAX_2B(_VMAP) / HAFISCAL_STEP2_NAMG (or the
    deprecated HAFISCAL_STEP2_ANDERSON alias) is enabled, this wrapper falls
    back LOUDLY to the stock eco.solve() (which handles them) rather than
    silently bypassing them. HAFISCAL_STEP5_ATI IS reproduced (P4 of the
    shared-policy-store plan, 2026-08-24): the worker runs the economy-level
    cold ladder — policy store -> ATI router (with its BUG-088 gate and
    in-family EGM fallback) -> accel -> EGM — via
    parallel_eco_solve(engine_ladder=True). Until then the wrapper was inert
    under the multiplier entry, which enables ATI by default.
...
_INCOMPATIBLE_SOLVE_FLAGS = (
    ...
    # HAFISCAL_STEP5_ATI: compatible since 2026-08-24 (engine ladder in the
    # fork worker — see module docstring).
)
