...
> Full registry of **all** `HAFISCAL_*` environment flags (118):
> [`docs/ENV_FLAGS.md`](docs/ENV_FLAGS.md) — kept complete by the guard test
> `test_env_flag_registry.py`.
...
- **Runtime**: ~21 hours (≈7 h per education group; reported 2025-11).
  `do_all.py`'s progress tracker budgets ~48 h (estimate set 2026-02).
  Actual wall time is strongly hardware-dependent; record runs in
  `../../reproduce/benchmarks/`.

### Step 3: Robustness with Splurge=0 (Optional — off by default)

- **Script**: `FromPandemicCode/EstimAggFiscalMAIN.py` with Splurge=0:
  `python EstimAggFiscalMAIN.py 1.01 2.0 0.7 0.5 0`
...
- **`Clean_Folders.py`**: **Intelligent cleanup utility** - reads flags from `AggFiscalMAIN.py` (SST) and cleans accordingly (known-stale: that file was retired — see note below)
...
> **Known-stale (2026-06-11)**: `Clean_Folders.py` still parses
> `AggFiscalMAIN.py`, which was retired at commit `c7e566d9` (2026-04-08);
> run flags now live in `AggFiscalMAIN_reduced.py` (`Run_Dict`). Until
> `Clean_Folders.py` is updated it will warn that flags are not found.
