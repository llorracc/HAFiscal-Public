...
- **Runtime**: **~64 min for the Jacobians + ~13 s for the experiments**,
  MEASURED 2026-08-03 on the i9-13900K reference box (unpinned numba/BLAS
  thread pools using ~19 cores — thread config materially affects this
  wall; record it with any re-measure). This retires the conflicting
  records ("~1 h" was right; the "~12-13 h" budget was 0.14-era). NOTE:
  Step 4 was BROKEN at HEAD from the 0.14.1→0.17.0 upgrade until
  2026-08-03 (BUG-069, six-relic migration chain in the vendored
  ConsMarkovModel + the HANK scripts) — it crashed in seconds while
  do_all's old return-code-swallowing spawner reported success.
...
- **Entry points** (two phases; the old single-phase `AggFiscalMAIN.py` was
  ...
  - **Step 5b — MC welfare-6**: `run_welfare6_parallel.py --baseline
    ...
    **Default AD engine since 2026-08-02: HYBRID replay-fed JAX-AD**
    (`HAFISCAL_WELFARE_ENGINE=hybrid`, resolved by `welfare_engine.py`):
    replay-fed AD fixed point + capture-presolve cache + sown-C tables —
    **~54-min** Baseline battery (vs ~72 min all-HARK), CPU-only. Rollback:
    `HAFISCAL_WELFARE_ENGINE=hark` (bit-identical pre-arc path; also
    auto-selected under `HAFISCAL_QE_FIDELITY=1` / `HAFISCAL_WORLD=
    as-corrected`). Accepted engine residual: CRN-paired −0.5…−1.3% on AD
    cells (owner sig-figs ruling 2026-08-02; certification band + details:
