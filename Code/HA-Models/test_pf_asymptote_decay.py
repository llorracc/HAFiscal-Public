...
# ===========================================================================
# (d) BYTE-NEUTRAL OFF — flag unset == flag '0', bit-for-bit per-state cFunc.
# ===========================================================================
def test_default_off_is_byte_for_byte_unchanged():
    # Solve with the flag explicitly '0' and compare cFunc samples to the
    # default-unset solve. Both take the legacy bare-LinearInterp path, so the
    # consumption function must be bit-identical on a dense (m, C) grid.
    ag_unset, _, _, _, _ = _solved(decay_on=False)  # flag popped (unset)

    try:
        ag_zero = _build_and_solve_college_cap(decay_on=False, flag_value="0")[0]
    finally:
        os.environ.pop("HAFISCAL_PF_DECAY_EXTRAP", None)

    cf_a = ag_unset.solution[0].cFunc
    cf_b = ag_zero.solution[0].cFunc
    ...
    for i in range(len(cf_a)):
        for C in (0.8, 1.0, 1.2):
            ...
            assert np.array_equal(a, b), (
                f"state {i}, C={C}: default-OFF cFunc differs from flag='0' "
                f"(max |Δ|={np.max(np.abs(a - b)):.3g}) — OFF path not byte-neutral")
...
def test_powerlaw_flag_value_attaches_powerlaw_form():
    ...
    ag_exp, MPCmin_e, hNrm_e, _, _ = _solved(decay_on=True)
