...
def tool_tm_ergodic(state):
    ...
    (welfare6_scenario.py:534, Simulate.py):
      * `compute_baseline_tm_data(economy, mCount, neutral_measure=False)`
    ...
    reproducible run-to-run. aMax is left at the builder default (500 for
    HS_Only via `build_tm_agg_fiscal_a`), which comfortably contains the small
    HS_Only wealth tail (support ~1.9); no `HAFISCAL_TM_AMAX` override is
    needed at this scale (the canonical aMax=1300 only matters for the
    most-patient College atom, absent here).
    """
    ...
    mCount = int(os.environ.get("HAFISCAL_TM_MCOUNT", "200"))
    bd = compute_baseline_tm_data(
        econ, mCount=mCount, neutral_measure=False, verbose=False)
    ...
    for i, agent in enumerate(econ.agents):
        ...
        tm_data = build_tm_agg_fiscal_a(
            agent, aCount=mCount, Cratio=1.0, neutral_measure=False,
