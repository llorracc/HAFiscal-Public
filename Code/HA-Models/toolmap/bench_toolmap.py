...
def build_economy(parametrization):
    ...
    # UI-state encoding governs the micro-state count (7 under the default
    # calendar encoding with the paper's window, 6 under bug_fix, 4 under legacy);
    # the per-micro-state IncShkDstn lists below must match it. Mirrors welfare6_scenario.py's construction (the
    ...
    # num_base_MrkvStates under the active UI encoding (Policy_ExtraBenefitQuarters
    # extension states: 0 legacy, 3 calendar/window, 2 calendar/history and bug_fix).
    # Order: [state0, u1Q, ..., uK Q, (extension states), noBen].
