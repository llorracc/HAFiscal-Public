...
| Defensive `JAX_2B_THREADS`/`duration_workers` cap | **CLOSED 2026-07-30 — the premise was BUG-064** | The 2 OOM kills of 2026-06-03 were real, but the ~16 GB-per-fork diagnosis was not: every duration child was re-solving the whole economy, so each built its own solution set rather than sharing copy-on-write. Fixed in `429ca4a4`/`ec397137`; re-measured peak python RSS **1.5 GB** across a dw=1..16 sweep with 45+ GB free, and the pool **scales** (6.07x at dw=8), so the default moved dw=2 -> 8 and `--duration-workers 1` is no longer a workaround for anything. See `BUGS_private/HAFiscal_BUG-064_duration_loop_redundant_resolve.md`. |
