...
- **AD fixed point (welfare-6 `*_AD` cells):** **HYBRID replay-fed JAX-AD is the DEFAULT engine (owner-adopted 2026-08-02)** — HARK's exact update rule iterated with the compiled replay kernel on HARK's captured exogenous panel + capture-presolve cache + corrected-head sown-C (`CRATIO_PREV`) tables. AD stage 723 s vs 2855 s (~4×); **54-min** Baseline battery (vs 72 all-HARK); CPU-only (GPU dead at battery scale, with proof). One knob: `HAFISCAL_WELFARE_ENGINE=hybrid|hark` (`Code/HA-Models/welfare_engine.py`; `hark` = bit-identical pre-arc rollback; `as-corrected`/QE-fidelity worlds auto-resolve to `hark`). Accepted residual vs all-HARK: CRN-paired −0.5…−1.3% on AD cells, unattributed after 9 refutations, owner sig-figs ruling 2026-08-02. Certification: S=3 whole-cell band + frozen-anchor guards (`plans/20260802-0300h_canonical-hybrid-default_plan.md`).
