...
def effective_config(*, include_estimation_only: bool = True) -> dict:
    ...
    for var, world_val in base.items():
        ...
        if var in ENTRY_POINT_OWNED:
            # The world wiring never applies these, so the catalog's value is NOT what ran. Report
            # what is actually in the environment, and None when nothing is -- which for a flag
            # means OFF. Reporting the catalog here wrote "TM_A_INDEXED = 1" into the sidecars of
            # runs that had used the m-indexed engine (BUG-124).
            env_final[var] = actual
            if actual is None:
                not_applied.append(var)
            continue
    ...
    # Welfare-6 engine axis (canonical hybrid default 2026-08-02). Guarded
    # import: welfare_engine.py lives one level up (Code/HA-Models); when it
    # is not importable in a stripped context, report the raw env value.
    try:
        from welfare_engine import resolve_welfare_engine
        _welfare_engine = resolve_welfare_engine()
    except Exception:
        _welfare_engine = os.environ.get("HAFISCAL_WELFARE_ENGINE") or None
    ...
    return {
        ...
        "welfare_engine": _welfare_engine,
        ...
        # entry-point-owned settings that were NOT in the environment: the catalog has a value for
        # them and the run did not use it. For a flag that means OFF (BUG-124).
        "env_entry_point_unset": sorted(not_applied),
    }
