...
## Core they were validating (in `FromPandemicCode/`)
- `welfare6_tm.py` — analytical TM-a welfare-6 (all cells). NOTE (B11, 2026-06-11):
  the companion `welfare6_tm_aggregate.py` is NOT a live "canonical core" file — it has
  ZERO importers and zero string references anywhere live (verified 2026-06-11). It is
  keep-for-now pending a future archival round, not canonical.
- `welfare6_tm_joint5d.py` + `_baseline` + `_batch` — exact 5D joint TM (ui_rec)
- `welfare6_hybrid_table.py` — paper output. NOTE (B11, 2026-06-11):
  `welfare6_tm_make_tex.py` likewise has ZERO consumers (no importer / string ref) and
  is keep-for-now pending a future archival round, NOT canonical paper output.
