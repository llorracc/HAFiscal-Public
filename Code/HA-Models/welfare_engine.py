...
2. world guard: HAFISCAL_QE_FIDELITY=1 or HAFISCAL_WORLD=as-corrected force
   ...
   method);
3. default: 'hark' (OWNER RULING 2026-08-27: the hybrid bundle is not certified to
   reproduce the all-HARK battery -- CRN-paired -0.5..-1.3 % on the AD cells, unattributed;
   flipped from 'hybrid', the 2026-08-02 default. 'hybrid' stays available by explicit env.)
"""
...
def resolve_welfare_engine(env=None):
    ...
    if env.get("HAFISCAL_WORLD", "").strip().lower() == "as-corrected":
        return "hark"
    return "hark"   # owner ruling 2026-08-27 (was "hybrid", 2026-08-02)
