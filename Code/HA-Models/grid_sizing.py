...
calibration:
  ...
  * the transition-matrix DISTRIBUTION grid top `dist_aGrid_max` (hand-set 1300 via
    HAFISCAL_TM_AMAX; called bare `aMax` before the owner's 2026-07-25 rename — the bare
    spelling collided with the solve grid's `aXtraMax`).
...
The two grids answer DIFFERENT questions and key off DIFFERENT objects:
  ...
  TM grid     — "how far does the ergodic wealth DISTRIBUTION's upper tail reach?"  This tail is
                ...
                MEASURES it (`adaptive_grid_tm.production_dist_aGrid_max()`).  This module supplies a
...
see prompts_local/2026-06-24_grid-sizing-experiments-journal.md, scripts scratchpad/exp{1,2,3}*):
  ...
  * The ergodic wealth tail is Pareto with ζ≈3: q(1−p) ≈ x_min·p^{−1/ζ}.  For p=1e-4, ζ=3:
    ...
    production_dist_aGrid_max and adaptive_grid_tm's single-grid design).
...
distribution has no stationary point — `production_dist_aGrid_max()` cannot measure a quantile (mass
escapes the grid).  In that regime use the analytic `tm_dist_aGrid_max` (the Pareto/wealth-scale form),
...
=========================================================================
  ...
  * ϕ and the per-group tails: rerun scratchpad/exp2_tail.py (or adaptive_grid_tm.production_dist_aGrid_max()).
...
# --- HAFISCAL_SOLVE_GRID_PROFILE (remedy F, wired 2026-08-21; default 'full' = byte-identical
# no-op). 'hermite60' = the proven T2a operating point (PCHIP/Hermite capacity at 60-75 pts
# beat production c192; plan 20260803-2030h): setdefaults the count BASIS to 60 (per-group
# scaled counts land ~74/75 under the K*h_bar tops) and the T2a Hermite slices
# (HAFISCAL_SLICE_INTERP=hermite, honored by the AggFiscalModel PF-decay solver; on the
# Step-1 KinkedR path the same env flips SetupParamsCSTW CubicBool=True -- HARK's native
# cubic-Hermite solver branch, wired 2026-08-21 after the count-only arm drifted nabla +1.6%:
# the Hermite representation is load-bearing at 60-basis grids). Explicit env always
# wins over the profile (setdefault). Acceptance status: see the remedies plan F section.
_sgp = os.environ.get("HAFISCAL_SOLVE_GRID_PROFILE", "full").strip().lower()
if _sgp == "hermite60":
    os.environ.setdefault("HAFISCAL_AXTRA_COUNT", "60")
    os.environ.setdefault("HAFISCAL_SLICE_INTERP", "hermite")
elif _sgp not in ("full", ""):
    raise ValueError(
        f"HAFISCAL_SOLVE_GRID_PROFILE={_sgp!r}: expected 'full' or 'hermite60'")
import sys
import warnings
...
# never undersized. bar = target max relative decay-extrapolation error over [aXtraMax, dist_aGrid_max].
SOLVE_C1 = 0.04
SOLVE_BAR = 0.01            # 1% tail accuracy (Step-2 β is insensitive — moments sit at m≈6 ≪ grid)

# Count-converged scaling BASIS for the extended-grid (K·h̄ / SOLVE_AMAX / endogenous) paths
# (owner pre-approved ruling 2026-07-23: 192 — the HS_Only pipeline probe measured C192 at
# +0.4 min over C48; measured-Q is biased ~−0.10 by top-knot spacing at count 48 and silent
# at 192). An explicit HAFISCAL_AXTRA_COUNT or a code-level count override (legacy_count !=
# the site's anchor, e.g. FAST_GRIDS' 24) wins; pure-legacy paths are untouched.
COUNT_CONVERGED_BASIS = 192
# K·h̄ tops beyond this are near-R=Γ blowups (worst measured: Rfree_1005 College h̄≈7000 ⟹
# K·h̄≈21000) — the rule is validated only on main-config h̄; warn and point at SOLVE_AMAX.
DEEP_TOP_WARN = 5000.0


# ---------------------------------------------------------------------------
# SST solve-grid POLICY (owner ruling 2026-07-24: "I want a SST for this kind
# of thing, so that when I fix it in one context the other contexts will
# inherit the fix"). The precedence block below is THE single implementation;
# EstimParameters.py, FromPandemicCode/Parameters.py and the Step-1 setup
# (Estimation_BetaNablaSplurge) all consume it — no site carries its own copy.
# ---------------------------------------------------------------------------
def powerlaw_form_active(environ=None):
    """True iff the power-law PF-decay FORM is the effective default.

    THE predicate (SST) for 'is the tail power-law': ``HAFISCAL_PF_DECAY_EXTRAP``
    truthy — exactly the attach site's set, ``raw not in ('0','','false','False')``
    — and not the literal ``exp`` opt-out (case/whitespace-insensitive). Matches
    the production attach in ``AggFiscalModel.solve_agg_cons_markov_alt``
    byte-for-byte; guards/routers (fti_step1, the Step-2 NAMG guard, the JAX-2B
    attach gate, step1_powerlaw_tail) delegate here.
    """
    e = os.environ if environ is None else environ
    raw = e.get('HAFISCAL_PF_DECAY_EXTRAP', '1')
    return raw not in ('0', '', 'false', 'False') and raw.strip().lower() != 'exp'


def powerlaw_measured_active(environ=None):
    """True iff the MEASURED (local two-secant) Q source is selected on top of
    the power-law form — the predicate the K·h̄ grid rule keys off
    (``HAFISCAL_PF_DECAY_Q`` in {'measured', legacy alias 'local2'})."""
    e = os.environ if environ is None else environ
    # 'farfield' (2026-08-21, remedy-F/S1 arc) is a MEASURED-FAMILY Q source: the decay
    # form and the K*hbar solve-top world are unchanged; only WHERE the exponent is
    # measured moves (far-field mini-solve instead of top-knot secants). Omitting it here
    # silently reverted the whole grid world to legacy 20/20 — the H3 invalid-arm bug.
    # 'chart' (2026-08-22, MoM log-gap chart A/B) is likewise measured-family: same
    # K*hbar solve-top world; only the REPRESENTATION moves (re-chart, not re-measure).
    # Omitting a family member here silently reverts the grid world to legacy 20/20 --
    # the H3 invalid-arm bug, twice registered.
    return (powerlaw_form_active(e)
            and e.get('HAFISCAL_PF_DECAY_Q', 'measured').strip().lower()
            in ('local2', 'measured', 'farfield', 'chart'))


def resolve_solve_grid(*, Rfree, PermGroFac, legacy_aMax, legacy_count, aXtraMin,
                       environ=None, allow_endogenous=False,
                       endogenous_kwargs=None, count_basis_anchor=48,
                       tag='[grid_sizing]'):
    """THE solve-grid precedence (SST) for ONE (R, Γ) atom/group.

        HAFISCAL_SOLVE_AMAX                                (explicit override)
      > endogenous decay-reach rule                        (only when allow_endogenous —
          HAFISCAL_ENDOGENOUS_GRID truthy; EstimParameters' opt-in branch: solve_grid_aMax
          with C1/bar/tm_cap from HAFISCAL_GRID_C1/HAFISCAL_GRID_BAR/HAFISCAL_TM_AMAX)
      > K·h̄ under the measured-Q default                   (powerlaw_measured_active:
          aXtraMax = K·Γ/(R−Γ), K = HAFISCAL_PF_DECAY_AMAX_MULT default 3)
      > (legacy_aMax, legacy_count)                        (no trigger — byte-identical legacy)

    Count policy (every fired branch): scale to hold near-0 log-density via
    ``solve_grid_count`` from the count basis — ``legacy_count`` itself unless it
    equals ``count_basis_anchor`` (the site's unmodified code default: 48 for the
    production files, 20 for Step-1) with no explicit ``HAFISCAL_AXTRA_COUNT``,
    in which case the count-converged ``COUNT_CONVERGED_BASIS``=192 is used
    (owner ruling 2026-07-23). FAST_GRIDS/code overrides therefore win by
    arriving as a non-anchor ``legacy_count``.

    Returns ``(aXtraMax, aXtraCount, why)`` — ``why`` is None when no trigger
    fired (caller keeps legacy silently), else the branch-description string the
    call sites print. The deep-top warning (> DEEP_TOP_WARN) lives HERE, once,
    printed with the caller's ``tag``.

    ``endogenous_kwargs``: dict (or zero-arg callable returning one, so eager
    per-group evaluation can be avoided) with keys ``beta``, ``CRRA``,
    ``LivPrb`` for ``solve_grid_aMax``. Required when the endogenous branch
    fires.
    """
    e = os.environ if environ is None else environ
    solve_amax_override = e.get('HAFISCAL_SOLVE_AMAX')
    endog_on = allow_endogenous and (
        e.get('HAFISCAL_ENDOGENOUS_GRID', '0') not in ('0', '', 'false', 'False'))
    measured_on = powerlaw_measured_active(e)
    if not ((solve_amax_override not in (None, '')) or endog_on or measured_on):
        # Legacy passthrough UNCOERCED (int stays int): downstream cache keys /
        # params dicts must be byte-identical to the pre-SST sites, which left
        # their legacy literals untouched.
        return legacy_aMax, legacy_count, None

    if solve_amax_override not in (None, ''):
        aXtraMax_new = float(solve_amax_override)
        why = f"HAFISCAL_SOLVE_AMAX={aXtraMax_new:g} (all groups)"
    elif endog_on:
        kw = endogenous_kwargs() if callable(endogenous_kwargs) else endogenous_kwargs
        if not kw:
            raise ValueError("resolve_solve_grid: the endogenous branch fired but "
                             "endogenous_kwargs (beta/CRRA/LivPrb) was not supplied")
        c1 = float(e.get('HAFISCAL_GRID_C1', SOLVE_C1))
        bar = float(e.get('HAFISCAL_GRID_BAR', SOLVE_BAR))
        tm_cap = float(e.get('HAFISCAL_TM_AMAX', '1300'))
        aXtraMax_new = solve_grid_aMax(float(Rfree), float(kw['beta']), float(kw['CRRA']),
                                       float(kw['LivPrb']), PermGroFac=float(PermGroFac),
                                       C1=c1, bar=bar, tm_cap=tm_cap)
        why = f"HAFISCAL_ENDOGENOUS_GRID=1 (C1={c1:g} bar={bar:g} tm_cap={tm_cap:g})"
    else:
        K = float(e.get('HAFISCAL_PF_DECAY_AMAX_MULT', '3'))
        aXtraMax_new = K * float(PermGroFac) / (float(Rfree) - float(PermGroFac))
        why = f"HAFISCAL_PF_DECAY_Q=local2 solve-top rule aXtraMax=K·h̄ (K={K:g})"

    count_base = int(legacy_count)
    if 'HAFISCAL_AXTRA_COUNT' not in e and int(legacy_count) == int(count_basis_anchor):
        count_base = COUNT_CONVERGED_BASIS
    count = solve_grid_count(aXtraMax_new, aXtraMin=float(aXtraMin),
                             legacy_aMax=float(legacy_aMax), legacy_count=count_base)
    if aXtraMax_new > DEEP_TOP_WARN:
        print(f"{tag} WARNING: K·h̄/solve top is very deep ({aXtraMax_new:.0f}) — near-R=Γ "
              f"parametrization? Consider an explicit HAFISCAL_SOLVE_AMAX.")
    return float(aXtraMax_new), int(count), why

# TM grid analytic estimate: dist_aGrid_max ≈ ϕ·(1+h). Measured ϕ = q/(1+h) ∈ [6.24 College, 7.54 Dropout].
...
TM_ROUND_TO = 100.0         # match adaptive_grid_tm.production_dist_aGrid_max rounding
...
def kesten_zeta(GPFacRaw_Liv, perm_shock_vals, perm_shock_pmv, LivPrb=1.0):
    ...
    Both round to "≈3"; the measured ergodic quantile (adaptive_grid_tm.production_dist_aGrid_max) is ground truth.
...
# (so the Pareto scale x_min — and any quantile-derived dist_aGrid_max — is undefined), and the GPF
# is pushed to/over 1 (ζ→0, tail non-integrable). BOTH the wealth scale AND the tail shape blow up,
# so there is NO finite quantile-derived dist_aGrid_max. The defensive choice is a large fixed
# constant, far beyond any economically relevant resource the finite-horizon simulation visits.
...
def solve_grid_aMax(R, beta, CRRA, LivPrb, *, PermGroFac=None, MrkvArray=None,
    ...
    aXtraMax = math.log(C1 / bar) / MPCmin
    if tm_cap is not None:
        aXtraMax = min(aXtraMax, float(tm_cap))
    return float(aXtraMax)
...
def solve_grid_count(aXtraMax, *, aXtraMin, legacy_aMax, legacy_count):
    ...
    HARK's make_grid_exp_mult spaces `count` points roughly uniformly in LOG over
    [aXtraMin, aXtraMax], so the density per log-unit is count/ln(aXtraMax/aXtraMin). Extending
    aXtraMax at a fixed count therefore THINS the moment-relevant region (m≈6) — which shifts
    the estimated beta. To hold that density (and beta) constant, scale the count by the
    log-range ratio:

        count = ceil(legacy_count · ln(aXtraMax/aXtraMin) / ln(legacy_aMax/aXtraMin)).
...
# ---------------------------------------------------------------------------
def tm_dist_aGrid_max(hNrm, *, phi=TM_PHI, threshold=PARETO_THRESHOLD, round_to=TM_ROUND_TO):
    """Analytic estimate of the TM distribution-grid top `dist_aGrid_max` ≈ ϕ·(1+h).

    (Renamed from `tm_grid_aMax` per the owner ruling 2026-07-25 — the distribution-grid
    top is `dist_aGrid_max`; the old name remains a deprecated warning alias below.)
    ...
    USE:
      * As a CROSS-CHECK of the measured `production_dist_aGrid_max()` (flag if they disagree by >~25%).
      * As the GIC-Mod-FAILURE FALLBACK: when gap_MOD ≤ 0 the normalized ergodic does not converge
        and production_dist_aGrid_max cannot measure — then this analytic value (needs only finite h via
        FHWC) sizes the grid; pass phi=TM_PHI_CONSERVATIVE (7.5) so no atom is undersized.
    ...
    if not math.isfinite(h):
        warnings.warn(f"tm_dist_aGrid_max: hNrm={hNrm} is non-finite — FHWC fails (human wealth infinite); "
                      f"returning FHWC_FALLBACK_AMAX={FHWC_FALLBACK_AMAX} (BUG-061 §math (c)).")
        return float(FHWC_FALLBACK_AMAX)
    if h < 0.0:
        raise ValueError(f"tm_dist_aGrid_max: hNrm={hNrm} < 0 — human wealth cannot be negative; the (I−W) "
                         f"solve is broken. Review the income/transition inputs.")
    dist_aGrid_max = float(phi) * (1.0 + h)
    if round_to:
        dist_aGrid_max = math.ceil(dist_aGrid_max / round_to) * round_to
    return float(dist_aGrid_max)


def tm_grid_aMax(hNrm, *, phi=TM_PHI, threshold=PARETO_THRESHOLD, round_to=TM_ROUND_TO):
    """DEPRECATED alias for :func:`tm_dist_aGrid_max` (owner ruling 2026-07-25: the TM
    distribution-grid top is `dist_aGrid_max`; the bare `aMax` spelling collided with the
    solve grid's `aXtraMax`). Warns, then delegates unchanged — kept for one deprecation
    cycle (the HAFISCAL_MODE→MULTIPLIER_ENGINE precedent)."""
    warnings.warn(
        "grid_sizing.tm_grid_aMax() is deprecated; use tm_dist_aGrid_max() "
        "(owner ruling 2026-07-25 rename of the TM distribution-grid top; "
        "behavior unchanged).",
        DeprecationWarning, stacklevel=2)
    return tm_dist_aGrid_max(hNrm, phi=phi, threshold=threshold, round_to=round_to)


def pareto_tail_aMax(x_min, zeta, *, threshold=PARETO_THRESHOLD, round_to=TM_ROUND_TO):
    """Pareto-quantile TM-grid estimate: dist_aGrid_max ≈ x_min · threshold^{−1/ζ}.

    The structurally-honest form (x_min = the scale at which the power-law tail sets in, ζ = the
    Kesten exponent).  Exponentially ζ-sensitive — why production_dist_aGrid_max MEASURES rather
    than uses this — but it is the right object when GIC-Mod fails and no measured quantile exists.
    """
    ...
    dist_aGrid_max = x_min * threshold ** (-1.0 / zeta)
    if round_to:
        dist_aGrid_max = math.ceil(dist_aGrid_max / round_to) * round_to
    return float(dist_aGrid_max)
...
if __name__ == "__main__":
    ...
    tg = tm_dist_aGrid_max(h_college)
    print(f"tm_dist_aGrid_max(h=195.1, phi=6.5) = {tg:.0f}  (production_dist_aGrid_max = 1300)")
    print(f"tm_dist_aGrid_max(h=195.1, phi=7.5 conservative) = {tm_dist_aGrid_max(h_college, phi=TM_PHI_CONSERVATIVE):.0f}")
    print(f"pareto_tail_aMax(x_min=57, zeta=3) = {pareto_tail_aMax(57.0, 3.0):.0f}  (production_dist_aGrid_max = 1300)")
