...
def _picard_sweep_factory(inp, DiscFac, tail_Q=None, tail_q_fn=None):
    """Build the in-formulation Picard-EGM sweep map X -> sweep(X) with the SAME
    floating slope-Q power-law tail as ConsumedATI 'chain' mode — or, when
    ``tail_Q`` (per-state array) is given, the tail PINNED at those exponents —
    or, when ``tail_q_fn(m_knots, c_knots, MPCmin, h) -> Q`` is given, the tail
    re-MEASURED from each sweep's own knots by that estimator (NaN -> exp tail):
    the same-fixed-point reference for a measured-Q ('lagged' + tail_q_fn,
    BUG-089) solve is this floating measured-Q map (the lagged mode's own map).
    ...
    _tq = None if tail_Q is None else np.asarray(tail_Q, dtype=float).reshape(-1)

    def _cont(Xj, j):
        if tail_q_fn is not None:
            q = float(tail_q_fn(aN + Xj, Xj, MPCmin, float(hNrm[j])))
            q = q if (np.isfinite(q) and q > 0.0) else float("nan")
        else:
            q = None if _tq is None else float(_tq[j])
        return _state_continuation(Xj, aN, hNrm[j], MPCmin, cFuncCnst,
                                   tail_form="powerlaw", tail_Q=q)
...
def _picard_powerlaw_arm(inp, DiscFac, move_tol=1e-13, egm_stop_tol=1e-6,
    ...
    sweep, _cont, X, MPCmin, hNrm = _picard_sweep_factory(inp, DiscFac, tail_Q=tail_Q,
                                                          tail_q_fn=tail_q_fn)
    S = inp["S"]
    moves = []
    egm_stop = None
