...
# G2 newborn-pool size (draw-structure plan, Fix B discriminator): the pool's
# sampling error is FROZEN (one draw reused for every death), acting as a
# persistent wealth-distribution perturbation. If that error drives the
# Check/TaxCut AD-cell offsets, they must shrink ~1/sqrt(pool_N). Keyed
# backward-compatibly via _HAFISCAL_NUMERICAL_ENV_VARS_IF_SET (keys.py).
_NB_POOL_N = int(os.environ.get("HAFISCAL_JAX_MC_NEWBORN_POOL_N", "10000"))
...
def _refuse_under_earnings_phase(where):
    """The JAX MC kernel evolves the J employment micro states only (init_panels carry micro0 = Mrkv % J,
    no phase; a per-J transition); under the earnings phase (2J micro states, a per-agent hazard) it would
    silently simulate every household as growing. Refuse loudly (the production welfare engine is HARK)."""
    import earnings_phase as _ep
    if _ep.enabled():
        raise RuntimeError(
            f"{where}: the JAX MC kernel has no earnings-phase state ({_ep.ENV}="
            f"{os.environ.get(_ep.ENV)!r}); use the HARK engine (HAFISCAL_WELFARE_ENGINE=hark) or unset the hazard.")


def _build_init_panels_via_hark_quick_sim(eco, shock_type, verbose=True):
    _refuse_under_earnings_phase("_build_init_panels_via_hark_quick_sim")
    ...
    if fast:
        ...
        eco_ref.sow_init['CratioNow'] = eco_ref.CFunc[0][
            EconomyMrkv_init[0] * eco_ref.num_micro_states].intercept   # block stride (= J; 2J under the phase)
...
def _run_vmap_all_cohorts_inline(
    ...
    # Newborn pools — same pool_N per cohort (default 10000; env-overridable)
    pool_N = _NB_POOL_N
    nbA_stk = np.zeros((n_cohorts, pool_N), dtype=np.float32)
    nbP_stk = np.zeros_like(nbA_stk)
    for c, agent in enumerate(eco.agents):
        # Newborn seed derives from seeds[0] so a reseeded run (whole-cell
        # --seed-offset) moves this stream too; seeds is already in the AD cache
        # key, so the derivation is keyed for free. seeds[0]=0 -> 99+c, the
        # historical literal, bit-identically.
        nbA, nbP, _ = draw_newborn_pool_from_agent(
            agent, pool_N=pool_N, seed=99 + c + int(seeds[0]) * 7907)
        nbA_stk[c] = nbA
        nbP_stk[c] = nbP
...
def solve_ad_recession_jax_multicohort(eco, base_AggCons,
    ...
    _refuse_under_earnings_phase("solve_ad_recession_jax_multicohort")
    ...
    # `or 100` would silently coerce an UNCAPPED agent (T_age=None under
    # HAFISCAL_T_AGE=none) to a 100-quarter cap — the exact bias this comment
    # block describes, reintroduced silently. Refuse by default: the belief-
    # consistent production path for uncapped welfare AD is HARK-AD
    # (welfare6_scenario routing, 2026-07-27). EVAL/DEV opt-in
    # (HAFISCAL_USE_JAX_MC_UNCAPPED=1, owner-ordered GPU evaluation the same
    # night): substitute the tolerance-truncated effective age chain the TM
    # engines themselves use — survivorship ≤1e-9 beyond the buffer, so the
    # implied wall carries ~1e-9 mass (belief-consistent to tolerance).
    # T_age_max is a jit-static KILL THRESHOLD only (jax_mc_ad_shuffle
    # alive_mask), so a ~3300-quarter chain costs one recompile, no arrays.
    if any(getattr(a, 'T_age', None) is None for a in eco.agents):
        if os.environ.get('HAFISCAL_USE_JAX_MC_UNCAPPED', '').lower() in ('1', 'on', 'true'):
            from tm_methods import effective_age_chain_length
            cohort_T_age = []
            for a in eco.agents:
                if getattr(a, 'T_age', None) is None:
                    cohort_T_age.append(int(effective_age_chain_length(
                        float(a.LivPrb[0][0]), None)))
                else:
                    cohort_T_age.append(int(a.T_age))
            print(f"  [jax-ad-mc] UNCAPPED EVAL MODE "
                  f"(HAFISCAL_USE_JAX_MC_UNCAPPED): T_age=None -> effective "
                  f"chain {sorted(set(cohort_T_age))} (survivorship-1e-9 "
                  f"truncation; production default remains HARK-AD)", flush=True)
        else:
            raise ValueError(
                "jax_mc_ad_multicohort: agent has T_age=None (HAFISCAL_T_AGE=none). "
                "The JAX-MC kernel requires a finite age cap; run this tool with a "
                "capped T_age or use the TM engines (which handle no-cap exactly), "
                "or set HAFISCAL_USE_JAX_MC_UNCAPPED=1 for the effective-age-chain "
                "EVAL port.")
    else:
    ...
    for it in range(num_max_iterations):
        ...
        for c_idx, agent in enumerate(eco.agents) if not ran_vmap_cohorts else []:
            c_start = time.time()
            if use_shuffle:
                # The stratified-shuffle kernel (simulate_jax_ad_shuffle) consumes
                # the legacy per-period table and predates the 1A/1C lifts; those
                # fast paths are bypassed under use_shuffle (gates at the vmap
                # sites), so build the table it actually needs regardless of the
                # lift flags. Without this, 2D_LIFT=1 + use_shuffle=True died with
                # UnboundLocalError on cfunc_table (found 2026-07-31, G1 smoke).
                inp = extract_recession_kernel_inputs(agent, scenario=shock_type)
                m_grid = np.asarray(inp['m_grid'])
                cfunc_table = extract_cfunc_table_per_period(
                    agent, Cratio_obs, m_grid, n_combined=n_combined)
            elif _USE_2D_LIFT and _BATCH_TABLES and batched_cfunc_jax is not None:
                ...
                cfunc_table = extract_cfunc_table_per_period(
                    agent, Cratio_obs, m_grid, n_combined=n_combined)
            # Same seeds[0]-derived offset as the vmap-cohorts path above.
            nbA, nbP, _ = draw_newborn_pool_from_agent(
                agent, pool_N=_NB_POOL_N, seed=99 + c_idx + int(seeds[0]) * 7907)
        ...
        if Total_Diff < convergence_cutoff:
            converged = True
            # BELIEF CONSISTENCY (owner ruling 2026-07-29). This loop solves the
            # agents at the START of an iteration, i.e. under Old_CFunc, and then
            # replaces eco.CFunc with Step_CFunc above. Breaking here would leave
            # the agents' policies solved under one rule while the simulation runs
            # under another -- the exact mismatch the invariant forbids. (HARK's
            # solve_ad_recession avoids it by solving AFTER its CFunc update.)
            # Convergence means the two rules differ by less than the cutoff, so
            # the right move is the same as for the post-AD sniff test: DISCARD
            # the marginally-updated rule and keep the one the policies were
            # actually solved under. MEASURED consequence of not doing this: the
            # stale pairing shifted welfare aggregates ~0.4% and was silently
            # repaired by a redundant re-solve in the duration loop.
            eco.CFunc = Old_CFunc
            for agent in eco.agents:
                agent.CFunc = eco.CFunc
            if verbose:
                print(f"  [jax-ad-mc] converged (Total_Diff={Total_Diff:.4g} < "
                      f"{convergence_cutoff:g}); keeping the CFunc the policies "
                      f"were solved under (discarding the final step update)",
                      flush=True)
            break
