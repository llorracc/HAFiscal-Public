...
def find_ergodic_distribution(TranMatrix, method='power', tol=1e-12, max_iter=5000):
    ...
    Two caveats this existence argument does NOT cover:
      * If GPF_out ≥ 1 (shave disabled / theGICfactor raised past the boundary), no
        ...
        point (aMax) and the result is a truncation artifact, not pi^*. Only finite-
        horizon MC is meaningful in that regime.
      * Even with GPF_out < 1, a most-patient atom with GPF_in > 1 has a FAT upper
        tail cut off only by mortality, so `aMax` must be tall enough to contain it
        (the (1−1e-4) ergodic aNrm quantile; see `adaptive_grid_tm.production_aMax`,
        which is sized by exactly this GIC-edge atom). Too-low aMax biases pi^*.
