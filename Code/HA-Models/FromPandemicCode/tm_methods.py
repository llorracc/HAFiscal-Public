...
import earnings_phase as _ep   # earnings-phase SST (Code/HA-Models on sys.path via EstimParameters)
import onset_spike_rule as _osr   # onset-spike SST (same path); BUG-122 Step 0


# ----------------------------------------------------------------------------- earnings-phase helpers
# Layout Mrkv = J_full*macro + J*phase + emp (J = num_base_MrkvStates, J_full = 2J when the phase is on).
# Every TM builder below works on ONE macro block of J_full micro states ("J_micro"); the employment
# decode of a micro index is `emp_of(j, J)`; the newborn allocation is the growing-block ergodic.
def _J_base_of(J_micro):
    """The employment count J from a micro block size (J_full = n_phases * J)."""
    return int(J_micro) // _ep.n_phases()


def _is_employed_micro(jp, J_micro):
    """Micro state jp of a J_micro block is an EMPLOYED state (emp == 0: state 0, and J under the phase)."""
    return int(_ep.emp_of(jp, _J_base_of(J_micro))) == 0


def _emp_mass(dist, A, J_micro):
    """Mass of the employed states in an (A*J_micro)-long, state-major distribution. Phase off: the
    old expression `np.sum(dist[:A])` verbatim (byte-identity); on: state 0 plus the matured-employed state."""
    if not _ep.enabled():
        return np.sum(dist[:A])
    Jb = _J_base_of(J_micro)
    return sum(np.sum(dist[j * A:(j + 1) * A]) for j in range(int(J_micro)) if _ep.emp_of(j, Jb) == 0)


def _state_fracs_of(dist, A, J_micro):
    """Per-micro-state masses of an (A*J_micro)-long distribution."""
    d = np.asarray(dist, dtype=np.float64)
    return np.array([np.sum(d[j * A:(j + 1) * A]) for j in range(int(J_micro))])


def _agg_pLvl_growth(agent, u_rate, state_fracs=None):
    """Per-period growth factor of aggregate permanent income on a state distribution. Phase off: the
    scalar SST rule (1-u) G_emp + u G_u (income_process_sst.effective_pLvl_growth, byte-identical). Phase
    on: sum_j f_j PermGroFac_j over the per-micro-state fractions -- the matured states grow at 1, so
    a scalar g would overstate the growth of E[p] by the matured (income-weighted) share."""
    if not _ep.enabled():
        return effective_pLvl_growth(agent, u_rate)
    f = np.asarray(state_fracs, dtype=np.float64)
    f = f / f.sum()
    G = np.asarray(agent.PermGroFac[0], dtype=np.float64)[:len(f)]
...
# HAFISCAL_TM_Q_METHOD (BUG-093, 2026-08-25): the ONE construction of the Q-measure (p-weighted)
# objects on the a-indexed TM path — the BASE level's ergodic, the experiments' STARTING
# distribution and the KERNEL the experiments propagate with. Before BUG-093 the start was
# 'cohort'->'doob' while base + kernel were the plain Q-kernel ('bst'): a null experiment (no
# recession) started 3.7 % above base at Baseline (+31 % at the most patient college atom,
# income exactly at base) because the plain kernel injects newborn p-mass at the population
# death rate delta while the true p-weighted dynamics inject at 1-(1-delta)G (~4.5x smaller:
# survivors' mass grows by G every quarter), so its stationary distribution under-weights the
# centuries-old dynasties that carry the uncapped world's tail.
#   'doob'   (default) Fix 4's exact perpetual-youth Q-marginal pi_Q = f/sum(f), (I - T_S,p) f =
#            delta_bar*pi_N, AND the matching step f' = T_S,p f + w_t pi_N (w_t = (delta_bar/p_bar)
#            / pLvl_factor_t), of which pi_Q is the exact fixed point -> null experiment == 1.
#   'bst'    the plain Q-kernel eigenvector and step (BST Theorem 1): exact under its own kernel,
#            biased vs the true p-weighted dynamics under mortality + growth.
#   'cohort' the cap-exact Q-marginal START (BUG-038; requires T_age) with the plain step —
#            the documented ~1 % L1 residual; auto-routes to 'doob' when T_age is None.
# Resolution order everywhere: explicit argument, then this env var, then the module default.
_HAFISCAL_TM_Q_METHOD_DEFAULT = 'doob'
_Q_METHODS = ('doob', 'cohort', 'bst')


def resolve_q_method_public():
    """The Q-construction in force (explicit env or the module default) — for provenance
    (the equilibrium store records it as a producer convention; BUG-093)."""
    return _resolve_q_method(None, verbose=False)


def _resolve_q_method(q_method=None, verbose=True):
    """The one Q-construction switch: explicit arg > HAFISCAL_TM_Q_METHOD > module default."""
    _q_env = _os.environ.get('HAFISCAL_TM_Q_METHOD', '').strip().lower()
    if _q_env:
        if _q_env not in _Q_METHODS:
            raise ValueError(
                f"HAFISCAL_TM_Q_METHOD={_q_env!r} not recognized; valid: doob, cohort, bst")
        if verbose and q_method is not None and q_method != _q_env:
            print(f"[tm-baseline] HAFISCAL_TM_Q_METHOD={_q_env!r} overrides q_method={q_method!r}",
                  flush=True)
        return _q_env
    q = _HAFISCAL_TM_Q_METHOD_DEFAULT if q_method is None else q_method
    if q not in _Q_METHODS:
        raise ValueError(f"q_method must be 'doob', 'cohort', or 'bst', got: {q!r}")
    return q


class _DoobStep(object):
    """The p-weighted survival step of the Q-distribution (BUG-093, q_method='doob'):
    f' = T_S,p f + w * NewBornDist, applied through ``_q_step`` (the newborn p-mass weight
    w = (delta_bar/p_bar)/pLvl_factor_t is period-specific, so this is deliberately NOT a
    matrix: a stray ``TM @ dist`` on it fails loudly instead of silently taking the plain step)."""
    __slots__ = ('T_surv_p', 'NewBornDist')

    def __init__(self, T_surv_p, NewBornDist):
        self.T_surv_p = T_surv_p
        self.NewBornDist = np.asarray(NewBornDist, dtype=np.float64)

    def __matmul__(self, other):
        raise TypeError("_DoobStep must be applied with _q_step(TM, dist, w) — it needs the "
                        "period's newborn p-mass weight (BUG-093).")


def _q_step(TM, dist, w=None):
    """One period of the (normalized) Q-distribution. Plain kernel: ``TM @ dist``. Doob kernel:
    ``T_S,p @ dist + w * pi_N`` — the caller normalizes afterwards, exactly as for the plain step
    (the total (1-delta)g + w is the p-mass growth the pLvl_factor recurrence tracks)."""
    if isinstance(TM, _DoobStep):
        if w is None:
            raise ValueError("_q_step: the Doob step needs the newborn p-mass weight w")
        out = TM.T_surv_p @ dist
        out = np.asarray(out.todense()).ravel() if sp.issparse(out) else np.asarray(out).ravel()
        return out + float(w) * TM.NewBornDist
    return TM @ dist


def _baseline_ergodic_a(agent, dist_aGrid_count, neutral_measure, q_method, interp,
                        Cratio=1.0, label=''):
    """The a-indexed BASE distribution built by ONE method (BUG-093) — shared by
    ``compute_baseline_tm_data`` (the experiments' start) and ``run_experiment_tm`` (the base
    level). Returns ``(tm_base, ergodic, q_method_used, doob_inj)``; ``doob_inj`` is the
    stationary newborn p-mass share delta_bar/p_bar = 1 - sum(T_S,p pi_Q) of the Doob
    construction (None otherwise) — the exact injection weight of the matching step."""
    q_method_i = q_method
    # Belief-consistency auto-route (2026-07-27): 'cohort' is the CAP-exact Q-measure method
    # (BUG-038: K_max = T_age-1 recursion — it REQUIRES a cap); 'doob' is the perpetual-youth
    # exact method. Under T_age=None the 'cohort' request routes to 'doob' instead of raising.
    if (q_method == 'cohort' and neutral_measure
            and getattr(agent, 'T_age', None) is None):
        print(f"[tm-baseline] {label}T_age=None -> q_method 'cohort' "
              f"auto-routes to 'doob' (perpetual-youth exact)", flush=True)
        q_method_i = 'doob'
    doob_inj = None
    if neutral_measure and q_method_i == 'doob':
        # Doob path: build TM-P, find pi_P, then derive pi_Q^true via the Doob h-transform
        # (Fix 4). Avoids BST Theorem 1 bias under mortality. Under a T_age cap, uses the
        # _effective_LivPrb approximation (~1% L1 error vs cohort exact); math doc §25.
        tm_base_P = build_tm_agg_fiscal_a(agent, aCount=dist_aGrid_count, Cratio=Cratio,
                                          neutral_measure=False,
                                          interpretation=interp)
        pi_P = find_ergodic_distribution(tm_base_P['TranMatrix'])
        doob_out = compute_doob_pi_q_a(agent, tm_base_P, pi_P,
                                       Cratio=Cratio, interpretation=interp)
        tm_base = tm_base_P
        ergodic = doob_out['pi_Q_doob']
        _Tf = doob_out['T_surv_p'] @ ergodic
        _Tf = np.asarray(_Tf.todense()).ravel() if sp.issparse(_Tf) else np.asarray(_Tf).ravel()
        doob_inj = float(1.0 - np.sum(_Tf))
    elif neutral_measure and q_method_i == 'cohort':
        # BUG-038 cohort path: build TM-P, then compute pi_Q analytically via the Q-twisted
        # cohort-age decomposition truncated at K_max = T_age - 1. Exact under cap.
        tm_base_P = build_tm_agg_fiscal_a(agent, aCount=dist_aGrid_count, Cratio=Cratio,
                                          neutral_measure=False,
                                          interpretation=interp)
        ...
        # D-8 (2026-05-05): 'degenerate' matches qe_fidelity's psi_unemp=1 behavior;
        # selectable via HAFISCAL_COHORT_UNEMP_SHOCKS.
        _unemp_shocks_mode = _os.environ.get(
            'HAFISCAL_COHORT_UNEMP_SHOCKS', 'degenerate')
        ergodic, _ = compute_pi_q_via_cohort_age(
            agent, tm_base_P, T_age=T_age_agent, Cratio=Cratio,
            interpretation=interp,
            unemp_shocks=_unemp_shocks_mode)
        tm_base = tm_base_P
    else:
        tm_base = build_tm_agg_fiscal_a(agent, aCount=dist_aGrid_count, Cratio=Cratio,
                                        neutral_measure=neutral_measure,
                                        interpretation=interp)
        ergodic = find_ergodic_distribution(tm_base['TranMatrix'])
    return tm_base, ergodic, q_method_i, doob_inj
...
def compute_E_pLvl_exact(agent, unemployment_rate=None):
    ...
    # the base micro block: J employment states, or 2J = [growing J | matured J] under the earnings
    # phase (a [:J] slice would keep the growing block only, scaled by 1-h -- a sub-stochastic chain)
    J = _ep.j_full(int(agent.num_base_MrkvStates))
    ...
    # Birth-state distribution: ergodic stationary of MrkvBase (earnings phase: the employment
    # chain's ergodic vector placed in the GROWING block -- the wrapped chain's own stationary
    # vector is degenerate, matured being absorbing)
    if _ep.enabled():
        birth_fracs = _ep.growing_block_ergodic(MrkvBase, int(agent.num_base_MrkvStates))
    else:
...
def compute_pLvl_distribution(agent, n_points=200, unemployment_rate=None):
    ...
    """
    if _ep.enabled():
        # Earnings phase: the scalar-g age mixture cannot represent the matured share; use the exact
        # per-(age, state) Markov mixture (the same object the MC seed and the Gini gate use).
        age_prbs, mu_k, sigma_k = _pLvl_markov_mixture_components(agent)
    else:
...
def _pLvl_markov_moment_recursion(agent):
    ...
    # the base micro block (2J under the earnings phase: the recursion then runs over the wrapped chain,
    # the hazard moving mass to the matured states whose log-growth is 0)
    J = _ep.j_full(int(agent.num_base_MrkvStates))
    MrkvBase = np.asarray(agent.MrkvArray[0], dtype=float)[:J, :J]
    PermGroFac_arr = np.asarray(agent.PermGroFac[0], dtype=float)[:J]
    ...
    # Birth distribution = ergodic stationary of MrkvBase (earnings phase: the employment
    # chain's ergodic vector placed in the GROWING block -- the wrapped chain's own stationary
    # vector is degenerate, matured being absorbing)
    if _ep.enabled():
        birth_fracs = _ep.growing_block_ergodic(MrkvBase, int(agent.num_base_MrkvStates))
...
def compute_pLvl_distribution_Q(agent, p_powers, n_points=200, unemployment_rate=None):
    ...
    J = _ep.j_full(int(agent.num_base_MrkvStates))   # the base micro block (2J under the earnings phase)
    MrkvBase = np.asarray(agent.MrkvArray[0], dtype=float)[:J, :J]
    PermGroFac_arr = np.asarray(agent.PermGroFac[0], dtype=float)[:J]

    # Ergodic micro-state fractions for birth distribution (earnings phase: births in the growing block)
    if _ep.enabled():
        birth_fracs = _ep.growing_block_ergodic(MrkvBase, int(agent.num_base_MrkvStates))
    else:
...
def _prop_cap_T_age(T_age):
    """BUG-092 localization knob (diagnostic, opt-in): ``HAFISCAL_TM_PROP_CAP_T_AGE=<int>``
    makes the AD-TM PROPAGATION (the mortality-replacement share and the standard-measure
    state-fraction recurrence in ``propagate_experiment_tm_a``) use the capped effective
    survival ``_effective_LivPrb(L, <int>)`` while the baseline ergodic and everything
    else keep the agent's own ``T_age`` — "cap only in propagation". Unset = unchanged."""
    raw = _os.environ.get('HAFISCAL_TM_PROP_CAP_T_AGE', '').strip()
    if not raw:
        return T_age
    try:
        return int(raw)
    except ValueError:
        raise ValueError(f"HAFISCAL_TM_PROP_CAP_T_AGE={raw!r} is not an integer")
...
def _propagate_state_fracs(state_fracs, micro_trans, LivPrb_arr, T_age):
    ...
    """
    T_age = _prop_cap_T_age(T_age)   # BUG-092 knob (unset = unchanged)
...
def build_tm_agg_fiscal(agent, mCount=_HAFISCAL_TM_MCOUNT_DEFAULT, mMin=0.001, mMax=None, mFac=3,
    ...
    # Markov stationary distribution (for newborn allocation; the growing-block ergodic under the phase)
...
def find_ergodic_distribution(TranMatrix, method=None, tol=1e-12, max_iter=5000):
    """
    Find the ergodic distribution of a column-stochastic transition matrix.

    method=None (every production call site) resolves from HAFISCAL_TM_ERGODIC_SOLVER:
      'power'  — the legacy 1e-12 power iteration. Its cost blows up as the chain goes
                 near-unit-root (spectral gap -> 0), which is exactly the GIC-edge
                 patient-atom regime: the 2026-08-20 speedup plan measured it as the
                 dominant term of College Step-2 evaluations (~30-60 s each).
      'direct' — speedup remedy B (owner-ordered 2026-08-20): pi as the solution of the
                 BORDERED sparse linear system (T - I)pi = 0 with sum(pi)=1 (last row of
                 the singular system replaced by the normalization). Deterministic, no
                 iteration, runtime INDEPENDENT of the spectral gap; degenerate solves
                 fall back to 'power' loudly. Default after the parity gate.
      'eigs'   — ARPACK leading eigenvector (the pre-existing alternative branch; kept
                 as a cross-check; can be slow when the subdominant eigenvalue clusters
                 near 1, i.e. precisely the regime 'direct' handles).
    ...
    if method is None:
        method = _os.environ.get('HAFISCAL_TM_ERGODIC_SOLVER', 'direct').strip().lower()
        if method not in ('power', 'direct', 'eigs'):
            raise ValueError(f"HAFISCAL_TM_ERGODIC_SOLVER must be power|direct|eigs; got {method!r}")

    if method == 'direct':
        # Remedy B: bordered sparse solve. (T - I) has rank N-1 for an irreducible
        # column-stochastic chain (the death->newborn reset guarantees irreducibility;
        # see the existence discussion above); replacing one row with the
        # normalization sum(pi) = 1 makes it nonsingular, and the solution is pi*
        # exactly (to LU precision) regardless of how small the spectral gap is.
        T = TranMatrix if sp.issparse(TranMatrix) else sp.csc_matrix(TranMatrix)
        A = (T - sp.identity(N, format='csc')).tolil()
        A[N - 1, :] = 1.0
        b = np.zeros(N)
        b[N - 1] = 1.0
        try:
            ...
            print(f"[ergodic direct] sparse solve raised {_e!r} -> falling back to power iteration")
            return find_ergodic_distribution(TranMatrix, method='power', tol=tol, max_iter=max_iter)
        pi = np.asarray(pi).flatten().real
        s = pi.sum()
        neg = float(np.abs(pi[pi < 0]).sum())
        if (not np.isfinite(s)) or s <= 0 or neg > 1e-8:
            print(f"[ergodic direct] degenerate solution (sum={s!r}, |neg|={neg:.3g}) "
...
def compute_kernels(
    ...
    **CRRA ≠ 1 — felicity power in the inner loop.** For every requested power ``k``,
    ...
    u_rate = float(1.0 - _emp_mass(erg, M, J))   # employed = state 0 (+ the matured-employed state)
...
def run_experiment_tm(economy, shock_type="base", dist_aGrid_count=None, Cratio=1.0,
                      ...
                      mCount=None, q_method=None, baseline_tm_data=None):
    """
    ...
    # BUG-093 (2026-08-25): the base level, the experiments' starting distribution and the
    # propagation kernel must be ONE construction (see _resolve_q_method). Reuse the caller's
    # baseline_tm_data when given (Step 5a computes it first); else build by the same helper.
    _q_resolved = _resolve_q_method(q_method, verbose=verbose)
    type_results = []
    for i, agent in enumerate(agents):
        ...
        if use_a:
            _bd_i = None
            if (baseline_tm_data is not None and i < len(baseline_tm_data)
                    and baseline_tm_data[i].get('tm_a_indexed', False)
                    and baseline_tm_data[i].get('tm_data') is not None
                    and float(Cratio) == 1.0):
                _bd_i = baseline_tm_data[i]
            if _bd_i is not None:
                tm_data, ergodic = _bd_i['tm_data'], _bd_i['ergodic']
            else:
                tm_data, ergodic, _, _ = _baseline_ergodic_a(
                    agent, dist_aGrid_count, neutral_measure, _q_resolved, interp,
                    Cratio=Cratio, label=f"agent {i}: ")
        else:
    ...
    if compute_welfare:
        ...
        for i, agent in enumerate(agents):
            ...
            epk_ov = None
            if neutral_measure:
                _Jq = _ep.j_full(int(agent.num_base_MrkvStates))
                _erg_1d = np.asarray(erg, float).ravel() / np.sum(erg)
                _u = float(1.0 - _emp_mass(_erg_1d, len(tm_data["dist_mGrid"]), _Jq))
                if abs(rho - 1.0) < 1e-12:
...
def _compute_check_buckets(agent, E_pLvl=None, n_buckets=None, unemployment_rate=None):
    ...
    """
    J_micro = _ep.j_full(agent.num_base_MrkvStates)   # per-micro-state arrays span the whole block
    ...
    for b in range(n_buckets):
        ...
        for j in range(J_micro):
            if _is_employed_micro(j, J_micro):
...
def _tax_cut_employed_factor(agent, macro_t, shock_type):
    ...
    experiment on the exact TM evolution): the published code SOLVED the tax cut with the
    typo (one PermShk atom x TaxCutIncFactor) but its Monte Carlo PAID the real cut (base
    shock histories x tax_cut_multiplier on employed TranShk). The TM mirrors that by keeping
    the typo'd solve (Simulate.py rebuilds it) and applying the run-time TranShk rescaling
    in the tax-cut window -- the pre-fix body of this function with the BUG-091 SST window --
    while build_experiment_period_tm undoes the typo'd PermShk atom in the KERNEL so the
    evolved process is the paper's simulated one (base PermShk, TranShk x TaxCutIncFactor).
    """
    if _os.environ.get('HAFISCAL_LEGACY_TAXCUT_ATOM', '').strip().lower() in ('1', 'on', 'true', 'yes'):
        if shock_type not in ('TaxCut', 'recessionTaxCut'):
            return 1.0
        from income_process_sst import taxcut_block_range
        _lo, _hi = taxcut_block_range(int(getattr(agent, 'TaxCutPeriods', 8)), agent.num_base_MrkvStates)
        mrkv_emp = macro_t * _ep.j_full(agent.num_base_MrkvStates)   # the block's first state
        if mrkv_emp > _lo - 1 and mrkv_emp < _hi:
            return float(getattr(agent, 'TaxCutIncFactor', 1.0))
        return 1.0
    return 1.0
...
def build_experiment_period_tm(agent, macro_t, macro_next, dist_mGrid, Cratio=1.0,
    ...
    """
    J_micro = _ep.j_full(agent.num_base_MrkvStates)   # one macro block = 2J micro states under the phase
    M = len(dist_mGrid)
    ...
    for jp in range(J_micro):
        ...
        if _is_employed_micro(jp, J_micro) and employed_tran_shk_scale != 1.0:
            if _os.environ.get('HAFISCAL_LEGACY_TAXCUT_ATOM', '').strip().lower() in ('1', 'on', 'true', 'yes'):
                # the paper's experiment: the kernel evolves the BASE PermShk (undo the typo'd atom the
                # solve used) with the real transitory cut applied below (see _tax_cut_employed_factor)
                raw = deepcopy(raw)
                _p = np.asarray(raw.atoms[0], dtype=np.float64).copy(); _p[1] = _p[1] / employed_tran_shk_scale
                raw.atoms = (_p, np.asarray(raw.atoms[1], dtype=np.float64))
    ...
    micro_trans = _ep.wrap_chain(agent.CondMrkvArrays[macro_next])   # the 2J x 2J block under the phase
...
def _solve_markov_ergodic(trans_matrix):
    ...
    Every caller uses it for the NEWBORN allocation ("newborns enter at ergodic fractions"). Under
    the earnings phase a one-macro-block chain is the wrapped [[(1-h)M, hM],[0, M]] and its
    stationary vector is degenerate (all mass matured), so the newborn distribution is the
    employment chain's ergodic vector in the GROWING block (earnings_phase.growing_block_ergodic).
    Phase off: the lstsq solve verbatim."""
    trans_matrix = np.asarray(trans_matrix, dtype=np.float64)
    if _ep.enabled():
        n = trans_matrix.shape[0]
        Jb = n // _ep.n_phases()
        if n == _ep.j_full(Jb) and not np.any(trans_matrix[Jb:, :Jb]):
            return _ep.growing_block_ergodic(trans_matrix, Jb)
    P = trans_matrix.T
    n = P.shape[0]
...
def _compute_post_hit_urate(agent, initial_macro_state):
    ...
    if _ep.enabled():
        # the employment composition is phase-independent (identical chains in both phases):
        # compute on the growing block's employment chain (J long)
        pi_erg = _ep.growing_block_ergodic(base_mrkv, J)[:J]
    else:
...
def adapt_initial_distribution(baseline_ergodic, agent, initial_macro_state,
    ...
    """
    J_micro = _ep.j_full(agent.num_base_MrkvStates)
    M = len(dist_mGrid)
    ...
    # the spike lays off the employed of EVERY phase (state 0, and J_base under the phase): emp 0 -> 1 in place
    for _ph in range(_ep.n_phases()):
        _e = int(_ep.compose(_ph, 0, 0, agent.num_base_MrkvStates))
        employed_mass = dist[_e*M : (_e+1)*M]
        dist[_e*M : (_e+1)*M] *= (1.0 - fraction_to_move)
        dist[(_e+1)*M : (_e+2)*M] += fraction_to_move * employed_mass
...
def propagate_experiment_tm(agent, baseline_ergodic, EconomyMrkv_init,
    ...
    J_micro = _ep.j_full(agent.num_base_MrkvStates)   # one macro block (2J under the phase)
    Splurge = agent.Splurge
    ...
    if base_aPol is not None:
        ...
        if _uses_spike:
            ...
            spike_frac = _osr.spike_fraction(Un, Ur)
            if spike_frac > 0.0:
                # BUG-122 Step 0 (flag HAFISCAL_ONSET_SPIKE_T0_EXEMPT; catalog default ON in
                # both worlds since 2026-09-06, so a reference arm must pin 0): under the
                # exemption the spiked share STAYS in its first benefit state for this recorded
                # period instead of being advanced out of it, so the state label -- which under
                # `calendar` IS the count of benefit quarters drawn -- records what it drew.
                # onset_spike_rule is the SST for both engines' spike; flag OFF is byte-identical.
                micro_trans_init[0, :] = _osr.period0_employed_row(
                    agent.CondMrkvArrays[initial_macro], spike_frac)
        # the spike blend is done on the UNWRAPPED J x J chain (rows 0 and 1); wrap afterwards
        micro_trans_init = _ep.wrap_chain(micro_trans_init)
        ...
        dist = _apply_micro_transition(dist, _ep.wrap_chain(agent.CondMrkvArrays[initial_macro]),
    ...
    u_ergodic = 1.0 - _emp_mass(baseline_ergodic, M_grid, J_micro)
    # SST: same g as MC/TM per-state PermGroFac (effective_pLvl_growth; per-state under the earnings phase)
    g_base_pLvl = _agg_pLvl_growth(agent, u_ergodic, _state_fracs_of(baseline_ergodic, M_grid, J_micro))
    ...
    for t in range(act_T):
        ...
        if neutral_measure:
            ...
            u_rec_t = 1.0 - (_std_state_fracs[0] if not _ep.enabled()
                             else sum(_std_state_fracs[j] for j in range(J_micro) if _is_employed_micro(j, J_micro)))
            _fracs_t = _std_state_fracs
        else:
            u_rec_t = 1.0 - _emp_mass(dist, M_grid, J_micro)
            _fracs_t = _state_fracs_of(dist, M_grid, J_micro)
        ...
        if is_check_period:
            ...
            g_rec_t = _agg_pLvl_growth(agent, u_rec_t, _fracs_t)
            ...
            if neutral_measure and _std_state_fracs is not None:
                _std_state_fracs = _propagate_state_fracs(
                    _std_state_fracs, _ep.wrap_chain(agent.CondMrkvArrays[macro_next]),
                    np.asarray(agent.LivPrb[0][:J_micro], dtype=np.float64),
                    getattr(agent, 'T_age', None))
            continue
...
def compute_baseline_tm_data(economy, dist_aGrid_count=None, mc_E_pLvl=None,
    ...
    # BUG-093: ONE Q-construction (base level, experiment start, propagation kernel); see
    # _resolve_q_method / _baseline_ergodic_a. HAFISCAL_TM_Q_METHOD overrides the argument.
    q_method = _resolve_q_method(q_method, verbose=verbose)
    assert_mortality_inclusive_ergodicity(economy.agents)
    baseline_data = []
    for i, agent in enumerate(economy.agents):
        use_a = getattr(agent, 'tm_a_indexed', False)
        interp = getattr(agent, 'interpretation', 'CDC')
        q_method_i, doob_inj = q_method, None
        if use_a:
            tm_base, ergodic, q_method_i, doob_inj = _baseline_ergodic_a(
                agent, dist_aGrid_count, neutral_measure, q_method, interp,
                label=f"agent {i}: ")
            dist_aGrid = tm_base['dist_aGrid']
            A = len(dist_aGrid)
            u_ergodic_i = 1.0 - _emp_mass(ergodic, A, tm_base['TranMatrix'].shape[0] // A)
            ...
            baseline_data.append({
                ...
                'tm_a_indexed': True,
                # BUG-093: the construction this start was built by, the kernel data the base
                # level reuses (run_experiment_tm) and the Doob step's exact injection weight
                'q_method': q_method_i,
                'tm_data': tm_base,
                'doob_inj': doob_inj,
            })
            ...
            J_micro = _ep.j_full(agent.num_base_MrkvStates)
            M = len(dist_mGrid)
            u_ergodic_i = 1.0 - _emp_mass(ergodic, M, J_micro)
    ...
    for i, agent in enumerate(agents):
        ...
        if use_a:
            result_i = propagate_experiment_tm_a(
                ...
                q_kernel=bd.get('q_method'), doob_inj=bd.get('doob_inj'),
...
def _MacroCFunc_to_MicroCFunc(MacroCFunc, num_base_MrkvStates):
    ...
    """
    dim = len(MacroCFunc)
    J = _ep.j_full(num_base_MrkvStates)   # every micro state of a block (2J under the phase) shares its rule
...
def run_ad_tm(economy, shock_type, EconomyMrkv_init,
    ...
    J = _ep.j_full(economy.num_base_MrkvStates)   # belief-block stride: 2J micro states per macro block under the phase
    ...
    if not skip_training:
        ...
        for iteration in range(num_max_iterations):
            ...
            MicroCFunc = _MacroCFunc_to_MicroCFunc(MacroCFunc, economy.num_base_MrkvStates)
...
def _build_period_tm_a(dist_aGrid, Splurge, cFuncs, IncShkDstn_list,
    ...
    for j in range(J):
        ...
        for jp in range(J):
            ...
            emp_fac = employed_tran_shk_scale if _is_employed_micro(jp, J) else 1.0   # both phases' employed states
...
def build_tm_agg_fiscal_a(agent, aCount=200, aMin=0.0, dist_aGrid_max=None, aFac=3,
    ...
    dist_aGrid_max : float or None, default None
        ...
        DOCTRINE (2026-08-22, the S1 moments-grid bug class): the dist top
        is an EXPLICIT input by design — never derive it from
        ``max(aXtraGrid)`` or any solve-grid property. Solve-side tail knots
        (aXtraExtra) extend the solve top without changing the wealth
        support; a dist rule keyed to the solve top silently rescales
        coverage when knots appear (the e3df0bae catch: estimation
        valley-scatter ~7x). Sweep 2026-08-22: no other extension-anchor
        derivation exists in Code/HA-Models.
...
def _legacy_taxcut_mimic_dstn(raw, factor):
    """HAFISCAL_LEGACY_TAXCUT_ATOM=1 (2026-08-27): the paper's SIMULATED tax-cut process for the
    employed state -- the base PermShk atoms (undo the typo'd atom the SOLVE used) with the real
    transitory cut applied to every TranShk atom. Used by the a-indexed kernel builder AND the
    aggregator so the evolution and the outlay accounting see the same process (the published
    Monte Carlo drew base shock histories and applied tax_cut_multiplier on employed TranShk)."""
    d = deepcopy(raw)
    _p = np.asarray(d.atoms[0], dtype=np.float64).copy(); _p[1] = _p[1] / factor
    d.atoms = (_p, np.asarray(d.atoms[1], dtype=np.float64) * factor)
    return d


def _legacy_taxcut_mimic_on():
    return _os.environ.get('HAFISCAL_LEGACY_TAXCUT_ATOM', '').strip().lower() in ('1', 'on', 'true', 'yes')
...
def build_experiment_period_tm_a(agent, macro_curr, dist_aGrid, Cratio=1.0,
    ...
    IncShkDstn_list = [agent.IncShkDstn[0][src_offset + jp]
                       for jp in range(J_micro)]
    if employed_tran_shk_scale != 1.0 and _legacy_taxcut_mimic_on():
        # the paper's experiment: kernel from the simulated process; the scale is folded in here
        for _jp in range(J_micro):
            if _is_employed_micro(_jp, J_micro):
                IncShkDstn_list[_jp] = _legacy_taxcut_mimic_dstn(IncShkDstn_list[_jp], employed_tran_shk_scale)
        employed_tran_shk_scale = 1.0

    # the per-period conditional chain over the whole block ([[(1-h)M, hM],[0, M]] under the phase);
    # an override (the period-0 spike blend) arrives already wrapped
    micro_trans = (_ep.wrap_chain(agent.CondMrkvArrays[macro_curr])
    ...
    if q_kernel == 'doob':
        # BUG-093: the p-weighted survival kernel T_S,p (raw shock lists — its G*psi weighting IS
        # the Harmenberg reweighting) + the newborn injection applied at step time (_q_step).
        T_surv_p = _build_p_weighted_survival_kernel_a(
            dist_aGrid, float(agent.Splurge), cFuncs, IncShkDstn_list,
            micro_trans, Rfree_arr, PermGroFac_arr, LivPrb_arr, Cratio=Cratio,
            ad_tran_shk_scale=ad_tran_shk_scale,
            employed_tran_shk_scale=employed_tran_shk_scale,
            TranShk_addition=TranShk_addition,
            interpretation=interpretation)
        return _DoobStep(T_surv_p, NewBornDist), IncShkDstn_list, cFuncs
...
def propagate_experiment_tm_a(agent, baseline_ergodic, EconomyMrkv_init,
    ...
    if _uses_spike:
        ...
        spike_frac = _osr.spike_fraction(Un, Ur)
        if spike_frac > 0.0:
            # BUG-122 Step 0 (flag HAFISCAL_ONSET_SPIKE_T0_EXEMPT; catalog default ON in both
            # worlds since 2026-09-06, so a reference arm must pin 0): see the twin
            # site in propagate_experiment_tm above and onset_spike_rule, the SST for the spike.
            micro_trans_init[0, :] = _osr.period0_employed_row(
                agent.CondMrkvArrays[initial_macro], spike_frac)
    # the spike blend is done on the UNWRAPPED J x J chain (rows 0 and 1) and wrapped afterwards, so both
    # phases' employed states are laid off at the same rate (identity when the phase is off)
    micro_trans_init = _ep.wrap_chain(micro_trans_init)
    ...
    u_ergodic = 1.0 - _emp_mass(baseline_ergodic, A_grid, J_micro)
    g_base_pLvl = _agg_pLvl_growth(agent, u_ergodic, _state_fracs_of(baseline_ergodic, A_grid, J_micro))
    _LivPrb_eff = _effective_LivPrb(
        np.asarray(agent.LivPrb[0][:J_micro], dtype=np.float64),
        _prop_cap_T_age(getattr(agent, 'T_age', None)))   # BUG-092 knob (unset = agent's T_age)
    delta_eff = 1.0 - np.mean(_LivPrb_eff)
    pLvl_factor = 1.0
    # BUG-093: which step advances the Q-distribution. 'doob' = the p-weighted survival step
    # whose fixed point is the Doob start (baseline_tm_data['q_method']); 'bst'/'cohort' = the
    # plain kernel. doob_inj = delta_bar/p_bar from the base construction (exact); the closed
    # form 1-(1-delta)g_base is the same quantity up to the shock-weighting of the base kernel.
    if q_kernel is None:
        q_kernel = _resolve_q_method(None, verbose=False)
    # The Doob construction is a Q-measure (p-weighted) object: under the P measure
    # (neutral_measure=False) the start is the plain P-ergodic and the step the P-kernel.
    _doob_step = (q_kernel == 'doob') and bool(neutral_measure)
    if _doob_step and doob_inj is None:
        doob_inj = 1.0 - (1.0 - delta_eff) * g_base_pLvl
    _q_kernel_arg = 'doob' if _doob_step else 'plain'
    ...
    for t in range(act_T):
        ...
        # Resolve micro_trans for this period: spike blend only at t=0 (the whole block, wrapped under the phase).
        micro_trans_t = (micro_trans_init if t == 0
                         else _ep.wrap_chain(agent.CondMrkvArrays[macro_t]))
        ...
        IncShkDstn_raw = [agent.IncShkDstn[0][src_offset + j]
                          for j in range(J_micro)]
        if emp_tc != 1.0 and _legacy_taxcut_mimic_on():
            # the paper's experiment: the aggregator (outlay, splurge) sees the simulated process too
            IncShkDstn_raw[0] = _legacy_taxcut_mimic_dstn(IncShkDstn_raw[0], emp_tc)
        ...
        if is_check_period:
            ...
            for bucket in check_info['buckets']:
                ...
                dist_next_b = _q_step(TM_b, dist, doob_inj / pLvl_factor if _doob_step else None)
                if sp.issparse(dist_next_b):
                    dist_next_b = np.asarray(dist_next_b).flatten()
                dist_next_agg += w_b * dist_next_b
            ...
            u_rec_t = 1.0 - (state_fracs[0] if not _ep.enabled()
                             else sum(state_fracs[j] for j in range(J_micro) if _is_employed_micro(j, J_micro)))
            g_rec_t = _agg_pLvl_growth(agent, u_rec_t, state_fracs)
            pLvl_factor = ((1.0 - delta_eff) * g_rec_t * pLvl_factor
                           + (1.0 - (1.0 - delta_eff) * g_base_pLvl))
            if neutral_measure and _std_state_fracs is not None:
                _std_state_fracs = _propagate_state_fracs(
        ...
        if bucket_carry is not None:
            ...
            for b_idx in range(len(bucket_carry['dists'])):
                ...
                dist_b_next = _q_step(TM_t, dist_b, doob_inj / pLvl_factor if _doob_step else None)
                if sp.issparse(dist_b_next):
                    dist_b_next = np.asarray(dist_b_next).flatten()
                dist_b_next = np.maximum(dist_b_next, 0.0)
                s_b = np.sum(dist_b_next)
            ...
            u_rec_t = 1.0 - _emp_mass(dist, A_grid, J_micro)
            g_rec_t = _agg_pLvl_growth(agent, u_rec_t, _state_fracs_of(dist, A_grid, J_micro))
            pLvl_factor = ((1.0 - delta_eff) * g_rec_t * pLvl_factor
                           + (1.0 - (1.0 - delta_eff) * g_base_pLvl))
            if neutral_measure and _std_state_fracs is not None:
                _std_state_fracs = _propagate_state_fracs(
        ...
        dist = _q_step(TM_t, dist, doob_inj / pLvl_factor if _doob_step else None)
        if sp.issparse(dist):
            dist = np.asarray(dist).flatten()
        dist = np.maximum(dist, 0.0)
        s = np.sum(dist)
...
def stratified_state_draw(flat_probs, J, A, N, unemp_share, rng):
    """Econ-1 (owner ruling 2026-08-28): draw N (j, a) bins from the joint ergodic over J micro states x A asset nodes with
    the UNEMPLOYED states (j >= 1) oversampled to a panel share `unemp_share`, returning the bins and the population weights
    w_i = pi_{j_i} / q_{j_i} (mean 1 by construction) that make every weighted statistic unbiased. Within a state the asset
    node is drawn from the conditional ergodic. Rationale: the UI welfare cell is carried by the ~4 % of the panel unemployed
    at the onset (u1 2.8 %, u2 0.9 %, ...), an effective N of a few hundred out of 10,000; oversampling them to 30 % cuts the
    UI cell's variance ~7x while the check/tax-cut cells' variance rises only ~1.4x (their carriers are employed).
    unemp_share <= 0 or None -> the plain multinomial draw (weights all 1)."""
    flat_probs = np.asarray(flat_probs, dtype=np.float64); flat_probs = flat_probs / flat_probs.sum()
    P = flat_probs.reshape(J, A); pi = P.sum(axis=1)
    if not unemp_share or unemp_share <= 0.0:
        bins = rng.choice(len(flat_probs), size=N, p=flat_probs)
        return bins, np.ones(N)
    # unemployed = the employment decode (j >= 1 of EACH phase: the matured-employed state J_base is employed)
    _unemp = np.array([not _is_employed_micro(j, J) for j in range(J)])
    pi_u = pi[_unemp].sum()
    if pi_u <= 0.0:
        bins = rng.choice(len(flat_probs), size=N, p=flat_probs)
        return bins, np.ones(N)
    q = np.empty(J)
    q[~_unemp] = (1.0 - unemp_share) * pi[~_unemp] / pi[~_unemp].sum()   # employed states keep their relative shares
    q[_unemp] = unemp_share * pi[_unemp] / pi_u                          # sampling shares by state
    counts = np.floor(q * N).astype(int); counts[0] += N - counts.sum()                    # exact N
    bins = np.empty(N, dtype=np.int64); w = np.empty(N, dtype=np.float64); pos = 0
    for j in range(J):
        n_j = int(counts[j])
        if n_j == 0:
            continue
        cond = P[j] / P[j].sum() if P[j].sum() > 0 else np.full(A, 1.0 / A)
        a_idx = rng.choice(A, size=n_j, p=cond)
        bins[pos:pos + n_j] = j * A + a_idx
        w[pos:pos + n_j] = pi[j] / (n_j / N)
        pos += n_j
    perm = rng.permutation(N)
    bins, w = bins[perm], w[perm]
    return bins, w / w.mean()
...
def initialize_mc_from_tm_ergodic(agents, baseline_tm_data, mc_warmup=24,
    ...
    def _weighted_tail_K():
        v = os.environ.get('HAFISCAL_MC_WEIGHTED_TAIL', '').strip()
        return int(v) if v else 0
    if _weighted_tail_K() > 0 and mc_plvl_init != 'analytic_markov':
        raise ValueError(
            f"HAFISCAL_MC_WEIGHTED_TAIL={_weighted_tail_K()} requires HAFISCAL_MC_PLVL_INIT="
            f"analytic_markov (got {mc_plvl_init!r}): the weighted-tail sampler is built on the "
            "unemployment-aware mixture only.")
    for i, agent in enumerate(agents):
        bd_i = baseline_tm_data[i]
        tm_a_init = bd_i.get('tm_a_indexed', False)
        if _weighted_tail_K() <= 0:
            agent.agent_weights = None       # hygiene: no stale weights from another seed
        _strat = 0.0; _w_state = None
        if tm_a_init:
            ...
            # Econ-1 (2026-08-28): HAFISCAL_MC_STRATIFY_UNEMP=<share> oversamples the unemployed micro states at the
            # onset with population weights (stratified_state_draw); unset/0 = the plain multinomial draw.
            _strat = os.environ.get('HAFISCAL_MC_STRATIFY_UNEMP', '').strip()
            _strat = float(_strat) if _strat else 0.0
            if _strat and not (0.0 < _strat < 1.0):
                raise ValueError(f'HAFISCAL_MC_STRATIFY_UNEMP={_strat!r}: expected a panel share in (0, 1)')
            J_i_s = len(flat_probs) // A_i
            bins, _w_state = stratified_state_draw(flat_probs, J_i_s, A_i, N_i, _strat, rng_i)
            agent_j = bins // A_i
            agent_aNrm = grid[bins % A_i]
            if _strat:
                _js, _cs = np.unique(agent_j, return_counts=True)
                print(f"[stratify-state] agent {i}: N={N_i} unemployed share drawn={float(np.sum(_cs[_js >= 1]))/N_i:.3f} "
                      f"(ergodic {float(np.sum(flat_probs.reshape(J_i_s, A_i)[1:])):.4f}); weights min={_w_state.min():.3f} max={_w_state.max():.3f}", flush=True)
        else:
            ...
            M_i = len(grid)
            J_i = len(erg) // M_i   # the ergodic's own micro count (2J under the earnings phase)
            rng_i = np.random.RandomState(agent.seed)
            N_i = agent.AgentCount
        ...
        elif mc_plvl_init == 'analytic_markov' and _weighted_tail_K() > 0:
            # BUG-092 weighted-tail sampler (opt-in, HAFISCAL_MC_WEIGHTED_TAIL=K): the same
            # unemployment-aware mixture, but the bottom (1-q) of the population as N-K
            # equal-population strata at their conditional-mean p and the top q as K
            # equal-p-MASS strata, each one household with the stratum's conditional-mean p
            # and its (tiny) population weight -> the panel's weighted E[p] is the analytical
            # E[p] exactly. Weights live on agent.agent_weights (mean 1) and flow through
            # run_experiment / mill_rule aggregation and the welfare battery's sums.
            import sys as _sys_wt
            _ha_wt = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            if _ha_wt not in _sys_wt.path:
                _sys_wt.path.insert(0, _ha_wt)
            from weighted_tail_sampler import (weighted_tail_pLvl_for_agent as _wt_draw,
                                               tail_pop_share as _wt_q)
            _K_i = int(min(_weighted_tail_K(), max(2, N_i // 4)))
            agent_pLvl, _wts_i, _wdiag = _wt_draw(agent, N_i, rng_i, K=_K_i, q=_wt_q())
            agent.agent_weights = np.asarray(_wts_i, dtype=np.float64)
            print(f"[weighted-tail] agent {i}: N={N_i} K={_K_i} q={_wdiag['q']} "
                  f"E[p] analytical={_wdiag['E_p_analytical']:.4f} weighted={_wdiag['E_p_weighted']:.4f} "
                  f"tail income share={_wdiag['tail_income_share']:.1%} max household share="
                  f"{_wdiag['max_income_share']:.2%} min weight={_wdiag['min_weight']:.1e}", flush=True)
        ...
        # Econ-1: compose the stratified-state weights with any weighted-tail weights (independent devices: the
        # tail weights concern p, drawn independently of the state; the state weights concern j) -> product, mean 1.
        if tm_a_init and _strat:
            _wt_prev = getattr(agent, 'agent_weights', None)
            _w_all = _w_state * (np.asarray(_wt_prev, dtype=np.float64) if _wt_prev is not None else 1.0)
            agent.agent_weights = _w_all / _w_all.mean()
