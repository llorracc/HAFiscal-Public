...
2 │ Phase 1 — Baseline AggCons    │ Both → E_P[p·c(m,z)].   │ TMMC §3,
  ...
  │                               │                         │ §10, §14
──┼───────────────────────────────┼─────────────────────────┼──────────────
3 │ Phase 1 — TM ladder (opt-in:  │ TM[dist_aGrid_count] → E_P[p·c]   │ TMMC §10.1,
...
#
# Why the key omits AgentCount, seed, and dist_aGrid_count: the Bellman is
...
TM_CONFIGS = {
    "TM-coarse":  {"dist_aGrid_count": 30},
    "TM-default": {"dist_aGrid_count": 50},
    "TM-50":      {"dist_aGrid_count": 50},   # canonical paired-cell coarse grid
    "TM-medium":  {"dist_aGrid_count": 75},
    "TM-fine":    {"dist_aGrid_count": 100},
    "TM-100":     {"dist_aGrid_count": 100},  # canonical paired-cell fine grid
    "TM-xfine":   {"dist_aGrid_count": 150},
}

# Paired-cell ladder presets (Plan v2). Each preset names exactly two
# cells; Cell 1 always uses TM dist_aGrid_count=50, Cell 2 always uses dist_aGrid_count=100.
...
def phase0_harness_checks(mc_configs=None, tm_configs=None):
    ...
    bl = compute_baseline_tm_data(economy, dist_aGrid_count=40, neutral_measure=True, verbose=False)
    assert_tm_baseline_indexing(economy, bl)

    intended = int(economy.act_T)
    tm_init = compute_baseline_tm_data(economy, dist_aGrid_count=40, neutral_measure=True, verbose=False)
...
def phase1_baseline_ergodic(mc_configs=None, tm_configs=None):
    ...
    tm_ref_dist_aGrid_count = TM_CONFIGS[tm_ref_label]["dist_aGrid_count"]
    print(f"\nComputing TM reference ({tm_ref_label}, dist_aGrid_count={tm_ref_dist_aGrid_count})...")
    t0 = time.time()
    ...
    with profile_block("phase1.compute_baseline_tm_data(Q)"):
        tm_data_ref = compute_baseline_tm_data(
            economy_tm, dist_aGrid_count=tm_ref_dist_aGrid_count, neutral_measure=True, verbose=False,
        )
    ...
    with profile_block("phase1.compute_baseline_tm_data(P)"):
        tm_data_P_ref = compute_baseline_tm_data(
            economy_tm, dist_aGrid_count=tm_ref_dist_aGrid_count, neutral_measure=False, verbose=False,
        )
    ...
    ref_base_tm = run_experiment_tm(
        economy_tm,
        shock_type="base",
        dist_aGrid_count=tm_ref_dist_aGrid_count,
    ...
    for tm_label in tm_configs:
        dist_aGrid_count = TM_CONFIGS[tm_label]["dist_aGrid_count"]
        t0 = time.time()
        base_r = run_experiment_tm(economy_tm, shock_type="base",
                                    dist_aGrid_count=dist_aGrid_count, neutral_measure=True)
        ...
        print(f"  {tm_label} (dist_aGrid_count={dist_aGrid_count}): AggCons_pc={AggCons_pc:.6f}, err={rel_err(AggCons_pc, ref_AggCons_pc_tm):.4f}%, {elapsed:.1f}s")
    ...
    for mc_label in mc_configs:
        ...
        for seed in seeds:
            ...
            tm_init_data = compute_baseline_tm_data(economy_mc, dist_aGrid_count=50, neutral_measure=True)
    ...
    if per_type_rows:
        ...
        print(
            f"\n  Per-type moments: TM-P ergodic ({tm_ref_label}, dist_aGrid_count={tm_ref_dist_aGrid_count}) vs "
    ...
    return {
        ...
        "per_type_tm_dist_aGrid_count": tm_ref_dist_aGrid_count,
    }
...
def _run_norec_experiment(shock_type, mc_configs, tm_configs):
    ...
    t0 = time.time()
    economy_tm, meta = setup_economy()
    baseline_tm = compute_baseline_tm_data(economy_tm, dist_aGrid_count=tm_ref_dist_aGrid_count, neutral_measure=True)
    base_tm_results = run_experiment_tm(economy_tm, shock_type="base",
                                        dist_aGrid_count=tm_ref_dist_aGrid_count, neutral_measure=True)
    ...
    ref_results = run_experiment_tm_nonbase(
        ...
        baseline_tm, mCount=tm_ref_dist_aGrid_count, neutral_measure=True,
    )
    t_ref = time.time() - t0
    ...
    for tm_label in tm_configs[:-1]:
        dist_aGrid_count = TM_CONFIGS[tm_label]["dist_aGrid_count"]
        t0 = time.time()
        bl_tm = compute_baseline_tm_data(economy_tm, dist_aGrid_count=dist_aGrid_count, neutral_measure=True)
        base_r = run_experiment_tm(economy_tm, shock_type="base", dist_aGrid_count=dist_aGrid_count, neutral_measure=True)
    ...
    for mc_label in mc_configs:
        ...
        for seed in seeds:
            ...
            with profile_block(f"_run_norec.compute_baseline_tm_data(mc_init,seed={seed})"):
                tm_init = compute_baseline_tm_data(economy_mc, dist_aGrid_count=50, neutral_measure=True)
...
def phase7_ad_loop(mc_configs=None, tm_configs=None, ad_policies=None):
    ...
    print(f"\nComputing TM AD reference ({tm_ref_label}, dist_aGrid_count={tm_ref_dist_aGrid_count})...")
    t0 = time.time()
    economy_tm, meta = setup_economy()
    Rfree = economy_tm.agents[0].Rfree[0]
    act_T = economy_tm.act_T
    ...
    baseline_tm = compute_baseline_tm_data(economy_tm, dist_aGrid_count=tm_ref_dist_aGrid_count,
                                           neutral_measure=True)
    base_tm_results = run_experiment_tm(economy_tm, shock_type="base",
                                        dist_aGrid_count=tm_ref_dist_aGrid_count, neutral_measure=True)
    ...
    def _tm_ad_for_shock(shock_type):
        ...
        for t in range(max_rec):
            ...
            r_t = run_ad_tm(
                ...
                mCount=tm_ref_dist_aGrid_count,
                neutral_measure=True,
                verbose=False,
                skip_training=(t > 0),
            )
    ...
    def _tm_noAD_for_shock(shock_type):
        ...
        for t in range(max_rec):
            r_t = run_experiment_tm_nonbase(
                ...
                baseline_tm, mCount=tm_ref_dist_aGrid_count,
...
def _compute_recession_baseline_tm(economy, baseline_tm, meta, dist_aGrid_count):
    ...
    for t in range(max_rec):
        ...
        r = run_experiment_tm_nonbase(eco, "recession", mk,
                                      baseline_tm, mCount=dist_aGrid_count, neutral_measure=True)
...
def main():
    ...
    parser.add_argument(
        ...
        help=("Convergence-aware preset: runs exactly two paired cells "
              "(Cell 1 = smaller MC + TM dist_aGrid_count=50; Cell 2 = larger MC + "
              "TM dist_aGrid_count=100). Cannot be combined with --mc or --tm. "
    ...
    if args.ladder is not None:
        ...
        print(f"[ladder] convergence claim: cell 1 -> cell 2 simultaneously "
              f"increases N (MC) and dist_aGrid_count (TM), so any drift in MC<->TM "
...
def _print_ladder_summary(ladder_name, invocations, all_results, canon, reg):
    ...
    print(f"# Convergence claim: as N (MC) and dist_aGrid_count (TM) both grow")
