...
bl_for_init = compute_baseline_tm_data(AggEco, dist_aGrid_count=50, verbose=False)
...
def run_tm_recession(eco_template, dist_aGrid_count, neutral_measure=False):
    eco_tm = deepcopy(eco_template)
    TM_N = sum(a.AgentCount for a in eco_tm.agents)
    bl_data = compute_baseline_tm_data(eco_tm, dist_aGrid_count=dist_aGrid_count, verbose=False,
                                        neutral_measure=neutral_measure)
    tm_base = run_experiment_tm(eco_tm, shock_type='base', dist_aGrid_count=dist_aGrid_count,
...
t0 = time()
tm_std_npv, tm_std_te, _ = run_tm_recession(AggEco, dist_aGrid_count=dist_aGrid_count, neutral_measure=False)
print(f"  NPV = {tm_std_npv:.4f}  ({time()-t0:.0f}s)")

print("\n[2/3] Neutral-measure TM (neutral_measure=True)...")
t0 = time()
tm_nm_npv, tm_nm_te, _ = run_tm_recession(AggEco, dist_aGrid_count=dist_aGrid_count, neutral_measure=True)
