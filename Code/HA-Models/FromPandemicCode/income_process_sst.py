...
# Earnings-phase SST (Code/HA-Models/earnings_phase.py; owner ruling 2026-08-28): the micro block of a macro
# state is J_full = n_phases * J -- {growing, matured} x the J employment/UI states -- with the layout
# Mrkv = J_full*macro + J*phase + emp. Every helper below that sizes a MACRO block uses J_full; the
# employment decode `i % J` stays valid because the phase blocks are J-aligned. With the phase OFF
# (the default until the gates pass) J_full == J and every expression is byte-for-byte the old one.
_ha_root = _os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))
if _ha_root not in _sys.path:
    _sys.path.insert(0, _ha_root)
...
def tile_PermGroFac_composite(PermGroFac_base, PermGroFac_unemp, num_mrkv_states, num_base_MrkvStates):
    ...
    Earnings phase ON: a state in the MATURED phase (phase_of(j) == 1) grows at
    earnings_phase.MATURED_GROWTH (= 1) whatever its employment state -- the plateau of the
    age-earnings profile; the growing phase keeps the rule above. The old expression is kept
    verbatim for the OFF case (byte-identity)."""
    G_emp = float(np.asarray(PermGroFac_base).ravel()[0])
    G_u = float(PermGroFac_unemp)
    nb = int(num_base_MrkvStates)
    n = int(num_mrkv_states)
    if _ep.enabled():
        return _ep.tile_permgrofac_composite(PermGroFac_base, PermGroFac_unemp, n, nb)
...
def effective_pLvl_growth(agent, unemployment_rate=None):
    ...
    A SCALAR growth rate cannot represent the earnings phase (the matured share of the
    population -- and of its income -- grows at 1, not G_emp), so with the phase ON this
    raises: consumers must use the per-state form (tm_methods._agg_pLvl_growth, the
    Markov p-recursions) instead of silently overstating the growth of E[p]."""
    if _ep.enabled():
        raise RuntimeError(
            "effective_pLvl_growth: a scalar g cannot represent the earnings phase "
            f"({_ep.ENV}={_os.environ.get(_ep.ENV)!r}); use the per-state growth "
            "(tm_methods._agg_pLvl_growth / the Markov p-recursions).")
...
def taxcut_block_range(TaxCutPeriods, num_base_MrkvStates):
    """Combined-Markov index range [lo, hi) of the states whose employed income carries
    the payroll tax cut. Each experiment period t owns TWO macro blocks (2t = normal
    at t, 2t+1 = recession at t; blocks 0,1 = the pre-experiment pair), so periods
    1..TaxCutPeriods are macro blocks 2 .. 2*TaxCutPeriods+1 — combined indices
    [2*nb, (2*TaxCutPeriods+2)*nb): blocks 2..17 = 8 quarters at TaxCutPeriods=8.
    SST since 2026-08-24 (BUG-091): the multiplier entry hard-coded 18*nb and the
    simulation's delivery 9*2*nb (both = 8 quarters), while the welfare drivers looped
    over ALL 2*(num_experiment_periods+1) blocks — a 10-quarter cut in the solve
    against an 8-quarter cut in the simulation.
    Earnings phase: a macro block holds J_full = n_phases*nb micro states (growing then
    matured), so the block range is sized by J_full -- the argument stays the EMPLOYMENT
    count, as at every call site (identical when the phase is off)."""
    nb = _ep.j_full(int(num_base_MrkvStates))
    return 2 * nb, (2 * int(TaxCutPeriods) + 2) * nb


def build_recession_taxcut_inc_shk_dstn(IncShkDstn_recession, TaxCutStatesIncShkDstn,
                                        TaxCutPeriods, num_base_MrkvStates):
    """The recession-tax-cut income process: the recession process with the tax-cut
    per-micro-state distributions substituted in the tax-cut blocks (see
    taxcut_block_range). One deep copy; the recession list is not mutated.
    Earnings phase: TaxCutStatesIncShkDstn is per EMPLOYMENT state (nb long); both phases
    of a macro block get the same employed/unemployed entries, and `i % nb` IS the
    employment decode because the phase blocks are nb-aligned (Mrkv = 2nb*macro + nb*phase + emp)."""
    from copy import deepcopy
    out = deepcopy(IncShkDstn_recession)
    lo, hi = taxcut_block_range(TaxCutPeriods, num_base_MrkvStates)
    nb = int(num_base_MrkvStates)
    for i in range(lo, hi):
        out[0][i] = TaxCutStatesIncShkDstn[i % nb]
    return out
