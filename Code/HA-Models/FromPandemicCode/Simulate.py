def Simulate(Run_Dict,figs_dir,Parametrization='Baseline'):
    ...
    # --- Opt-in cohort-parallel economy solve (R8 item 8 scout) -----------
    # HAFISCAL_STEP5A_PARALLEL_SOLVE (unset/0 = stock eco.solve(), byte-
    # identical default). Wrapper, worker budgeting, and probe support live in
    # Code/HA-Models/step5a_parallel_solve.py (that dir was put on sys.path
    # above). _step5a_entry_pid distinguishes the pre-fork parent (site
    # 'initial') from the forked shock-type workers so 'auto' budgets compose
    # with the outer fork instead of multiplying it.
    _step5a_entry_pid = os.getpid()
    try:
        import step5a_parallel_solve as _s5aps
    except ImportError:
        _s5aps = None

    def _eco_solve(eco, label):
        if _s5aps is not None:
            _s5aps.step5a_eco_solve(eco, entry_pid=_step5a_entry_pid,
                                    label=label)
        else:
            eco.solve()
    # ----------------------------------------------------------------------
    ...
    for ThisType in BaseTypeList:
        ...
        # Earnings phase (2026-08-28): build the J-long EMPLOYMENT block, then wrap it to [growing J | matured J]
        # (the same distribution objects in both phases -- the phase changes only the deterministic growth);
        # the recession list below TILES this wrapped block per macro state, which is exactly the middle layout
        # Mrkv = 2J*macro + J*phase + emp. Identity when the phase is off.
        ThisType.IncShkDstn = [_ep.wrap_list([emp_src] + [IncShkDstn_unemp]*UBspell_normal + [IncShkDstn_unemp_nobenefits]*(num_base_MrkvStates - 2 - UBspell_normal) + [IncShkDstn_unemp_nobenefits])]
        ...
        # BUG-050 FIXED 2026-07-26 (was: plain copy of IncShkDstn_recession).
        #
        # Under LEGACY (4-state) encoding the plain copy is CORRECT: the UI extension is
        # delivered via a different MrkvArray_recessionUI (the freeze-window keeps agents in
        # benefit-paying states longer), so recessionUI != recession through the Markov
        # dynamics. That branch is unchanged and stays byte-identical.
        #
        # Under the income-delivered encodings (calendar since 2026-08-26; bug_fix before) the
        # extension must instead be delivered as INCOME:
        # the U3/U4 extension micro-states pay WITH-benefits income (IncShkDstn_unemp) during
        # the recession, where plain recession has them paying no-benefits. The old plain copy
        # left them on no-benefits, so recessionUI was IDENTICAL to recession — both
        # dNPV_AggCons and dNPV_AggIncome were exactly 0, giving a 0/0 = nan multiplier. That
        # is a MISSING IMPLEMENTATION, not a by-construction 0/0 (contrast ui_norec, which is
        # genuinely 0/0 because UI never activates outside a recession).
        #
        # The construction below MIRRORS the already-validated welfare path
        # (welfare6_scenario.py:431-447, validated under BUG-043) so the multiplier and welfare
        # paths agree by construction rather than by coincidence. This is the same class of gap
        # as BUG-048 (recessionTaxCut income wiring), fixed the same way.
        #
        # Default ON: the value it replaces is nan, so nothing usable can regress
        # ([[feedback_error_vs_sample_changes]] — errors are fixed as the default with a
        # revert toggle). HAFISCAL_UI_INCOME_WIRING=off restores the old nan behavior.
        _ui_wiring = os.environ.get('HAFISCAL_UI_INCOME_WIRING', 'on').strip().lower()
        if _ui_wiring not in ('on', 'off'):
            raise ValueError(
                f"HAFISCAL_UI_INCOME_WIRING must be 'on' or 'off', got {_ui_wiring!r}")
        _n_extension = num_base_MrkvStates - 2 - UBspell_normal   # Policy_ExtraBenefitQuarters
        if _ui_wiring == 'on' and _n_extension > 0:
            # Which extension states pay, at which macro states: the rule SST
            # (Code/HA-Models/ui_extension_rule.py; calendar window under 'calendar',
            # recession-conditional under 'bug_fix'). The SAME call the welfare battery
            # (welfare6_scenario.py) and the MC override (AggFiscalModel.py) make.
            from EstimParameters import (HAFISCAL_UI_STATE_ENCODING as _ui_enc,
                                         UI_EXT_WINDOW as _ui_win)
            ...
            extension_micro_indices = list(range(
                UBspell_normal + 1, UBspell_normal + 1 + _n_extension))
            for macro_idx in range(n_macro):
                for micro_idx in extension_micro_indices:
                    if _pays_extension(_ui_enc, macro_idx, micro_idx, UBspell_normal,
                                       _n_extension, _ui_win):
            ...
            ThisType.IncShkDstn_recessionUI = IncShkDstn_recessionUI
        else:
            # legacy encoding (_n_extension == 0, byte-identical) or explicit opt-out
        ...
        # HAFISCAL_LEGACY_TAXCUT_ATOM=1 (2026-08-27, owner protocol section 5e): rebuild the PUBLISHED
        # construction (HaFiscal-QE Simulate.py:87 -- the second PermShk atom x TaxCutIncFactor, TranShk
        # untouched) so the TRUE original model can be run as the start column of the revision report and
        # the BUG-023 fix measured forward. Reproduction only; default off = the fix.
        import os as _os_tc
        if _os_tc.environ.get('HAFISCAL_LEGACY_TAXCUT_ATOM', '').strip().lower() in ('1', 'on', 'true', 'yes'):
            _perm_legacy = np.asarray(EmployedIncShkDstn.atoms[0], dtype=np.float64).copy()
            _perm_legacy[1] = _perm_legacy[1] * ThisType.TaxCutIncFactor
            EmployedIncShkDstn.atoms = (_perm_legacy, np.asarray(EmployedIncShkDstn.atoms[1], dtype=np.float64))
            print("[taxcut-atoms] HAFISCAL_LEGACY_TAXCUT_ATOM=1: the published construction (one PermShk atom "
                  f"x {ThisType.TaxCutIncFactor}, TranShk untouched) -- BUG-023 PRESENT (reproduction arm)", flush=True)
        else:
        ...
        # Tax states are 2,3 (q1) 4,5 (q2) ... 16,17 (q8): blocks 2..TaxCutPeriods+1,
        # via the SST (BUG-091, 2026-08-24; byte-identical to the former hard-coded 18).
        from income_process_sst import build_recession_taxcut_inc_shk_dstn
        ThisType.IncShkDstn_recessionTaxCut = build_recession_taxcut_inc_shk_dstn(
            IncShkDstn_recession, TaxCutStatesIncShkDstn,
            ThisType.TaxCutPeriods, num_base_MrkvStates)
    ...
    with profile_function("initial_economy_solve", input_size=len(TypeList)):
        _eco_solve(AggDemandEconomy, 'initial')
    t1 = time()
    ...
    # HAFISCAL_MC_INIT (owner ruling 2026-08-29 14:40 -- the QE way under an age cap): 'auto' = the TM-ergodic init + short
    # warmup when the model is age-free (exact there), the QE burn-in (newborns, >= 2*T_age quarters of the base economy) when
    # T_age is set (exact there; the age-free ergodic start is NOT the capped model's steady state -- the 32-h timing run's
    # drift); 'tm' / 'burnin' force either. Run_Dict['mc_use_tm_init'] keeps its meaning under 'auto'.
    _mc_init_mode = os.environ.get('HAFISCAL_MC_INIT', 'auto').strip().lower()
    _T_age_caps = [getattr(a, 'T_age', None) for a in AggDemandEconomy.agents]
    _capped_any = any(t is not None for t in _T_age_caps)
    if _mc_init_mode == 'tm':
        _mc_tm_init = True
    elif _mc_init_mode == 'burnin':
        _mc_tm_init = False
    elif _mc_init_mode == 'auto':
        _mc_tm_init = bool(Run_Dict.get('mc_use_tm_init', True)) and not _capped_any
    else:
        raise ValueError(f"HAFISCAL_MC_INIT={_mc_init_mode!r}: expected auto | tm | burnin")
    if use_MC and _capped_any:
        _T_age_max = max(int(t) for t in _T_age_caps if t is not None)
        _burn_q = int(os.environ.get('HAFISCAL_MC_BURNIN_QUARTERS', '0') or 0) or 2 * _T_age_max   # diagnostic override
        if not _mc_tm_init and int(AggDemandEconomy.act_T) < _burn_q:
            progress(f"[mc-init] age cap T_age={_T_age_max}: burn-in raised from {AggDemandEconomy.act_T} to {_burn_q} quarters (the QE way)")
            AggDemandEconomy.act_T = _burn_q
            AggDemandEconomy.EconomyMrkvNow_hist = [0] * AggDemandEconomy.act_T   # allocated at construction from act_T
            for _a in AggDemandEconomy.agents:
                _a.T_sim = max(int(getattr(_a, 'T_sim', 0) or 0), AggDemandEconomy.act_T)
        progress(f"[mc-init] age cap T_age={_T_age_max}: " + ('TM-ergodic init FORCED (HAFISCAL_MC_INIT=tm) -- not the QE way' if _mc_tm_init
                 else f'QE-way burn-in of {AggDemandEconomy.act_T} quarters from newborns (HAFISCAL_MC_INIT={_mc_init_mode})'))
    need_tm_baseline = use_TM or (use_MC and _mc_tm_init)
    if need_tm_baseline:
    ...
    if use_MC:
        ...
        if baseline_tm_data is not None and _mc_tm_init:
            ...
            for i, agent in enumerate(AggDemandEconomy.agents):
                ...
                if tm_a_init:
                    ...
                    A_i = len(grid)
                    J_i = _ep.j_full(agent.num_base_MrkvStates)   # the ergodic's micro count (2J under the phase)
                ...
                L = agent.LivPrb[0][0]
                # HAFISCAL_T_AGE=none => geometric ages, tolerance-truncated
                # (effective_age_chain_length; imported from tm_methods above).
                T_ag = effective_age_chain_length(L, agent.T_age)
            ...
            if os.environ.get('HAFISCAL_SKIP_DRIFT_TEST', '0') != '1':
                try:
                    ...
                    _all_pass = True
                    # Lorenz drift is assessed at the EDUCATION GROUP, not per discount-factor
                    # atom (owner, 2026-09-06: "any Lorenz comparison must be to the relevant
                    # GROUP"). Step 2 fits a group's Lorenz curve by concatenating its atoms into
                    # one weighted array; most of the within-group wealth inequality this model
                    # produces comes FROM the spread across those atoms, so an atom's own Lorenz
                    # curve discards exactly the dimension the estimation targets and no threshold
                    # on it means anything. The per-atom numbers below are still printed -- they
                    # are useful for locating where a group-level gap comes from -- but they no
                    # longer gate. Collected here, assessed after the loop.
                    _grp = {}
                    for i, agent in enumerate(AggDemandEconomy.agents):
                        bd_i = baseline_tm_data[i]
                        ...
                        tma_moments = _drift_mod.compute_tma_analytical_moments(
                            ...
                            J=_ep.j_full(agent.num_base_MrkvStates),
                        ...
                        # per-atom: DIAGNOSTIC only since 2026-09-06 (see the note above)
                        _drift_mod.assess_and_report(
                            drift, label=f"agent_{i} [per-atom, diagnostic]", agent=agent,
                            hard_fail=False,
                        )
                        # accumulate this atom into its education group, both engines, on the
                        # population weight the aggregation uses everywhere else
                        try:
                            _e = getattr(agent, 'EducType', None)
                            _w = float(getattr(agent, 'pop_rescale_factor', 1.0))
                            _aL = np.asarray(agent.state_now['aLvl'], dtype=float).ravel()
                            _pL = np.asarray(agent.state_now['pLvl'], dtype=float).ravel()
                            _mc_a = _aL / np.where(_pL > 0, _pL, np.nan)
                            _mc_a = _mc_a[np.isfinite(_mc_a)]
                            _erg = np.asarray(bd_i['ergodic'], dtype=float)
                            _grid = np.asarray(bd_i['dist_aGrid'], dtype=float).ravel()
                            _erg = _erg.reshape(-1, _grid.size) if _erg.ndim == 1 else _erg
                            _tm_w = _erg.sum(axis=0)          # marginal over the Markov states
                            _g = _grp.setdefault(_e, {'mc': [], 'tm': []})
                            if _mc_a.size:
                                _g['mc'].append((_mc_a, np.full(_mc_a.shape, _w / _mc_a.size)))
                            _g['tm'].append((_grid, _tm_w * _w))
                        except Exception as _e_grp:
                            print(f"[drift] agent_{i}: could not pool for the group metric: {_e_grp}")
                        ...
                        if os.environ.get('HAFISCAL_DRIFT_DIAG_DIST', '0') == '1':
                            ...
                            erg_2d = _np.asarray(bd_i["ergodic"]).reshape(
                                _ep.j_full(agent.num_base_MrkvStates), len(bd_i["dist_aGrid"]))
                    ...
                    # --- the PRIMARY gate: Lorenz drift at the education group ---------------
                    _EDUC = {0: 'dropout', 1: 'high school', 2: 'college'}
                    _pp = float(os.environ.get('HAFISCAL_DRIFT_LORENZ_PP', '3.0'))
                    # REPORT-ONLY by default since 2026-09-06. The Lorenz band has no derivation:
                    # the MC panel is a finite sample DRAWN FROM the TM ergodic it was seeded with,
                    # so the only discrepancy possible is sampling error, and the acceptance
                    # criterion should be a sampling band -- as this module's own pLvl check already
                    # is (z*scale/sqrt(N), with a stated false-fail rate). A flat 3pp is too loose
                    # where the share is large and too tight where it is near zero, which is exactly
                    # how it behaved: it aborted the pipeline on one near-constraint cohort and
                    # passed everything else regardless.
                    #
                    # The GATING role belongs to toolmap/test_tm_ergodic_parity.py, which is in
                    # `make test-fast` since the same day (12 s): it compares this same production
                    # seeding path against the MC forward sim with a documented band. And that test
                    # is itself only a tripwire -- see the note in the Makefile for the validation
                    # that actually established these engines agree (the 2026-06-23 matched
                    # re-validation, beta to <= 0.06 % across cohorts, and the Phase A/B ledger).
                    #
                    # The numbers below are still PRINTED in full, because drift belongs in the
                    # headline of an MC run, not buried. HAFISCAL_DRIFT_LORENZ_HARD_FAIL=1 restores
                    # the abort for anyone deliberately hunting a divergence.
                    _hard = os.environ.get('HAFISCAL_DRIFT_LORENZ_HARD_FAIL', '0').strip() == '1'
                    _breaches = []
                    for _e in sorted(_grp, key=lambda x: (x is None, x)):
                        _g = _grp[_e]
                        _mc_l = _drift_mod.pooled_lorenz_shares(_g['mc'])
                        _tm_l = _drift_mod.pooled_lorenz_shares(_g['tm'])
                        if _mc_l is None or _tm_l is None:
                            print(f"[drift GROUP {_EDUC.get(_e, _e)}] not computable "
                                  f"({len(_g['mc'])} MC pieces, {len(_g['tm'])} TM pieces)")
                            continue
                        _d = np.asarray(_mc_l) - np.asarray(_tm_l)
                        _lbl = _EDUC.get(_e, f"educ {_e}")
                        print(f"[drift GROUP {_lbl}] pooled over {len(_g['tm'])} discount-factor atoms  "
                              f"[PRIMARY]")
                        print(f"[drift GROUP {_lbl}]   MC   Lorenz p20/40/60/80 = "
                              + " ".join(f"{x:7.3f}" for x in _mc_l))
                        print(f"[drift GROUP {_lbl}]   TM-a Lorenz p20/40/60/80 = "
                              + " ".join(f"{x:7.3f}" for x in _tm_l))
                        print(f"[drift GROUP {_lbl}]   drift (pp)               = "
                              + " ".join(f"{x:+7.3f}" for x in _d))
                        _bad = [(q, v) for q, v in zip((20, 40, 60, 80), _d) if abs(v) > _pp]
                        if _bad:
                            _breaches.append((_lbl, _bad))
                            print(f"[drift GROUP {_lbl}]   ✗ exceeds ±{_pp:g}pp at "
                                  + ", ".join(f"p{q} ({v:+.3f}pp)" for q, v in _bad))
                        else:
                            print(f"[drift GROUP {_lbl}]   ✓ within ±{_pp:g}pp")
                    if _breaches:
                        _all_pass = False
                        _msg = ("[drift] GROUP Lorenz drift exceeds threshold:\n  "
                                + "\n  ".join(f"{lbl}: " + ", ".join(f"p{q} {v:+.3f}pp" for q, v in bad)
                                             for lbl, bad in _breaches)
                                + f"\n  (band ±{_pp:g}pp, which has NO sampling derivation -- see the "
                                  "note above and toolmap/test_tm_ergodic_parity.py, which is the "
                                  "gate; HAFISCAL_DRIFT_LORENZ_PP sets the band, "
                                  "HAFISCAL_DRIFT_LORENZ_HARD_FAIL=1 makes it fatal)")
                        if _hard:
                            raise RuntimeError(_msg)
                        print("WARNING: " + _msg)
                    if _all_pass:
                        print('[drift] ✓ every education group within the Lorenz drift threshold')
    ...
    if Run_Baseline:   
        ...
        if use_MC:
            ...
            with profile_function("baseline_experiment_MC"):
                # ONE simulation, both views (2026-09-06). These were two consecutive
                # run_experiment calls on the SAME baseline: the second re-simulated an entire
                # baseline in order to repack arrays the process already held (~2 min at
                # production scale, on every MC run). `Full_Output='both'` returns the pair.
                base_results, base_results_full = AggDemandEconomy.run_experiment(
                    **base_dict_agg, Full_Output='both')
        ...
        if use_TM:
            ...
            with profile_function("baseline_experiment_TM"):
                base_results_tm = run_experiment_tm(
                    ...
                    # BUG-093: the base level is built from the SAME ergodic the experiments
                    # start from (one Q-construction; see tm_methods._resolve_q_method)
    ...
    def Run_FullRoutineNoRecessions(shock_type):
        ...
        if shock_type=='Check' or shock_type=='UI' or shock_type=='TaxCut':   
            ...
            _eco_solve(AggDemandEconomy_Routine, 'norec_' + shock_type)
    ...
    def Run_FullRoutine(shock_type):
        ...
        if Run_NonAD:
            AggDemandEconomy_Routine.switch_shock_type(shock_type)
            _eco_solve(AggDemandEconomy_Routine, 'nonad_' + shock_type)
        ...
        if Run_AD:
            if use_TM:
                ...
                # R6b consume (2026-08-03, HAFISCAL_AD_BELIEF_SEED=1, default
                # OFF): install a published AD-belief sidecar as the TM
                # Phase-1 warm start. Fingerprint-gated (same-engine TM fp or
                # the cross-engine jax fp — a cross-engine belief is an
                # allowed starting guess); the payload rides in
                # _ad_seed_CFunc so the switch_shock_type below cannot
                # clobber it; run_ad_tm honors + clears it (one-shot). The
                # training loop still runs to its own convergence_tol.
                if os.environ.get('HAFISCAL_AD_BELIEF_SEED', '0') == '1':
                    try:
                        from solution_cache import ad_belief as _adb_seed
                        from solution_cache.keys import (
                            gather_solve_inputs as _gsi_seed,
                            hash_solve_inputs as _hsi_seed)
                        _sl = _adb_seed.load_ad_belief(shock_type, Parametrization)
                        if _sl is None:
                            print(f"  [ad_belief] SEED (TM): no sidecar for "
                                  f"({Parametrization}, {shock_type}); flat start.",
                                  flush=True)
                        else:
                            _sbelief, _sfp, _seng = _sl
                            _fp_tm = _hsi_seed(_gsi_seed(
                                AggDemandEconomy_Routine, shock_type=shock_type,
                                mc_method=None))
                            _fp_jax = _hsi_seed(_gsi_seed(
                                AggDemandEconomy_Routine, shock_type=shock_type,
                                mc_method='jax_mc_2d_vmap'))
                            if (_sfp in (_fp_tm, _fp_jax)
                                    and len(_sbelief) == len(AggDemandEconomy_Routine.CFunc)):
                                AggDemandEconomy_Routine._ad_seed_CFunc = _sbelief
                                AggDemandEconomy_Routine._ad_warm_start = True
                                from solution_cache import record_reuse_event as _rre_ts
                                _rre_ts("ad_belief_seed", "consumed", "tolerance",
                                        shock_type=shock_type,
                                        source_engine=_seng, engine="tm")
                                print(f"  [ad_belief] SEED (TM): warm-start armed "
                                      f"for {shock_type} (engine={_seng}"
                                      f"{', CROSS-ENGINE' if _sfp != _fp_tm else ''}).",
                                      flush=True)
                            else:
                                print(f"  [ad_belief] SEED (TM): fingerprint/state "
                                      f"mismatch for {shock_type} (engine={_seng}); "
                                      f"flat start.", flush=True)
                    except Exception as _e_seed:
                        print(f"  [ad_belief] SEED (TM): skipped ({_e_seed}); "
                              f"flat start.", flush=True)
                AggDemandEconomy_Routine.switch_shock_type(shock_type)
                _eco_solve(AggDemandEconomy_Routine, 'adtm_' + shock_type)
                ...
                if os.environ.get('HAFISCAL_AD_BELIEF_PUBLISH', '1') == '1':
                    try:
                        ...
                        from solution_cache import record_reuse_event as _rre_tp
                        _rre_tp("ad_belief_seed", "published", "none",
                                shock_type=shock_type, engine="tm")
                ...
                # AD-equilibrium publish (owner ruling 2026-08-25, HAFISCAL_AD_EQUILIBRIUM_SHARE):
                # the FULL pair the TM AD loop leaves on the economy — the trained belief
                # CFunc and every household's policy solved at it (Phase 1 trains, Phase 2 only
                # evaluates) — so the welfare battery can measure on this equilibrium instead of
                # re-solving one. Best-effort, flag-gated; never aborts Step-5a.
                try:
                    from solution_cache import equilibrium_store as _eqs_pub
                    import tm_methods as _tmm_q
                    if _eqs_pub.enabled():
                        _eqs_pub.save(AggDemandEconomy_Routine, shock_type,
                                      producer={'site': 'step5a-tm', 'engine': 'tm',
                                                'sim_method': str(sim_method)},
                                      # the producer's conventions (recorded, printed on
                                      # HIT; a change REPLACES the entry — not key inputs)
                                      extra={'parametrization': str(Parametrization),
                                             'convergence_tol': float(convergence_tol_solvingAD),
                                             'num_max_iterations': int(num_max_iterations_solvingAD),
                                             'tm_mCount': tm_mCount,
                                             'tm_neutral_measure': bool(tm_neutral_measure),
                                             # BUG-093: the Q-construction is a producer
                                             # convention -- a change REPLACES the entry
                                             'tm_q_method': _tmm_q.resolve_q_method_public()})
                except Exception as _e_eq:
                    print(f"  [ad-equilibrium] publish skipped for {shock_type}: {_e_eq}", flush=True)
            ...
            if use_MC:
                ...
                if os.environ.get('HAFISCAL_AD_BELIEF_PUBLISH', '1') == '1':
                    try:
                        ...
                        _rre_mp("ad_belief_seed", "published", "none",
                                shock_type=shock_type, engine="hark_mc")
        ...
        if Run_1stRoundAD:
            if use_TM:
                ...
                AggDemandEconomy_Routine.switch_shock_type(shock_type)
                _eco_solve(AggDemandEconomy_Routine, 'first_' + shock_type)
                AggCons_base = base_results_tm['AggCons']
