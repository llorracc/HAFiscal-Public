...
# Dossier H4 (plan 20260809-0006h): the two literals above are NOT the
# household block's steady state — the Jacobian stage's weighted
# aggregates (now emitted in HA_Fiscal_Jacs.obj) are C=0.7620, A=20.47
# (10% and 14x apart). HAFISCAL_HANK_SS_SOURCE: "hardcoded" (default —
# the historical hybrid pairing), "jacs" (feed both from the obj),
# "jacs_c" (feed C only) — SENSITIVITY-PROBE arms for the owner ruling;
# the proper resolution is a modeling decision (recalibrate the GE
# block vs document the mismatch).
_ss_src = os.environ.get("HAFISCAL_HANK_SS_SOURCE", "hardcoded").strip().lower()
if _ss_src in ("jacs", "jacs_c"):
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                           'HA_Fiscal_Jacs.obj'), 'rb') as _f:
        _jacs_ss = pickle.load(_f)
    C_ss = C_ss_sim = float(_jacs_ss['C_ss_weighted'])
    if _ss_src == "jacs":
        A_ss = A_ss_sim = float(_jacs_ss['A_ss_weighted'])
    del _jacs_ss
    print(f"[hank-ss] SS from Jacobian stage: C_ss={C_ss:.6f} A_ss={A_ss:.6f}", flush=True)
...
do_splurge = True
if do_splurge:
    # H1 of the scoping dossier (plan 20260809-0006h): the historical
    # hardcoded 0.3 matches neither the paper's stated splurge
    # (0.25 / varsigma=0.246) nor any estimated value on disk
    # (0.2598 CDC / 0.2704 ESC). HAFISCAL_HANK_SPLURGE selects:
    # "legacy" -> 0.3 (DEFAULT until the owner adoption ruling),
    # "calib" -> the live interpretation-aware estimate
    # (EstimParameters.Splurge), or an explicit float.
    # HAFISCAL_QE_FIDELITY=1 always -> 0.3 (published reproduction).
    _sp_env = os.environ.get("HAFISCAL_HANK_SPLURGE", "legacy").strip().lower()
    if os.environ.get("HAFISCAL_QE_FIDELITY", "") == "1" or _sp_env == "legacy":
        splurge = 0.3
    elif _sp_env == "calib":
        from EstimParameters import Splurge as _calib_splurge
        splurge = float(_calib_splurge)
    else:
        splurge = float(_sp_env)
    ...
    # Dossier H1 consistency fix (flag-gated, default OFF pending the
    # same adoption ruling): the by-education JacobianDict enters the
    # SAME models but was historically never splurge-adjusted — an
    # internal inconsistency between the aggregate and by-educ blocks.
    if os.environ.get("HAFISCAL_HANK_SPLURGE_BYEDUC", "0").strip().lower() in ("1", "on", "true"):
        _old_by_educ = deepcopy(Jacobian_Dict_by_educ)
        for _educ in ['dropout', 'highschool', 'college']:
            _Ck, _Ak = 'C_' + _educ, 'A_' + _educ
            for jacobian_input in ['transfers', 'tau', 'UI_extend', 'UI_rr', 'eta', 'w']:
                _oldC = _old_by_educ[_Ck][jacobian_input]
                present_value = np.sum((_oldC/R**np.arange(periods)), axis=0)
                _comp = np.diag(present_value*R**np.arange(periods))
                Jacobian_Dict_by_educ[_Ck][jacobian_input] = (
                    splurge*_comp + (1-splurge)*_oldC)
                Jacobian_Dict_by_educ[_Ak][jacobian_input] = (
                    (1-splurge)*_old_by_educ[_Ak][jacobian_input])
...
# Candidate-routed (2026-08-03): tracked paper figure; raw canonical-path
# writes predated the QE freeze and rewrote it in place on every Step-4 run.
from generated_output import output_path as _hank_out
plt.savefig(_hank_out("Figures/HANK_IRFs_w_splurge.jpg"))
plt.savefig(_hank_out("Figures/HANK_IRFs_w_splurge.pdf"))
plt.savefig(_hank_out("Figures/HANK_IRFs_w_splurge.svg"))
plt.savefig(_hank_out("Figures/HANK_IRFs_w_splurge.png"))
...
# H2 of the scoping dossier (the 2026-06-12 QA's known-and-caption-
# mitigated regime mix): the emitted pickle historically carried
# transfers/tax from the ACTIVE-Taylor loop above while UI stayed at the
# FIXED-REAL values from the first loop — the active-UI series lands in
# the differently-named multipliers_UI_extend (line above). "consistent"
# makes all three active-Taylor. Default preserves the historical mix
# pending the owner/R3 ruling.
if os.environ.get("HAFISCAL_HANK_MULT_REGIME", "mixed").strip().lower() == "consistent":
    multipliers_UI_extensions = multipliers_UI_extend.copy()
    print("[hank-h2] multiplier pickle regime = consistent (active Taylor)", flush=True)
...
def plot_consumption_irfs_three_experiments(irf_UI1,irf_UI2,irf_UI3, irf_SC1,irf_SC2,irf_SC3, irf_TC1,irf_TC2,irf_TC3):
    ...
    # Candidate-routed (2026-08-03) — see the note at the sibling write site.
    from generated_output import output_path as _hank_out2
    plt.savefig(_hank_out2("Figures/HANK_IRFs_w_splurge.pdf"))
    plt.savefig(_hank_out2("Figures/HANK_IRFs_w_splurge.jpg"))
    plt.savefig(_hank_out2("Figures/HANK_IRFs_w_splurge.png"))
    plt.savefig(_hank_out2("Figures/HANK_IRFs_w_splurge.svg"))
    plt.show(block=False)
    # plt.close()
...
def plot_consumption_irf(irf1, irf2, irf3, y_max, filename, legend = False):
    ...
    if filename:
        ...
        lock_axes_for(filename)   # published-axes lock (owner 2026-08-26); after plt.ylim above
        plt.savefig(os.path.join(figures_dir, f"{routed}.pdf"))
        plt.savefig(os.path.join(figures_dir, f"{routed}.png"))
        plt.savefig(os.path.join(figures_dir, f"{routed}.svg"))
        plt.savefig(os.path.join(figures_dir, f"{routed}.jpg"))
        plt.show(block=False)
        # plt.close()
...
# Go up one level and then into the sibling directory (change 'output' to your desired name)
sibling_dir = os.path.join(os.path.dirname(current_dir), 'Results_HANK')

# Make sure it exists
os.makedirs(sibling_dir, exist_ok=True)

# Full path to the file
file_path = os.path.join(sibling_dir, 'multipliers_across_horizon_w_splurge.obj')
...
# Added 2026-08-09 (dossier S3.5): the GE stage previously emitted no run
# record for the paper's Figure-5 family.
try:
    ...
    _prov_pkl = _prov_os.path.join(_prov_ha, "Results_HANK",
                                   "multipliers_across_horizon_w_splurge.obj")
    _prov.emit([_prov_pkl], command=" ".join(_prov_sys.argv),
               argv=_prov_sys.argv,
