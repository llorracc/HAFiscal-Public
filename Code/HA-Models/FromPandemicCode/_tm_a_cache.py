...
def _agent_signature(agent) -> dict[str, Any]:
    ...
    # Earnings phase (2026-08-28): the hazard is a model primitive of the chain (MrkvArray already
    # separates the two, but make it legible); added ONLY when on so every phase-off key is unchanged.
    try:
        import earnings_phase as _ep
        if _ep.enabled():
            sig["earnings_phase_hazard"] = float(_ep.hazard())
    except ImportError:
        pass
    return sig
