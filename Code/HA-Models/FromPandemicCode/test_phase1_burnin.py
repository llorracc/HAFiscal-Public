...
def experiment_burnin_sweep():
    ...
    tm_data = compute_baseline_tm_data(economy, mCount=100, neutral_measure=True)

    # TM reference
    base_tm = run_experiment_tm(economy, shock_type="base", mCount=100, neutral_measure=True)
    N_total = sum(a.AgentCount for a in economy.agents)
    ...
    for wu in warmup_values:
        ...
        tm_data_w = compute_baseline_tm_data(eco_w, mCount=100, neutral_measure=True)
...
def experiment_init_mismatch():
    ...
    tm_data_0 = compute_baseline_tm_data(eco_0, mCount=100, neutral_measure=True)
...
def experiment_agentcount_sweep():
    ...
    for N in agent_counts:
        ...
        for s in range(n_seeds):
            ...
            tm_d = compute_baseline_tm_data(eco_s, mCount=100, neutral_measure=True, verbose=False)
...
def experiment_tm_grid_sweep():
    ...
    base_ref = run_experiment_tm(eco, shock_type="base", mCount=200, neutral_measure=True)
    ...
    for mc in m_counts:
        t0 = time.time()
        base_r = run_experiment_tm(eco, shock_type="base", mCount=mc, neutral_measure=True)
