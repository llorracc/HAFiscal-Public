...
def tm_q_buggy(eco_ref, meta, dist_aGrid_count):
    """Original buggy path: baseline via base chain, exp via TaxCut chain.
    Returns (multiplier, NPV(dC), NPV(dY))."""
    bl = compute_baseline_tm_data(eco_ref, dist_aGrid_count=dist_aGrid_count, neutral_measure=True, verbose=False)
    base = run_experiment_tm(eco_ref, "base", dist_aGrid_count=dist_aGrid_count, neutral_measure=True, verbose=False)
    ...
    exp = run_experiment_tm_nonbase(eco, "TaxCut", EM_init, bl,
                                    mCount=dist_aGrid_count, neutral_measure=True, verbose=False)
...
def tm_q_fixed(eco_ref, meta, dist_aGrid_count):
...
def main():
    ...
    mult_tm, npvc_tm, npvy_tm = tm_q_buggy(eco_ref, meta, dist_aGrid_count)
