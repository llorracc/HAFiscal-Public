...
def Welfare_Results(saved_results_dir,table_dir,Parametrization='Baseline'):
    ...
    #### METHOD 3
    # BUG-082 (2026-08-20): the consumption-equivalent gain. The pre-fix code divided the
    # lifetime-utility change by W_c = PDV(1)*N ("#*** assumes log utility") — a consumption
    # equivalent only at CRRA=1; at CRRA=2 that divisor is ~12x the correct one
    # (sum_t R^-t sum_i c^(1-rho)) and the quotient keeps units of 1/c. Now: the exact CRRA
    # consumption equivalent of each cell's utility change relative to that cell's own
    # no-policy path (Code/HA-Models/welfare_ce.py; HAFISCAL_WELFARE_CE_MODE=log_legacy
    # reproduces the published division). The cost share NPV/P_c is unchanged.
    import sys as _sys_ce, os as _os_ce
    _ha_root_ce = _os_ce.path.abspath(_os_ce.path.join(_os_ce.path.dirname(_os_ce.path.abspath(__file__)), '..'))
    if _ha_root_ce not in _sys_ce.path:
        _sys_ce.path.insert(0, _ha_root_ce)
    from welfare_ce import consumption_equivalent as _ce
    P_c = 1/(1-SP_discount_rate)*base_results['AggCons'][0]
    _T_ce, _N_ce = base_welfare.shape
    # lifetime horizon (default; HAFISCAL_WELFARE_CE_HORIZON): the reference path's utility
    # beyond the simulated window is completed with the stationary baseline's per-period
    # felicity sum (the economy returns to its ergodic distribution).
    _u_cont = float(np.mean(np.sum(base_welfare, axis=1)))
    def _ce_cell(pol_panel, ref_panel):
        return _ce(SP_welfare(pol_panel, SP_discount_rate), SP_welfare(ref_panel, SP_discount_rate),
                   CRRA, Rfree_base[0], _T_ce, _N_ce, u_cont_per_period=_u_cont)
    x_Check,  x_Check_rec,  x_Check_recAD  = _ce_cell(check_welfare,  base_welfare), _ce_cell(recession_Check_welfare,  recession_welfare), _ce_cell(recession_Check_welfare_AD,  recession_welfare_AD)
    x_UI,     x_UI_rec,     x_UI_recAD     = _ce_cell(UI_welfare,     base_welfare), _ce_cell(recession_UI_welfare,     recession_welfare), _ce_cell(recession_UI_welfare_AD,     recession_welfare_AD)
    x_TaxCut, x_TaxCut_rec, x_TaxCut_recAD = _ce_cell(TaxCut_welfare, base_welfare), _ce_cell(recession_TaxCut_welfare, recession_welfare), _ce_cell(recession_TaxCut_welfare_AD, recession_welfare_AD)

    Check_consumption_welfare   = (x_Check_rec  - NPV_AddInc_Rec_Check[-1]/P_c)  - (x_Check  - NPV_AddInc_Check[-1]/P_c)
    UI_consumption_welfare      = (x_UI_rec     - NPV_AddInc_UI_Rec[-1]/P_c)     - (x_UI     - NPV_AddInc_UI[-1]/P_c)
    TaxCut_consumption_welfare  = (x_TaxCut_rec - NPV_AddInc_Rec_TaxCut[-1]/P_c) - (x_TaxCut - NPV_AddInc_TaxCut[-1]/P_c)

    Check_consumption_welfare_AD   = (x_Check_recAD  - NPV_AddInc_Rec_Check[-1]/P_c)  - (x_Check  - NPV_AddInc_Check[-1]/P_c)
    UI_consumption_welfare_AD      = (x_UI_recAD     - NPV_AddInc_UI_Rec[-1]/P_c)     - (x_UI     - NPV_AddInc_UI[-1]/P_c)
    TaxCut_consumption_welfare_AD  = (x_TaxCut_recAD - NPV_AddInc_Rec_TaxCut[-1]/P_c) - (x_TaxCut - NPV_AddInc_TaxCut[-1]/P_c)
    ...
    # BUG-082: the same consumption equivalents as METHOD 3 (x_* above), in ratio form.
    Check_consumption_welfare5   = (x_Check_rec  / (NPV_AddInc_Rec_Check[-1]/P_c))  - (x_Check  / (NPV_AddInc_Check[-1]/P_c))
    UI_consumption_welfare5      = (x_UI_rec     / (NPV_AddInc_UI_Rec[-1]/P_c))     - (x_UI     / (NPV_AddInc_UI[-1]/P_c))
    TaxCut_consumption_welfare5  = (x_TaxCut_rec / (NPV_AddInc_Rec_TaxCut[-1]/P_c)) - (x_TaxCut / (NPV_AddInc_TaxCut[-1]/P_c))

    Check_consumption_welfare_AD5   = (x_Check_recAD  / (NPV_AddInc_Rec_Check[-1]/P_c))  - (x_Check  / (NPV_AddInc_Check[-1]/P_c))
    UI_consumption_welfare_AD5      = (x_UI_recAD     / (NPV_AddInc_UI_Rec[-1]/P_c))     - (x_UI     / (NPV_AddInc_UI[-1]/P_c))
    TaxCut_consumption_welfare_AD5  = (x_TaxCut_recAD / (NPV_AddInc_Rec_TaxCut[-1]/P_c)) - (x_TaxCut / (NPV_AddInc_TaxCut[-1]/P_c))
