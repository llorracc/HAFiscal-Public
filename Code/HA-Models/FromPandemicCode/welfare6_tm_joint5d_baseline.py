...
# Supervised pools (infrastructure A6): a worker killed by the OOM-killer fails
# the map within seconds instead of hanging it (Code/HA-Models/pool_supervision.py).
...
def _parallel_solve_scenarios(ctx_base, tmp_dir):
    ...
    results = supervised_map(
        _solve_scenario_worker, work, n_workers=3, start_method='fork',
        site='welfare6_tm_joint5d_baseline._parallel_solve_scenarios')
...
def main():
    ...
    if args.cohort_vmap:
        ...
        if workers <= 1:
            ...
            flat_raw = supervised_map(
                _run_one_duration, flat_work, n_workers=eff_workers,
                start_method='fork',
                site='welfare6_tm_joint5d_baseline cohort x duration pool')
