...
def test_default_off_env_gate(monkeypatch):
    # Explicit legacy tail: this test covers the on/off env gate only. Under the
    # 2026-07-23 power-law default, NAMG=1 would instead hit the tail-form guard
    # (covered by Code/HA-Models/test_step2_namg_tail_guard.py).
    monkeypatch.setenv("HAFISCAL_PF_DECAY_EXTRAP", "exp")
...
def test_deprecated_anderson_alias(monkeypatch):
    ...
    and does NOT warn."""
    monkeypatch.setenv("HAFISCAL_PF_DECAY_EXTRAP", "exp")  # gate test; see above
    monkeypatch.delenv("HAFISCAL_STEP2_NAMG", raising=False)
    monkeypatch.delenv("HAFISCAL_STEP2_ANDERSON", raising=False)
    assert AggregateDemandEconomy._step2_namg_enabled() is False
...
def _solve_capture(eco, use_namg):
    if use_namg:
        ...
        # Benchmarking escape hatch (F1 tail guard, 2026-07-23): under the
        # power-law default the NAMG opt-in refuses (exp-tailed solver); this
        # parity test IS the sanctioned mismatch-benchmark use of the escape.
        # In-sample (m <= 12) the tail-form feedback is far below the 5e-3
        # solver-parity gate, so the comparison's meaning is unchanged.
        os.environ["HAFISCAL_NAMG_ALLOW_TAIL_MISMATCH"] = "1"
    else:
        os.environ.pop("HAFISCAL_STEP2_NAMG", None)
    try:
        ...
        os.environ.pop("HAFISCAL_NAMG_ALLOW_TAIL_MISMATCH", None)
