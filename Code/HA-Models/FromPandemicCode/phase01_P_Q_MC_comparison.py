...
def run_all_tm(economy, dist_aGrid_count, neutral_measure, label):
    """Run base + all experiments, return dict of NPVs and TE series."""
    t0 = time()

    bl_data = compute_baseline_tm_data(economy, dist_aGrid_count=dist_aGrid_count,
                                       neutral_measure=neutral_measure, verbose=False)
    base = run_experiment_tm(economy, shock_type='base', dist_aGrid_count=dist_aGrid_count,
                             neutral_measure=neutral_measure, verbose=False)
    ...
    for name, shock_type, _ in experiments:
        ...
        r = run_experiment_tm_nonbase(eco, shock_type, rec_path, bl_data,
                                      mCount=dist_aGrid_count, neutral_measure=neutral_measure,
                                      verbose=False)
...
print(f"\n{'='*76}")
print(f"TM EXPERIMENTS (dist_aGrid_count={TM_MCOUNT}, {EXP_HORIZON} periods)")
print("=" * 76)
...
print(f"  dist_aGrid_count={TM_MCOUNT}, MC N={MC_N_TOTAL:,} x {MC_NUM_SEEDS} seeds, "
...
print("SUMMARY")
print("=" * 76)
print(f"  TM grid:    dist_aGrid_count = {TM_MCOUNT}")
