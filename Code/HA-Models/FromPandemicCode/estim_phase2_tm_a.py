...
def betas_obj_func_educ_tm_a(beta, spread, GICx, educ_type=2, print_mode=False):
    ...
    # ATI-ESTIMATION DEFAULT (owner ruling 2026-07-27): the ConsumedATI routing
    # is this surface's default solver accelerator — measured 3.05× per eval-set
    # and a 7-minute warm re-estimation; consistency vs the EGM calibration:
    # β to ~6 digits in every group, worst ∇ shift 0.714% ≤ the 2% ∇ budget
    # (ratified 2026-07-27 — ∇ is a weakly-identified spread; the 0.1% β
    # standard priced a 3× mode out of default over a 1.3e-4-absolute wobble).
    # Entry-point-scoped like the tail state; explicit env always wins; EGM
    # remains one env var away (HAFISCAL_STEP5_ATI=0) as the cross-check.
