...
def return_parameters(Parametrization='Baseline',OutputFor='_Main.py'):
    ...
    # for output
    # CWD-TRAP FIX (2026-08-12): these were derived from os.getcwd(), so any
    # import from outside FromPandemicCode/ (pytest at repo root, tools,
    # notebooks) silently resolved the calibration reads against the wrong
    # tree and fell back to the NON-ESC files (the "[ESC calibration HAZARD]"
    # warning — a real ~4% multiplier shift if the numbers were then used).
    # Anchor on __file__ instead: identical directories under the production
    # cwd (FromPandemicCode/), correct ones everywhere else.
    Abs_Path         = os.path.dirname(os.path.abspath(__file__))
    Abs_Path_Results = os.path.dirname(Abs_Path)
    ...
    import earnings_phase as _ep   # Code/HA-Models is on sys.path (EstimParameters puts it there)
    ...
    # WORLD axis (2026-06-14): the DiscFacEstim *betas* are also world-specific
    # (the as-corrected counterfactual re-estimates them), so they load through
    # resolve_calib_path (interp + world, with a loud wrong-world fallback guard).
    # The SHARED splurge (Result_AllTarget / Result_CRRA_*) is world-independent
    # and stays on resolve_path (interp-only). default world -> both identical.
    import sys as _sys
    _parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    if _parent_dir not in _sys.path:
        _sys.path.insert(0, _parent_dir)
    from _interpretation import resolve_path, resolve_calib_path

    CRRA = 2.0
    Rfree_base = [1.01]
    Rspell = 6            # Expected length of recession, in quarters. If R_shared = True, must be an integer
    betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_2.0_R_1.01.txt')
    Splurge_txt_location = resolve_path(Abs_Path_Results+'/Target_AggMPCX_LiquWealth/Result_AllTarget.txt')
    # SINGLE SOURCE (Q14, conclusions_private/2026-09-02_dual-path-sweep_table.md §4; Phase-2 brief
    # 2026-09-03): the no-benefits unemployment income (0.5 of permanent income) is defined ONCE in
    # EstimParameters.py next to IncUnemp, where the positional-argv convention of every Step-2 caller
    # (argv = Rfree CRRA IncUnemp IncUnempNoBenefits Splurge) can override BOTH. IncUnemp already
    # arrives here through the import block above; a literal 0.5 here meant an argv[4] override reached
    # the Step-2 estimation but never this Step-5 parameter set. Same precedence for both rates now:
    # argv override > EstimParameters default, then the LowerUBnoB branch below. Do not re-literalize.
    from EstimParameters import IncUnempNoBenefits
    from EstimParameters import ADelasticity   # SINGLE SOURCE: the base demand elasticity is defined
    # ONCE in EstimParameters.py and shared here (Parameters already imports EstimParameters). The
    # ADElas parametrization override (→0.5) is applied just below; do not re-literalize the base.
    ...
    if Parametrization == 'ADElas' or Parametrization == 'ADElas_PVSame':
        ...
        CRRA = 1.0
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_1.0_R_1.01.txt')
        ...
        CRRA = 3.0
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_3.0_R_1.01.txt')
        ...
        Rfree_base = [1.005]
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_2.0_R_1.005.txt')
    elif Parametrization == 'Rfree_1015' or Parametrization == 'Rfree_1015_PVSame':
        Rfree_base = [1.015]
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_2.0_R_1.015.txt')
        ...
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_2.0_R_1.01_altBenefits.txt')
        IncUnempNoBenefits = 0.15
        IncUnemp = 0.3
    elif Parametrization == 'Splurge0' or Parametrization == 'Splurge0_PVSame':
        betas_txt_location = resolve_calib_path(Abs_Path_Results+'/Results/DiscFacEstim_CRRA_2.0_R_1.01_Splurge0.txt')
    ...
    # DERIVATION under the BUG-053 default (GPF-shave, cap = GICmax·factor^CRRA
    # with CRRA=2 — see EstimParameters.gic_capped_beta): the CO invariant
    # requires cap_CO = 1.0064·factor² > 0.98 ⇒ factor > 0.9868 ⇒ GICx > 4.31.
    # CO fallback re-derived 4.0 → 4.5 (2026-06-12, owner-approved; finding b):
    # GICx=4.5 ⇒ factor = logistic(4.5) ≈ 0.9890 ⇒ cap_CO ≈ 1.0064×0.9890² ≈
    # 0.9844 > 0.98 ✓. (The old 4.0, derived under the pre-BUG-053 LINEAR
    # β-shave cap = GICmax·factor where the bound was factor > 0.9737 ⇒
    # GICx > 3.61, gave cap_CO ≈ 0.9705 under the GPF-shave — it CLIPPED the
    # central College β whenever the fallback fired.) HS=3.0 still passes:
    # cap_HS ≈ 1.0057×0.9526² ≈ 0.9126 > 0.91; DO's β=0.72 is far below cap.
    ...
    GICxDefaults = [4.0, 3.0, 4.5]
  
    myEstim = [[],[],[]]
    readCount = 0
    # As-corrected vintage guard (2026-08-23): the calibration's footer splurge
    # must equal the splurge in use — content-based, since mtimes cannot expose
    # a tracked-stale _ascorrected[_candidate] file (the 08-12/08-23 trap class).
    from _interpretation import assert_calib_vintage_matches
    assert_calib_vintage_matches(betas_txt_location, Splurge_txt_location)
    ...
    # UI extension
    from EstimParameters import UBspell_extended  # = 5: average duration of unemployment benefits when extended and assuming policy remains in place, in quarters (one source; the freeze's ExtraUBperiods = UBspell_extended - UBspell_normal)
    ...
    # Base count honors the same env override as EstimParameters (wiring gap found
    # 2026-07-23: the Step-5 K·h̄ block scaled from a hardcoded 48, so count sweeps
    # never reached the multiplier pipeline).
    aXtraCount = int(os.environ.get('HAFISCAL_AXTRA_COUNT', 48))
    if 'HAFISCAL_AXTRA_COUNT' in os.environ:
        print(f'[axtra-override:step5] HAFISCAL_AXTRA_COUNT → aXtraCount={aXtraCount}')
    ...
    T_age = None            # EPOCH 2026-07-27 (owner): NO maximum-age cap — belief-consistent
                            # perpetual youth (agents' constant-hazard beliefs = the actual
                            # process; the paper's stated model). The QE-era cap (200 = die at
                            # 75; Crawley 2022, commit 770d4d04; BUG-038-restored) executed
                            # 28.5% of each cohort at an UNDOCUMENTED wall. Old world:
                            # HAFISCAL_T_AGE=200 or HAFISCAL_WORLD=as-corrected. See
    ...
    # ---- Step-5 per-group solve-grid top (local2 measured-tail candidate) ----
    # Mirror of the EstimParameters.py precedence block (owner design 2026-07-22,
    # plans/20260722_local-two-secant-tail-q_plan.md): the local two-secant tail
    # exponent (HAFISCAL_PF_DECAY_Q=local2) needs log-leverage in (x+h) below the
    # grid top — span-infeasible at the legacy aXtraMax=40 << h̄~197, where the
    # estimator can only fall back. Rule from primitives (no solve):
    # aXtraMax_g = K·h̄_g with h̄_g = Γ_g/(R−Γ_g) and K = HAFISCAL_PF_DECAY_AMAX_MULT
    # (default 3). The count is scaled to hold near-0 density (a fixed count thins
    # the calibration-moment region and shifts β ~0.3%; EstimParameters E4/E5).
    # Precedence: HAFISCAL_SOLVE_AMAX > local2 rule > legacy. Default (both
    # triggers unset): the block is a no-op and every init dict is byte-identical.
    # NOTE for sensitivity parametrizations: h̄ uses THIS run's Rfree_base/Γ_g, so
    # e.g. Rfree_1005 with College Γ≈1.0049 yields a very large K·h̄ — local2 is
    # opt-in and validated only on the main-config h̄ (see the plan).
    # SST (owner ruling 2026-07-24): the precedence (SOLVE_AMAX > K·h̄-under-
    # measured-Q > legacy; the endogenous branch is EstimParameters' opt-in and
    # stays disabled here — allow_endogenous=False preserves this site's
    # historical semantics) lives in grid_sizing.resolve_solve_grid, the SAME
    # implementation EstimParameters + the Step-1 setup consume. This site only
    # gathers its primitives per group. Byte-parity gated by
    # test_resolve_solve_grid.py + the 2026-07-24 golden-probe matrix.
    # FAST_GRIDS/HAFISCAL_AXTRA_COUNT still win the count basis by arriving in
    # aXtraCount (!= the 48 anchor blocks the 192 promotion).
    import grid_sizing as _gs  # Code/HA-Models is on sys.path (parent-dir insert above)
    _grid_why = None
    (aXtraMax_d, aXtraCount_d, _grid_why) = _gs.resolve_solve_grid(
        Rfree=Rfree_base[0], PermGroFac=PermGroFac_base_d[0],
        legacy_aMax=aXtraMax, legacy_count=aXtraCount, aXtraMin=aXtraMin,
        allow_endogenous=False, count_basis_anchor=48, tag='[grid_sizing:step5]')
    (aXtraMax_h, aXtraCount_h, _grid_why) = _gs.resolve_solve_grid(
        Rfree=Rfree_base[0], PermGroFac=PermGroFac_base_h[0],
        legacy_aMax=aXtraMax, legacy_count=aXtraCount, aXtraMin=aXtraMin,
        allow_endogenous=False, count_basis_anchor=48, tag='[grid_sizing:step5]')
    (aXtraMax_c, aXtraCount_c, _grid_why) = _gs.resolve_solve_grid(
        Rfree=Rfree_base[0], PermGroFac=PermGroFac_base_c[0],
        legacy_aMax=aXtraMax, legacy_count=aXtraCount, aXtraMin=aXtraMin,
        allow_endogenous=False, count_basis_anchor=48, tag='[grid_sizing:step5]')
    if _grid_why is not None:
        print(f"[grid_sizing:step5] {_grid_why} → per-group solve grids aXtraMax/aXtraCount: "
              f"Dropout={aXtraMax_d:.0f}/{aXtraCount_d} HS={aXtraMax_h:.0f}/{aXtraCount_h} "
              f"College={aXtraMax_c:.0f}/{aXtraCount_c} (legacy {aXtraMax}/{aXtraCount})")
    ...
    # UI-extension encoding: see EstimParameters.py (and Code/HA-Models/ui_extension_rule.py)
    # for HAFISCAL_UI_STATE_ENCODING / HAFISCAL_UI_EXTENSION_POLICY. Determined at import
    # time and shared with EstimParameters.py.
    ...
    if Parametrization in ('Reduced_Run', 'Smoke_Test', 'HS_Only'):
        ...
        T_age = None  # EPOCH 2026-07-27: uncapped in Reduced too (one model; was 100)
        DiscFacCount = 1
    ...
    def make_cond_mrkv_arrays_base(Urate_normal, Uspell_normal, UBspell_normal):
        ...
        # so the base scenario carries the same micro states as the experiments.
    ...
    #   legacy:   ub = UBspell_normal (= 2)                            -> 4 micro states
    #   calendar: ub = UBspell_normal + n_extension (3 under the paper's window, 2 under
    #             'history')                                            -> 7 / 6 micro states
    #   bug_fix:  ub = UBspell_normal + 2                               -> 6 micro states
    # Policy_ExtraBenefitQuarters (EstimParameters) is that extension-state count, 0 under
    # legacy. The extension states make the extension quarters explicit in the state space so
    # the policy can be delivered in INCOME (the plain chain in every scenario) instead of by
    # the transition freeze -- see EstimParameters.py / ui_extension_rule.py.
    ...
    def make_cond_mrkv_arrays_recession_ui(Urate_normal, Uspell_normal, UBspell_normal, Urate_recession, Uspell_recession, num_experiment_periods, ExtraUBperiods):
        ...
        if HAFISCAL_UI_STATE_ENCODING != 'legacy':
            # calendar / bug_fix: no freeze -- cond_mrkv is identical to the recession scenario
            # (the same Markov dynamics across the policy/no-policy pair). The policy lives
            # entirely in IncShkDstn (extension-state income per the rule SST).
            MrkvArray_normal      = small_MrkvArray(E_persist_normal,    U_persist_normal,    _ub_chain_length)
            MrkvArray_recession   = small_MrkvArray(E_persist_recession, U_persist_recession, _ub_chain_length)
            CondMrkvArrays = [MrkvArray_normal, MrkvArray_recession]*(num_experiment_periods+1)
        else:
            # Legacy: the published freeze window (transition_ub=False on transitions INTO
            # experiment blocks 1..ExtraUBperiods; ExtraUBperiods = UBspell_extended - UBspell_normal)
            MrkvArray_normal         = small_MrkvArray(E_persist_normal,    U_persist_normal,    _ub_chain_length)
            MrkvArray_recession      = small_MrkvArray(E_persist_recession, U_persist_recession, _ub_chain_length)
    ...
    def make_full_mrkv_array(MacroMrkvArray, CondMrkvArrays):
        # Earnings phase: wrap every macro pair's employment block into [[(1-h) M, h M], [0, M]]
        # (macro outermost, phase in the middle, employment innermost -- see earnings_phase.py);
        # MacroMrkvArray / CondMrkvArrays stay unwrapped. Identity when the phase is off. The SAME
        # wrap as EstimParameters.make_full_mrkv_array; AggFiscalModel.update_mrkv_array uses THIS one.
        return [_ep.wrap_chain(make_hierarchical_mrkv_array(MacroMrkvArray, CondMrkvArrays),
                               CondMrkvArrays[0].shape[0])]
    ...
    if _ep.enabled():
        # Earnings phase: the wrapped chain's stationary vector is degenerate (matured absorbing);
        # newborns are born GROWING at the employment chain's ergodic mix (earnings_phase.py).
        init_mrkv_dist_d = _ep.growing_block_ergodic(MrkvArray_base_d[0], num_base_MrkvStates)
        init_mrkv_dist_h = _ep.growing_block_ergodic(MrkvArray_base_h[0], num_base_MrkvStates)
        init_mrkv_dist_c = _ep.growing_block_ergodic(MrkvArray_base_c[0], num_base_MrkvStates)
    else:
    ...
    # HAFISCAL_T_AGE: override the maximum-age cap (owner decision 2026-07-26).
    # The QE-era cap (T_age=200 = die at 75 having entered at 25) is UNDOCUMENTED
    # in the paper and, under memoryless LivPrb=1-1/160, executes 28.5% of every
    # cohort at the wall — raising effective mortality 40% and thinning the
    # wealth tail (cap-atom Kesten alpha 2.16 capped vs 1.78 uncapped). Values:
    # unset/'' = status quo (200 Baseline / 100 Reduced); 'none' = NO cap (the
    # paper's own perpetual-youth reading; owner-selected end-state, flips at
    # the tail-state epoch); positive int = that cap in quarters. Registered in
    # docs/ENV_FLAGS.md; consumers handle None via _effective_LivPrb (exact) and
    # effective_age_chain_length (tolerance-truncated age chains).
    ...
    if _t_age_env in ('', None):
        ...
        try:
            T_age = int(_t_age_env)
        except ValueError:
            raise ValueError(
                f"HAFISCAL_T_AGE must be 'none' or a positive integer (quarters); "
                f"got {_t_age_env!r}")
        if T_age <= 0:
            raise ValueError(f"HAFISCAL_T_AGE integer must be positive; got {T_age}")

    # Define a parameter dictionary for dropout type
    init_dropout = {"cycles": 0, # 00This will be overwritten at type construction
                    "T_cycle": T_cycle,
                    # Burn-in length = 2 × cap (pre-dc6390e3 QE-era convention);
                    # uncapped: keep the Baseline-era 400 (Step-1 MC burn-in only,
                    # deprecated path per BUG-063 — the TM engines ignore T_sim).
                    'T_sim': (T_age * 2) if T_age is not None else 400,
                    'T_age': T_age,
                    ...
                    # per-state arrays over the base micro block ([growing J | matured J] under the earnings
                    # phase: Rfree/LivPrb identical across phases, PermGroFac = 1 in every matured state)
    ...
    if Parametrization in ('Reduced_Run', 'Smoke_Test', 'HS_Only'):
        ...
        act_T = 100
        # AgentCountTotal override for the reduced parametrizations. BUG-fix
        # (2026-06-13, BUG-058): this block was mis-nested under `if _ad_iter_env:`
        # since commit e1fa6368 (2026-05-04), so (a) the reduced params silently kept
        # the full Baseline default (10000) unless HAFISCAL_AD_MAX_ITER was set, and
        # (b) setting HAFISCAL_AD_MAX_ITER to ANY value also forced Baseline->5000.
        # Re-parented here so AgentCountTotal depends ONLY on the parametrization and
        # HAFISCAL_AD_MAX_ITER controls only num_max_iterations_solvingAD. Baseline
        # (the unset path) is unaffected (stays 10000), so frozen paper numbers do
        # not change. See ENV_FLAGS.md HAFISCAL_AD_MAX_ITER (owner-review resolved
        # 2026-06-13).
    ...
    # NOTE (BUG-058 fix): this env-override block is INDEPENDENT of the AgentCountTotal
    # block above. It was previously inserted such that the AgentCountTotal if/elif/else
    # got re-parented under `if _ad_iter_env:`, which (a) silently halved Baseline N to
    # 5000 whenever HAFISCAL_AD_MAX_ITER was set, and (b) dropped the reduced-parametrization
    # small-N override whenever it was unset. Keep these two blocks separate.
    ...
    # cohort gets 100% of AgentCountTotal. effective_EducShares (a zero entry =
    # masked cohort) is the single source of truth for "which cohorts are active".
    # Consumers build ONLY the active cohorts: Simulate()'s TypeList loop skips
    # zero-share education groups (so masked cohorts are never created/solved/
    # simulated — ~3-5x faster for HS_Only, numerically identical since masked
    # cohorts carried AgentCount=0 / zero weight), and the standalone economy
    # builders (test_asymptotic_equality_revised.setup_economy, welfare6_scenario)
    # likewise `continue` on data_EducShares[e]==0. The 3-cohort BaseTypeList
    # *templates* remain (cheap to instantiate; only TypeList membership triggers
    # a solve), so the HAFISCAL_AGENTCOUNT_{D,H,C} escape hatch can still force a
    # masked cohort back in.
