...
for a in AggEco.agents:
    ...
    # BUG-090 (2026-08-24): unemployed-state income via the income-process SST (the
    # same call as Simulate.py / the estimator), honoring HAFISCAL_PERM_DURING_UNEMP;
    # HAFISCAL_WELFARE6_LEGACY_UNEMP_INCSHK=1 rebuilds the pre-fix point mass.
    _p_on = getattr(a, 'perm_shocks_during_unemployment', False)
    _t_on = getattr(a, 'tran_shocks_during_unemployment', False)
    if os.environ.get('HAFISCAL_WELFARE6_LEGACY_UNEMP_INCSHK', '').strip().lower() in ('1', 'on', 'true', 'yes'):
        _p_on = _t_on = False
    ...
    # BUG-091 (2026-08-24): tax cut in blocks 2..TaxCutPeriods+1 (8 quarters), via the SST.
...
for a in AggEco.agents:
    ...
    a.RfreeNow = 1.0
    a.CaggNow = 1.0
# The QE way under an age cap (owner 2026-08-29 14:40; see welfare6_scenario.py): burn-in >= 2*T_age, TM injection OFF unless =1.
_T_age_cap = next((getattr(a, 'T_age', None) for a in AggEco.agents), None); _capped = _T_age_cap is not None
_burn_q = int(os.environ.get('HAFISCAL_MC_BURNIN_QUARTERS', '0') or 0) or (2 * int(_T_age_cap) if _capped else 0)
if _capped and int(AggEco.act_T) < _burn_q:
    print(f"[hybrid-welfare6] age cap T_age={_T_age_cap}: burn-in raised from {AggEco.act_T} to {_burn_q} quarters (the QE way)", flush=True)
    AggEco.act_T = _burn_q
    AggEco.EconomyMrkvNow_hist = [0] * AggEco.act_T
    for a in AggEco.agents:
...
if _capped:
    print(f"[hybrid-welfare6] age cap T_age={_T_age_cap}: " + ('TM-ergodic injection FORCED (HAFISCAL_WELFARE6_TM_INIT=1)' if _do_tm_init else f'QE-way burn-in of {AggEco.act_T} quarters; TM-ergodic injection OFF'), flush=True)
if _do_tm_init:
