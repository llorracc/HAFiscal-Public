...
STATUS (2026-06-10 decision): NON-CANONICAL — MC + CRN + stratified-shuffle is
the canonical method for ALL welfare-6 cells (see
conclusions_private/2026-06-10_welfare_method_unified_MC.md). TM-a welfare is
kept as a TaxCut-only backup (joint-5D) and diagnostic cross-check; do not use
its output for the paper.
...
~5-15% UI. (The "MC unreliable for UI" caveat that originally accompanied that
UI figure is OBSOLETE: since the stratified-shuffle fix, MC ui_rec agrees to
+0.05% and UI cells are reportable — 2026-06-10 decision.)
...
def _setup_tm_a_agents(ctx):
    ...
    for ag in AggEco.agents:
        bd = compute_baseline_tm_data(AggEco, dist_aGrid_count=aCount)
...
def run_scenario_tm_full_capture(ctx, base_baseline_tm_data, shock_type,
    ...
    for d_idx, dur in enumerate(durations):
        ...
        if is_ad:
            J = _ep.j_full(AggEco.num_base_MrkvStates)   # belief-block stride (2J under the earnings phase)
            CFunc = AggEco.CFunc
    ...
    for i, agent in enumerate(AggEco.agents):
        per_cohort_agent_state.append({
            ...
            # the MICRO count of a macro block (2J under the earnings phase): the pol->base embedding `jp % J`
            # below must keep the (phase, emp) pair, not just emp
            'num_base_MrkvStates': int(_ep.j_full(agent.num_base_MrkvStates)),
...
def main():
    ...
    aCount = int(os.environ.get("HAFISCAL_TM_MCOUNT", 100))
    print(f"Computing baseline_tm_data (dist_aGrid_count={aCount})...")
    t1 = time.time()
    initial_bd = compute_baseline_tm_data(ctx['AggEco'], dist_aGrid_count=aCount)
