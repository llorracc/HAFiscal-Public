...
class AggFiscalType(AggIndMrkvConsumerType):
    ...
    def update_solution_terminal(self):
        ...
        #
        # GATING: only when HAFISCAL_PF_DECAY_EXTRAP is ON. Default OFF leaves the
        # legacy IdentityFunction (c=m) terminal byte-for-byte unchanged. The same
        # FHWC/RIC guard as the solver: if MPCmin<=0 (RIC fails) or h non-finite
        # (FHWC fails) there is no finite affine PF line, so keep the legacy c=m
        # terminal (the existing fallback).
        _pf_decay = os.environ.get('HAFISCAL_PF_DECAY_EXTRAP', '0') not in ('0', '', 'false', 'False')
        if _pf_decay:
...
def solve_agg_cons_markov_alt(solution_next,IncShkDstn,LivPrb,DiscFac,CRRA,Rfree,PermGroFac,
    ...
    _pf_decay_raw = os.environ.get('HAFISCAL_PF_DECAY_EXTRAP', '0')
    _pf_decay = _pf_decay_raw not in ('0', '', 'false', 'False')
    # Form of the decay tail when ON: the literal value 'powerlaw' selects the
    # power-law gap (theory-correct; HAFiscal-local mirror of the HARK-PR
    # decay_extrap_form='powerlaw' — see powerlaw_decay.py + the 2026-06-24
    # derivation §8); ANY other truthy value keeps the legacy exponential
    # attach (the original PR-3 form), so existing '1' callers are unchanged.
    _pf_slice_ctor = LinearInterp
    if _pf_decay and _pf_decay_raw.strip().lower() == 'powerlaw':
        ...
        for n in range(Ccount):
            ...
            if _pf_decay:
                ...
                if _level_diff > _tol and _slope_top >= _sl:
                    cFuncBaseByC_list.append(_pf_slice_ctor(m_temp, c_temp, intercept_limit=_ic, slope_limit=_sl))
                else:
...
class AggregateDemandEconomy(Market):
    ...
    def _step2_namg_enabled():
        ...
        if os.environ.get('HAFISCAL_STEP2_NAMG', '') == '1':
            return True
        if os.environ.get('HAFISCAL_STEP2_ANDERSON', '') == '1':
            import warnings
            warnings.warn(
