...
from income_process_sst import tile_PermGroFac_composite, taxcut_block_range
# Earnings-phase SST (owner ruling 2026-08-28): Mrkv = J_full*macro + J*phase + emp with J = num_base_MrkvStates
# (the employment/UI count) and J_full = 2J when HAFISCAL_EARNINGS_PHASE_HAZARD > 0 (else J: every helper is the
# identity). Employment decodes `% J` stay valid; macro decodes are `_ep.macro_of` (// J_full); the MC carries the
# phase per agent through a separate hazard draw (`_advance_phase`) and recomposes with `_ep.compose`.
import earnings_phase as _ep
import onset_spike_rule as _osr   # onset-spike SST; BUG-122 Step 0
...
def _mrkv_strata_ids(env_spec, pLvl, aNrm, agents):
    """Econ-1 (2026-08-28): stratum id per agent in `agents` from HAFISCAL_SHUFFLE_MRKV_STRATA, e.g. "p:5" (quintiles of
    permanent income among the agents of the source state) or "p:5,a:4" (income quintiles x wealth quartiles). Returns
    None when the knob is unset/empty (= the existing rank-based stratified mode)."""
    if not env_spec:
        return None
    n_p = n_a = 1
    for part in env_spec.split(","):
        ...
        if key.strip() == "p":
            ...
            raise ValueError(f"HAFISCAL_SHUFFLE_MRKV_STRATA: unknown key {key!r} (use p:<n>,a:<n>)")
    if n_p == 1 and n_a == 1:
        return None
    def _bins(x, n):
        if n == 1 or len(x) < 2 * n:
        ...
        return (ranks * n) // len(x)            # equal-count quantile bins by rank
    return _bins(np.asarray(pLvl, float)[agents], n_p) * n_a + _bins(np.asarray(aNrm, float)[agents], n_a)


def _madow_round(K_exact, N_g, u):
    """Systematic (Madow) rounding of the fractional quotas: the leftover L = N_g - sum(floor) units go to the destinations
    whose cumulative-residual interval contains one of the points u, u+1, ..., u+L-1 (u ~ U(0,1)). Totals are exact and
    E[K_k] = K_exact_k EXACTLY, whatever the stratum size -- Hamilton's largest-residual rule (the plain path) is only
    asymptotically unbiased and a stratum of 10-15 households can round 4.2 -> 4 the same way every period."""
    K = np.floor(K_exact).astype(int); L = int(N_g - K.sum())
    if L > 0:
        c = np.cumsum(K_exact - K); c[-1] = max(c[-1], float(L))          # guard the float tail
        add = np.minimum(np.searchsorted(c, u + np.arange(L), side="left"), len(K) - 1)
        K[add] += 1                                                       # each destination at most once (residual < 1)
    return K


def _stratified_quota_assign(agents_j, draws_j, probs, strata=None, rng=None):
    """Rank-based stratified assignment of the agents of one source state to destination states with exact quotas,
    optionally WITHIN strata (Econ-1: income / wealth quantiles) so the movers' composition is fixed per period. Each
    household's marginal transition probability is `probs` regardless of strata (rank within stratum is uniform); quotas
    are exact within each stratum. Rounding of the fractional quotas: floor + largest residual (Hamilton) on the plain
    path (strata None, rng None -- byte-identical to the certified engine); with strata AND an rng, Madow's systematic
    rounding (`_madow_round`, one uniform per stratum in stratum order) so the expected quota equals N_g * p exactly even
    in strata of a dozen households. Returns the destination state per agent (aligned to agents_j)."""
    J = len(probs); dest = np.zeros(len(agents_j), dtype=int)
    groups = [np.arange(len(agents_j))] if strata is None else [np.where(strata == g)[0] for g in np.unique(strata)]
    for idx in groups:
        if len(idx) == 0:
            continue
        order = idx[np.argsort(draws_j[idx], kind="stable")]
        N_g = len(idx); K_exact = N_g * np.asarray(probs, float)
        if rng is not None:     # Madow rounding: always on the strata path; on the plain path only with HAFISCAL_SHUFFLE_MRKV_ROUNDING=madow
            K = _madow_round(K_exact, N_g, float(rng.random()))
        else:
            K = np.floor(K_exact).astype(int)
            leftover = N_g - int(K.sum())
            if leftover > 0:
                residuals = K_exact - K
                for _ in range(leftover):
                    k_max = int(np.argmax(residuals)); K[k_max] += 1; residuals[k_max] = -1.0
        offset = 0
        for kk in range(J):
            if K[kk] == 0:
                continue
            dest[order[offset:offset + K[kk]]] = kk; offset += int(K[kk])
    return dest



def _spike_t0_exempt():
    """Hold the onset-spiked households in their first benefit state at t = 0?

    Thin delegate to the onset-spike SST, which carries the full account of what the spike is,
    why its published ordering makes the state label lie, and why BUG-122's four-quarter cap is
    unimplementable until it does not: ``Code/HA-Models/onset_spike_rule.py``. The TM engines
    read the same flag through ``onset_spike_rule.period0_employed_row``, so the two engines'
    timing cannot drift apart. Flag ``HAFISCAL_ONSET_SPIKE_T0_EXEMPT``; the catalog default is ON
    in BOTH worlds (BUG_FIX, owner ruling 2026-09-06). Unsetting it does NOT select OFF on a
    normal run -- the world block setdefaults it -- so a reference arm must pin ``0`` explicitly.
    """
    return _osr.t0_exempt()


def _run_experiment_views(Full_Output, welfare_dict, return_dict):
    """The dict(s) ``run_experiment`` hands back, for each documented ``Full_Output`` value.

    A function of its own, and gated, because the three-way branch is easy to break in silence.
    ``'ForWelfare'`` builds ONLY the welfare view, and a version of this that reached for the
    plain one instead raised ``UnboundLocalError: return_dict`` in every scenario of the welfare
    battery -- introduced 2026-09-05 with the ``'both'`` view and found 2026-09-06 by the first
    battery run after it. Nothing in `make test-fast` runs a welfare scenario, so only a gate on
    the branch itself catches this class of mistake.

    Returns a tuple: the views to decorate with the shared entries, and (for ``'both'``) the
    tuple the caller returns as-is.
    """
    if Full_Output == 'both':
        return (welfare_dict, return_dict)
    if Full_Output == 'ForWelfare':
        return (welfare_dict,)
    return (return_dict,)


def _spike_draw_threshold(agent, shock_type, urate_normal, urate_target):
    """The uniform-draw cutoff that selects the onset-spiked households.

    The published rule lays off ``f = (Ur - Un) / (1 - Un)`` of the employed. Under the t = 0
    exemption those households are held in their first benefit state instead of transitioning,
    so none of them finds work again in the same quarter and FEWER must be laid off to reach the
    same unemployment rate -- ``onset_spike_rule.exempt_spike_fraction`` is that resizing, and
    without it the first recession quarter overshoots the doubled-unemployment target by 12.5 %.
    Returns the cutoff, so ``draws > cutoff`` marks the spiked.
    """
    frac = _osr.spike_fraction(urate_normal, urate_target)
    if _osr.t0_exempt() and shock_type in _osr.SPIKED_SHOCK_TYPES:
        chain = _spike_period0_chain(agent)
        if chain is not None:
            frac = _osr.exempt_spike_fraction(chain, frac)
    return 1.0 - frac


def _spike_period0_chain(agent):
    """The micro chain the FIRST RECORDED period transitions through, or None if unavailable.

    The spike happens before the simulation loop, which sets the macro state from
    ``EconomyMrkvNow_hist``; period 0's chain is that history's first entry. Returning None makes
    the caller fall back to the published fraction rather than guess.
    """
    try:
        hist = getattr(agent, 'EconomyMrkvNow_hist', None)
        arrays = getattr(agent, 'CondMrkvArrays', None)
        if hist is None or arrays is None or len(hist) == 0:
            return None
        return np.asarray(arrays[int(hist[0])], dtype=np.float64)
    except Exception:
        return None
...
class AggFiscalType(AggIndMrkvConsumerType):
    ...
    def __init__(self,cycles=1,time_flow=True,**kwds):
        ...
        if hasattr(self, 'num_base_MrkvStates'):
            # HARK's hierarchical decodes (macro_from_combined = // num_micro_states) see the WHOLE micro
            # block of a macro state: 2J under the earnings phase, J otherwise. num_base_MrkvStates stays J.
            self.num_micro_states = _ep.j_full(self.num_base_MrkvStates)
        ...
        if _ep.enabled():
            # the per-agent earnings-phase hazard uniform: its own draw each period (CRN-stable, from the
            # agent's RNG; NOT part of the employment quota machinery), recorded in shock_history so the
            # experiment replays (hit_with_recession_shock) use the same fixed draws as the burn-in
            self.shock_vars += ['phase_draw']
    ...
    def check_markov_inputs(self):
        """HAFiscal's per-state shape tripwire (HAFISCAL_CHECK_MARKOV_INPUTS=1; earnings-phase bring-up
        2026-08-28). HARK's own check assumes a time-varying Rfree, which HAFiscal does not use, so it
        cannot run here. Every MrkvArray[t] must be StateCount x StateCount and every IncShkDstn[t] list
        StateCount long (a `[:J]`-sized list under the earnings phase would otherwise be indexed past its
        end or, worse, silently solve the growing block only); the per-state arrays Rfree / LivPrb[t] /
        PermGroFac[t] must be at least StateCount long (the estimation surface deliberately carries
        arrays longer than its base chain -- first-StateCount sliced by the solver -- so LONGER is allowed,
        SHORTER never is)."""
        S = int(np.asarray(self.MrkvArray[0]).shape[0])
        for t, M in enumerate(self.MrkvArray):
            if np.asarray(M).shape != (S, S):
                raise ValueError(f"check_markov_inputs: MrkvArray[{t}] has shape {np.asarray(M).shape}, expected ({S}, {S})")
        # The composite layout Mrkv = J_full*macro + J*phase + emp must hold with THIS agent's own counts:
        # S == n_phases * num_base_MrkvStates * n_macro. The estimation surface used to carry the production
        # count (7) on a 4-state chain -- invisible pre-phase, a NaN-controls crash under the phase (2026-08-29).
        J = int(getattr(self, "num_base_MrkvStates", 0) or 0)
        n_macro = int(np.asarray(getattr(self, "MacroMrkvArray", np.array([[1.0]]))).shape[0])
        if J and S != _ep.n_states(J, n_macro):
            raise ValueError(f"check_markov_inputs: StateCount {S} != n_phases*J*n_macro = {_ep.n_phases()}*{J}*{n_macro}: "
                             f"num_base_MrkvStates does not describe this agent's chain")
        if np.asarray(self.Rfree).size < S:
            raise ValueError(f"check_markov_inputs: Rfree has {np.asarray(self.Rfree).size} entries < StateCount {S}")
        for name in ("LivPrb", "PermGroFac"):
            for t, arr in enumerate(getattr(self, name)):
                if np.asarray(arr).size < S:
                    raise ValueError(f"check_markov_inputs: {name}[{t}] has {np.asarray(arr).size} entries < StateCount {S}")
        for t, lst in enumerate(self.IncShkDstn):
            if not isinstance(lst, (list, tuple)) or len(lst) != S:
                raise ValueError(f"check_markov_inputs: IncShkDstn[{t}] has {len(lst) if isinstance(lst, (list, tuple)) else 'no'} "
                                 f"per-state entries, expected StateCount {S}")
    ...
    def pre_solve(self):
        ...
        if os.environ.get("HAFISCAL_CHECK_MARKOV_INPUTS", "").strip().lower() in ("1", "on", "true", "yes"):
            self.check_markov_inputs()   # the earnings-phase tripwire: every per-state object sized to StateCount
    ...
    def initialize_sim(self):
        ...
        * **Markov / employment (`shocks['Mrkv']`):** unless ``use_prestate`` /
        ...
        self.MacroMrkvNow = self._macro_of(self.shocks['Mrkv'])
    ...
    def make_idiosyncratic_shock_histories(self):     
        ...
        if _ep.enabled():
            self.phase_draw_fixed_hist = self.shock_history['phase_draw'].copy()
        self.Mrkv_univ = None
    ...
    def hit_with_recession_shock(self, shock_type):
        ...
        if _os_diag.environ.get('HAFISCAL_PHASER_DUMP', '') == '1':
            ...
            _macro = _ep.macro_of(np.asarray(self.shocks['Mrkv']).astype(int), _J)
        ...
        # BUG-122 Step 0: the cutoff is resized under the t = 0 exemption so the
        # unemployment rate is the same either way (see _spike_draw_threshold).
        draws_empy2umemp = draws > _spike_draw_threshold(self, shock_type, old_Urate, this_Urate)
        if _ep.enabled():
            # Earnings phase: the spike lays off EMPLOYED households of BOTH phases (emp == 0, state 0 or J);
            # +1 moves emp 0 -> 1 inside the household's own (macro, phase) block.
            _spiked = np.logical_and(np.equal(_ep.emp_of(self.shocks['Mrkv'], self.num_base_MrkvStates), 0), draws_empy2umemp)
            MrkvNew[_spiked] = MrkvNew[_spiked] + 1
            self._onset_spiked = _spiked.copy()   # BUG-122 Step 0
        else:
            _spiked_plain = np.logical_and(np.equal(self.shocks['Mrkv'], 0), draws_empy2umemp)
            MrkvNew[_spiked_plain] = 1 # Move people from employment to unemployment such that total unemployment rate is as required. Don't touch already unemployed people.
            self._onset_spiked = _spiked_plain.copy()   # BUG-122 Step 0

        # macro-block shifts are by the FULL micro block (2J under the earnings phase; J otherwise)
        if shock_type=="base":
            ...
            MrkvNew += 3*_ep.j_full(self.num_base_MrkvStates) # then put everyone into the recession ???????????????????????
        elif shock_type=="UI" or shock_type=="TaxCut" or shock_type=="Check":
            MrkvNew += 2*_ep.j_full(self.num_base_MrkvStates) # then put everyone into first experiment mrkv state ???????????????????????
        ...
        self.MicroMrkvNow = self.shocks['Mrkv'] % self.num_base_MrkvStates
        self.MacroMrkvNow = self._macro_of(self.shocks['Mrkv'])
        phase_now = self._phase_of(self.shocks['Mrkv'])   # earnings phase rides in Mrkv (all 0 when off)
        MicroMrkvNow_start = copy(self.MicroMrkvNow)
        MacroMrkvNow_start = copy(self.MacroMrkvNow)
        phase_start = copy(phase_now)
        for t in range(self.T_sim):
            ...
            phase_now = self._advance_phase(phase_now, t)
            # BUG-122 Step 0 (flag, default OFF): keep the onset-spiked in their FIRST
            # benefit state at t = 0 so the state label records what they actually drew.
            # See _spike_t0_exempt() for why the cap depends on it.
            if (t == 0 and _spike_t0_exempt()
                    and getattr(self, '_onset_spiked', None) is not None
                    and shock_type not in ('base', 'UI', 'TaxCut', 'Check')
                    and np.size(self._onset_spiked) == np.size(self.MicroMrkvNow)):
                self.MicroMrkvNow = np.where(self._onset_spiked, MicroMrkvNow_start, self.MicroMrkvNow)
            MrkvNow = self._compose(phase_now, self.MacroMrkvNow, self.MicroMrkvNow)
        ...
        self.shocks['Mrkv'] = self._compose(phase_start, self.MacroMrkvNow, self.MicroMrkvNow)
        ...
        if shock_type=="recessionTaxCut" or shock_type=="TaxCut":
            # macro blocks 2..2*TaxCutPeriods+1 = periods 1..TaxCutPeriods (BUG-091 SST;
            # == the former hard-coded 9*2*nb at TaxCutPeriods=8; the SST sizes the blocks by the
            # full micro block, 2J under the earnings phase)
            _tc_lo, _tc_hi = taxcut_block_range(self.TaxCutPeriods, self.num_base_MrkvStates)
            tax_cut_states = np.logical_and(np.greater(self.shock_history['Mrkv'], _tc_lo-1), np.less(self.shock_history['Mrkv'], _tc_hi))
        ...
        # Earnings phase: the fixed history was drawn in state 0 (growing employed), so it carries
        # psi * G_emp; a MATURED household's permanent income grows at MATURED_GROWTH (= 1) instead,
        # so its applied shock is psi * MATURED_GROWTH (the stimulus check's normalization below
        # divides by the APPLIED shock). Phase off: the fixed history itself (same object).
        _perm_applied = self._phase_adjusted_perm_shocks(self.shock_history['Mrkv'])
        self.shock_history['PermShk'][employed] = _perm_applied[employed]
        self.shock_history['TranShk'][employed] = self.tran_shock_fixed_hist[employed]*tax_cut_multiplier[employed] + CheckAmount[employed] / _perm_applied[employed]
        ...
        if getattr(self, 'perm_shocks_during_unemployment', False):
            self.shock_history['PermShk'][unemp_without_benefits] = _perm_applied[unemp_without_benefits]
            self.shock_history['PermShk'][unemp_with_benefits] = _perm_applied[unemp_with_benefits]
        elif getattr(self, 'unemp_pLvl_grows_like_employed', False):
            G_uniform = self._phase_adjusted_uniform_growth(self.shock_history['Mrkv'])   # scalar G_emp when the phase is off
            self.shock_history['PermShk'][unemp_without_benefits] = G_uniform[unemp_without_benefits] if np.ndim(G_uniform) else G_uniform
            self.shock_history['PermShk'][unemp_with_benefits] = G_uniform[unemp_with_benefits] if np.ndim(G_uniform) else G_uniform
        else:
        ...
        # UI-extension states (calendar / bug_fix encodings): the lines above classified every
        # unemployed state other than noBen as unemp_with_benefits, so the extension states
        # u_{UBspell_normal+1}.. were assigned IncUnemp. They pay it only in the recessionUI
        # scenario and only where the rule SST (Code/HA-Models/ui_extension_rule.py -- the SAME
        # rule Simulate.py and welfare6_scenario.py bake into IncShkDstn_recessionUI) says the
        # extension is in force; everywhere else they pay IncUnempNoBenefits. Under legacy there
        # are no extension states (n_ext == 0) and nothing happens here.
        try:
            from EstimParameters import HAFISCAL_UI_STATE_ENCODING, UI_EXT_WINDOW
        except ImportError:
            HAFISCAL_UI_STATE_ENCODING, UI_EXT_WINDOW = 'legacy', None
        J = self.num_base_MrkvStates
        ub_normal = self.UBspell_normal
        n_ext = J - 2 - ub_normal
        if n_ext > 0:
            from ui_extension_rule import extension_pay_mask
            micro = self.shock_history['Mrkv'] % J
            macro = _ep.macro_of(self.shock_history['Mrkv'], J)   # // 2J under the earnings phase
            extension_states = (micro >= ub_normal + 1) & (micro <= ub_normal + n_ext)
            if shock_type in ('recessionUI', 'UI'):
                paid = extension_pay_mask(HAFISCAL_UI_STATE_ENCODING, macro, micro,
                                          ub_normal, n_ext, UI_EXT_WINDOW)
                extension_inactive = extension_states & ~paid
            else:
                # All other scenarios: the extension states never pay the extension.
                extension_inactive = extension_states
    ...
    def _hit_with_recession_shock_shuffled(self, shock_type):
        ...
        MrkvNew = self.shocks['Mrkv']
        old_Urate = self.Urate_normal
        # BUG-122 Step 0: resized under the t = 0 exemption -- see _spike_draw_threshold
        draws_empy2umemp = draws > _spike_draw_threshold(self, shock_type, old_Urate, this_Urate)
        if _ep.enabled():
            # Earnings phase: lay off employed households of BOTH phases (emp == 0); +1 = emp 0 -> 1 in place
            _spiked = np.logical_and(np.equal(_ep.emp_of(self.shocks['Mrkv'], J), 0), draws_empy2umemp)
            MrkvNew[_spiked] = MrkvNew[_spiked] + 1
            self._onset_spiked = _spiked.copy()   # BUG-122 Step 0
        else:
            _spiked_plain = np.logical_and(np.equal(self.shocks['Mrkv'], 0), draws_empy2umemp)
            MrkvNew[_spiked_plain] = 1
            self._onset_spiked = _spiked_plain.copy()   # BUG-122 Step 0

        J_full = _ep.j_full(J)   # the whole micro block of a macro state (2J under the earnings phase)
        if shock_type == "base":
            MrkvNew = MrkvNew
        ...
        if _os_diag.environ.get('HAFISCAL_PHASER_DUMP', '') == '1':
            ...
            _macro = _ep.macro_of(np.asarray(self.shocks['Mrkv']).astype(int), J)
            _agent_seed = getattr(self, 'seed', 'NA')
        ...
        for t in range(self.T_sim):
            ...
            if _mrkv_mode == 'iid':
                ...
                new_micro = np.zeros(self.AgentCount, dtype=int)
                MicroMrkvPrev = self.MicroMrkvNow.copy()
                # Econ-1 (2026-08-28): HAFISCAL_SHUFFLE_MRKV_STRATA="p:5[,a:4]" additionally stratifies the exact quotas
                # within each source state by the households' CURRENT-period permanent-income (and wealth) quantiles
                # (recomputed each period from state_now; both this and initial-income strata are unbiased), so the
                # composition of the movers (the recession's job losers, who carry the UI cells) is fixed per period;
                # each household's marginal transition probability is unchanged (rank within stratum is uniform).
                _strata_spec = _os_mt.environ.get('HAFISCAL_SHUFFLE_MRKV_STRATA', '').strip()
                _mrkv_rounding = _os_mt.environ.get('HAFISCAL_SHUFFLE_MRKV_ROUNDING', 'hamilton').strip().lower()
                if _mrkv_rounding not in ('hamilton', 'madow'):
                    raise ValueError(f"HAFISCAL_SHUFFLE_MRKV_ROUNDING={_mrkv_rounding!r}: use hamilton (default) or madow")
                for jj in range(J):
                    these_j = (MicroMrkvPrev == jj)
                    N_j = int(these_j.sum())
                    if N_j == 0:
                    ...
                    _strata = _mrkv_strata_ids(_strata_spec, self.state_now['pLvl'], self.state_now['aNrm'], agents_j) \
                        if _strata_spec else None
                    # the Madow rounding uniforms: CRN-stable per (period, source state), independent of the rank draws.
                    # Always with strata (small strata need it); on the plain path opt-in via HAFISCAL_SHUFFLE_MRKV_ROUNDING=madow
                    # (default 'hamilton' = the certified engine's floor + largest-residual rule, byte-for-byte).
                    _rng_round = np.random.default_rng([int(self._shuffle_base_seed) + t, jj]) \
                        if (_strata is not None or _mrkv_rounding == 'madow') else None
                    new_micro[agents_j] = _stratified_quota_assign(agents_j, draws_j, cond_mrkv[jj], _strata, _rng_round)
            else:
            ...
            # the earnings-phase hazard is NOT a quota'd transition: its own per-agent draw (fixed history)
            phase_now = self._advance_phase(phase_now, t)
            # BUG-122 Step 0 (flag, default OFF): keep the onset-spiked in their FIRST
            # benefit state at t = 0 so the state label records what they actually drew.
            # See _spike_t0_exempt() for why the cap depends on it.
        ...
        if shock_type in ("recessionTaxCut", "TaxCut"):
            _tc_lo, _tc_hi = taxcut_block_range(self.TaxCutPeriods, J)   # BUG-091 SST (== 9*2*J at 8); full micro block
            tax_cut_states = np.logical_and(
                np.greater(self.shock_history['Mrkv'], _tc_lo - 1),
                np.less(self.shock_history['Mrkv'], _tc_hi))
            tax_cut_multiplier[tax_cut_states] *= self.TaxCutIncFactor
        ...
        # earnings phase: matured households' applied shock is psi * MATURED_GROWTH (see the twin above)
        _perm_applied = self._phase_adjusted_perm_shocks(self.shock_history['Mrkv'])
        self.shock_history['PermShk'][employed] = _perm_applied[employed]
        self.shock_history['TranShk'][employed] = (
            self.tran_shock_fixed_hist[employed] * tax_cut_multiplier[employed]
            + CheckAmount[employed] / _perm_applied[employed])
    ...
    def switch_shock_type(self, shock_type):
        ...
        if shock_type == "base":
            ...
            # BUG-094 (2026-08-26): was CondMrkvArrays_recession — numerically inert (the solve
            # reads MrkvArray) but the policy-store key hashed the recession leaf, splitting every
            # base-config entry saved after a recession switch from its fresh-agent twin.
            self.CondMrkvArrays = self.CondMrkvArrays_base
        ...
        # Keep MacroMrkvArray in step with the chain just installed (2026-08-29). update_mrkv_array()
        # sets both together, and the stock experiment path calls it right before solve_if_changed();
        # the hoisted cohort-parallel solve (step5a_parallel_solve.py) solves straight after THIS
        # swap, so the earnings-phase tripwire (check_markov_inputs, n_macro from MacroMrkvArray)
        # saw the base [[1.]] against a 588-state chain and halted every appendix-config 5a fork.
        # MacroMrkvArray is not a solver input and not a policy-store key leaf: solutions unchanged.
        if shock_type == "base":
            self.MacroMrkvArray = np.array([[1.0]])
        elif hasattr(self, 'Rspell') and hasattr(self, 'num_experiment_periods'):
    ...
    def get_states(self):
        ...
        # Earnings phase (decision 2026-08-28): the REALIZED phase and employment state, the PERCEIVED macro
        # state -- the idiosyncratic block is the whole 2J micro block, hence the modulus J_full.
        _Jp = _ep.j_full(self.num_base_MrkvStates)
        self.MrkvNowPcvd = np.remainder(self.shocks['Mrkv'],_Jp) + _Jp*np.floor_divide(self.MrkvNowPcvd,_Jp)
    ...
    def get_micro_markov_states(self):
        ...
        if _ep.enabled():
            # the earnings-phase hazard uniform: drawn AFTER the employment draw so the employment RNG
            # stream is untouched by the phase; never drawn when the phase is off (RNG byte-identity)
            self.shocks['phase_draw'] = Uniform(seed=self.RNG.integers(2**31-1)).draw(self.AgentCount)

    # ----------------------------------------------------------------- earnings-phase helpers
    def _macro_of(self, mrkv):
        """Macro block of a composite state (// 2J under the earnings phase, // J otherwise)."""
        return _ep.macro_of(mrkv, self.num_base_MrkvStates)

    def _phase_of(self, mrkv):
        """Earnings phase of a composite state (all zeros when the phase is off)."""
        return _ep.phase_of(mrkv, self.num_base_MrkvStates)

    def _compose(self, phase, macro, micro):
        """Mrkv = J_full*macro + J*phase + emp (== J*macro + emp when the phase is off)."""
        return _ep.compose(phase, macro, micro, self.num_base_MrkvStates)

    def _advance_phase(self, phase_prev, t=None):
        """One period of the growing -> matured hazard: a growing household matures when its own uniform
        draw (the fixed history at period t, or this period's live draw) falls below h; matured is
        absorbing; newborns (t_age == 0) are born growing. Identity when the phase is off."""
        if not _ep.enabled():
            return phase_prev
        u = self.phase_draw_fixed_hist[t] if t is not None else self.shocks['phase_draw']
        phase = np.where(np.logical_or(np.asarray(phase_prev) == 1, u < _ep.hazard()), 1, 0)
        phase[self.t_age == 0] = 0
        return phase

    def _phase_adjusted_perm_shocks(self, Mrkv_hist):
        """The permanent shock APPLIED on the experiment-replay path: psi x PermGroFac[realized state].

        The fixed history (`perm_shock_fixed_hist`) was drawn with every household forced into state 0
        (growing employed), so every entry is psi x G_emp. In HARK's convention PermShk = psi x G and
        pLvl_{t+1} = pLvl_t x PermShk, so a household whose REPLAYED state has a different growth factor
        -- unemployed (PermGroFac_unemp = 1, the paper's convention) or matured (MATURED_GROWTH = 1) --
        must get psi x its own state's growth: that is what the solver and the transition matrix assume
        (per-state PermGroFac; belief consistency). BUG-102 (2026-08-29, found by the earnings-phase
        bring-up probe `probe_unemp_growth_mc.py`): until this fix the unemployed kept psi x G_emp on
        this path under perm_shocks_during_unemployment=True (the default world), an excess drift
        u x log G ~ +0.8 % of E[p] over a 40-quarter experiment; the live (burn-in) path was already
        per-state. HAFISCAL_REPLAY_PERSTATE_GROWTH=0 reproduces the pre-fix behaviour (the phase-only
        correction, or the bare fixed history when the phase is off)."""
        G_emp = float(self.PermGroFac[0][0])
        if os.environ.get('HAFISCAL_REPLAY_PERSTATE_GROWTH', '1').strip().lower() in ('0', 'off', 'false'):
            if not _ep.enabled():
                return self.perm_shock_fixed_hist
            out = self.perm_shock_fixed_hist.copy()
            matured = self._phase_of(Mrkv_hist) == 1
            out[matured] = out[matured] / G_emp * _ep.MATURED_GROWTH
            return out
        pgf = np.asarray(self.PermGroFac[0], dtype=float)
        growth = pgf[np.asarray(Mrkv_hist, dtype=int)]          # the replayed state's own growth factor
        return self.perm_shock_fixed_hist / G_emp * growth

    def _phase_adjusted_uniform_growth(self, Mrkv_hist):
        """G_emp for growing households, MATURED_GROWTH for matured ones (the unemp_pLvl_grows_like_employed
        convention); the scalar G_emp when the phase is off."""
        G_uniform = float(self.PermGroFac[0][0])
        if not _ep.enabled():
            return G_uniform
        return np.where(self._phase_of(Mrkv_hist) == 1, _ep.MATURED_GROWTH, G_uniform)

    def get_markov_states(self):
        phase_prev = self._phase_of(self.shocks['Mrkv'])   # the phase rides in the composite state
        self.get_macro_markov_states()
        self.get_micro_markov_states()
        MrkvNow = self._compose(self._advance_phase(phase_prev), self.MacroMrkvNow, self.MicroMrkvNow)
...
if _HAS_DUAL_MEASURE:
    class DualAggFiscalType(DualMeasureMixin, AggFiscalType):
        ...
        def hit_with_recession_shock(self, shock_type):
            ...
            if shock_type in ("recessionTaxCut", "TaxCut"):
                # BUG-091 SST window (== the former 9*2*J at TaxCutPeriods=8); full micro block under the phase
                _tc_lo, _tc_hi = taxcut_block_range(self.TaxCutPeriods, self.num_base_MrkvStates)
                tc_states = np.logical_and(
                    self.shock_history['Mrkv'] > _tc_lo - 1,
                    self.shock_history['Mrkv'] < _tc_hi,
                )
            ...
            if self.dual_measure:
                ...
                if mrkv_univ is not None:
                    ...
                    for (t, j), draws in base_dict.items():
                        # The merged dual_measure cache (post-#1814) also stores
                        # ("newborn", j) entries for the Q newborn redraw; they are
                        # not (t_cycle, state) cells, and newborns are NaN'd out of
                        # the flat record below regardless.
        ...
        def _draw_Q_shocks(self):
            ...
            use_full_state = n_Q_states > _ep.j_full(J_base)   # the base micro block is 2J under the phase
            ...
            for t in range(self.T_cycle):
                ...
                for j in unique_j:
                    ...
                    N = np.sum(these)
                    if N > 0:
                        j_q = int(j) if use_full_state else int(j % _ep.j_full(J_base))
                        j_pgf = int(j % len(self.PermGroFac[t - 1]))   # composite index into the tiled per-state growth
...
def compute_pf_decay_limits(MrkvArray, Rfree, PermGroFac, IncShkDstn, Cgrid,
    ...
    # macro block of state j: // 2J under the earnings phase (a `// J` decode would flag the whole
    # matured block of the base scenario as a recession state and AD-scale its income)
    RecState = np.array([
        (int(_ep.macro_of(j, num_base_MrkvStates)) % 2 == 1)
        for j in range(StateCount)
    ])
...
if os.environ.get('HAFISCAL_SOLVE_ACCEL', '').strip().lower() in (
        'newton2d', 'anderson2d'):
...
def _solved_discfac_of(agent):
    """The scalar DiscFac an agent currently carries (HARK may store it as a list)."""
    return float(np.asarray(agent.DiscFac).reshape(-1)[0])


def stamp_solved_discfac(agent):
    """BUG-080: record the DiscFac under which ``agent.solution`` was just produced.

    Read by :func:`warm_start_seed`. Every solve site in
    ``AggregateDemandEconomy.solve`` calls this after ``post_solve()`` so a later
    in-place reassignment of ``DiscFac`` can be recognised as a change of problem.
    """
    agent._solved_DiscFac = _solved_discfac_of(agent)


BUG080_DECLINED = {'count': 0}   # diagnostic counter: warm seeds declined by the BUG-080 provenance check


def warm_start_seed(agent, warm_start=True):
    """The ONE warm-start gate for the economy solve (BUG-080 fix, 2026-09-09).

    Returns the previous converged solution to seed the backward iteration
    from, or ``None`` for a cold start from the constrained-PF terminal.

    Two conditions, both required:
      1. the stored solution has the agent's CURRENT Markov state count (the
         historical gate: a 6-state base policy cannot seed a 132/252-state
         recession solve);
      2. the stored solution was produced under a weakly MORE patient
         ``DiscFac`` than the agent now carries (``_solved_DiscFac >= DiscFac``).
         By Carroll-Kimball a more patient policy consumes less, so it lies
         at/below the new beta's perfect-foresight line and the BUG-062
         concavity guard's premise (iterates approach from below) holds. A seed
         solved under a LESS patient beta consumes MORE, starts the first sweep
         ABOVE the new line, and the guard -- correctly -- refuses: that is
         BUG-080 (the MC Step-2 objective's in-place ``agent.DiscFac``
         reassignment, and the fit-table pass that writes the estimated atoms
         into agents built at the module-level atoms). Declining the seed keeps
         the guard exact and the in-place speed-up for every
         impatience-increasing move.
    A solution with no stamp (set by a cache restore or by code that predates
    the stamp) keeps the historical warm behaviour, so nothing that never
    changes ``DiscFac`` -- Step 5a, the welfare battery, the AD loop -- is
    touched; the stamp is written by every solve site, so from the first solve
    of a process on the check is live.
    """
    if not (warm_start and hasattr(agent, 'solution') and len(agent.solution) > 0):
        return None
    ...
    if prev_states != current_states:
        return None
    solved_beta = getattr(agent, '_solved_DiscFac', None)
    if solved_beta is not None:
        beta_now = _solved_discfac_of(agent)
        if solved_beta < beta_now - 1e-12:
            BUG080_DECLINED['count'] += 1
            if os.environ.get('HAFISCAL_SOLVE_WALL_VERBOSE', '') == '1':
                print(f"[warm-start] BUG-080: DiscFac raised in place {solved_beta:.6f} -> "
                      f"{beta_now:.6f}; seed declined, cold start from the constrained-PF terminal",
                      flush=True)
            return None
    return prev_sol
...
def maybe_accel_solution(agent, from_solution=None):
    ...
    _census = os.environ.get('HAFISCAL_SOLVE_ACCEL_CENSUS', '') == '1'
    if _census:
        import traceback as _tb
        _caller = next((f"{fr.name}" for fr in reversed(_tb.extract_stack())
                        if fr.name not in ("maybe_accel_solution", "solve",
                                           "<module>")), "?")
        print(f"[accel-census] caller={_caller} "
              f"states={getattr(agent, 'MrkvArray', [np.empty((0, 0))])[0].shape[0]} "
              f"cycles={getattr(agent, 'cycles', '?')} method={method}",
              flush=True)
    if not method:
        return None
    try:
        ...
        if getattr(agent, 'cycles', None) != 0:
            return None
        if method in ('newton2d', 'anderson2d'):
            from solver_accel import newton2d_solve_agent
            solution, info = newton2d_solve_agent(
                agent, engine=('anderson' if method == 'anderson2d'
                               else 'newton'))
        ...
        if _census or os.environ.get('HAFISCAL_SOLVE_ACCEL_VERBOSE', '') == '1':
            print(f"[accel-warm] method={method} "
                  f"warm_started={info.get('warm_started')} "
                  f"iters={info.get('steps', info.get('iters'))} "
                  f"stop={info.get('stop')}", flush=True)
...
def solve_agg_cons_markov_alt(solution_next,IncShkDstn,LivPrb,DiscFac,CRRA,Rfree,PermGroFac,
    ...
    _pf_local2 = False
    # NOTE 2026-08-21: 'farfield' (the Step-1 far-field exponent mode) keeps THIS path's
    # certified measured-Q attach unchanged — S2 adoption of the far-field estimator is a
    # separate decision; listing it here only prevents an accidental legacy fallback.
    if _pf_decay and os.environ.get('HAFISCAL_PF_DECAY_Q', 'measured').strip().lower() in ('local2', 'measured', 'farfield'):
    ...
    for j in range(StateCount):
            ...
            AggState = _ep.macro_of(j, num_base_MrkvStates)   # // 2J under the earnings phase
...
class AggregateDemandEconomy(Market):
    ...
    def mill_rule(self, cLvl_splurge):
        ...
        if hasattr(self,'base_AggCons'):
            ...
            _apply_mr = _agg_mode_mr in ('on', '1', 'true') or (_agg_mode_mr == 'auto' and _has_co_mr)
            _wts_mr = [getattr(a, 'agent_weights', None) for a in self.agents]
            if any(w is not None for w in _wts_mr):
                # BUG-092 weighted-tail sampler: per-household weights (mean 1) x per-type factor
                AggCons = sum(
                    float(np.dot(np.asarray(this_cLvl, dtype=np.float64),
                                 (np.asarray(w, dtype=np.float64) if w is not None
                                  else np.ones(len(this_cLvl)))))
                    * (getattr(a, 'pop_rescale_factor', 1.0) if _apply_mr else 1.0)
                    for this_cLvl, a, w in zip(cLvl_splurge, self.agents, _wts_mr)
                )
            elif _apply_mr:
                AggCons = sum(
            ...
            _Jb = _ep.j_full(self.num_base_MrkvStates)   # first state of a macro block (2J apart under the phase)
            CratioNext = self.CFunc[EconomyMrkvNow*_Jb][EconomyMrkvNext*_Jb](self.Cratio)
        else:
            #self.CratioNow = 1.0
    ...
    def run_experiment(self, shock_type = "recession", UpdatePrb = 1.0, Splurge = 0.0, EconomyMrkv_init = [0], Full_Output = True):
        """Run one experiment and return the requested view of it.

        `Full_Output` selects WHAT IS PACKED, not what is computed: the simulation runs and the
        panels are assembled from the agents' stored history before the choice is made. The two
        historical views overlap but neither contains the other -- `True` carries the normalized
        consumption, market-resources and aggregate series, `'ForWelfare'` carries the permanent-
        shock and mortality panels instead.

        `'both'` (added 2026-09-06) returns the pair `(for_welfare, full)` from ONE simulation.
        Simulate.py used to ask for the two views with two consecutive `run_experiment` calls on
        the same baseline, i.e. it simulated an entire baseline a second time to repack arrays the
        process already held -- about two minutes at production scale, on every MC run.
        """
        ...
        self.sow_init['CratioNow'] = self.CFunc[0][EconomyMrkv_init[0]*_ep.j_full(self.num_base_MrkvStates)].intercept
        ...
        # BUG-092 weighted-tail sampler (opt-in): per-household population weights set at the
        # seed (agent.agent_weights, mean 1) multiply the per-type factors.
        _per_agent_w = [getattr(ThisType, 'agent_weights', None) for ThisType in self.agents]
        if any(w is not None for w in _per_agent_w):
            agent_weights = agent_weights * np.concatenate([
                (np.asarray(w, dtype=np.float64) if w is not None
                 else np.ones(ThisType.AgentCount))
                for w, ThisType in zip(_per_agent_w, self.agents)])
        ...
        welfare_dict = return_dict = None
        if Full_Output==True or Full_Output=='both':
        ...
        if Full_Output=='ForWelfare' or Full_Output=='both':
            ...
            welfare_dict = {'cLvl_all_splurge' : cLvl_all_splurge,
                           'pLvl_all' :        pLvl_all,
                           'aNrm_all' :        aNrm_all,
                           'TranShk_all' :     TranShk_all,
        ...
        # one simulation, one or both views; each gets its own copy of the shared Q entries below
        _views = _run_experiment_views(Full_Output, welfare_dict, return_dict)

        if has_Q:
            NPV_AggCons_Q = calculate_NPV(AggCons_Q, self.act_T, ThisType.Rfree[0])
            for _d in _views:
                _d['AggCons_Q'] = AggCons_Q
                _d['NPV_AggCons_Q'] = NPV_AggCons_Q

        return _views if Full_Output == 'both' else _views[0]
    ...
    def solve(self, warm_start=True, only_agents=None):
        ...
        only_agents (speedup remedy C, owner-ordered 2026-08-20): when given, ONLY
        that subset is (re-)solved -- the Step-2 objective swaps one education
        group's 7 types and the other 14 already hold their converged solutions, so
        re-walking them (even warm, ~1 sweep each) is pure waste. Subset mode uses
        the plain EGM ladder below (the JAX-2B branches iterate self.agents and are
        not consulted); None = the historical full-list behavior, byte-identical.
        ...
        if (use_2b or use_2b_vmap) and only_agents is None:
            ...
            # the BUG-080 provenance check lives in warm_start_seed.
            from_solutions = [warm_start_seed(agent, warm_start) for agent in self.agents]
        ...
        # Shared solved-policy store (plans_local/20260824-1530h): a cold AD-off
        # solve loads a previously solved policy for the SAME problem under the
        # SAME solver (one-sweep fixed-point guard on every hit) and publishes
        # what it solves. Off by default in phase 1 (HAFISCAL_POLICY_STORE=1);
        # with the flag unset this loop is byte-identical to before, plus the
        # solve-wall accounting line (P0 of the plan).
        try:
            from solution_cache import policy_store as _ps
            _ps_on = _ps.enabled()
        except Exception:
            _ps, _ps_on = None, False
        _acct = []
        for agent in (self.agents if only_agents is None else only_agents):
            from_solution = warm_start_seed(agent, warm_start)   # state-count + BUG-080 provenance gate
            agent.pre_solve()
            _t0 = _time_module.time()
            _engine = None
            _store_eligible = _ps_on and _ps.is_cold_ad_off(agent, from_solution)
            if _store_eligible and _ps.try_load(agent, self) is not None:
                _engine = 'store'
            elif _store_eligible and _ps.require_hits():
                _ps.raise_on_miss(agent, 'economy.solve')   # strict mode (BUG-091)
            elif _step2_namg and self._try_solve_namg_base(agent, warm_start):
                _engine = 'namg'
            elif _step5_ati and self._try_solve_ati_markov(agent, from_solution):
                _engine = 'ati'
            else:
                if _accel_method:
            ...
            agent.post_solve()
            stamp_solved_discfac(agent)
            if _store_eligible and _engine != 'store':
                _ps.save(agent, producer={'engine': _engine, 'site': 'economy.solve'})
            _acct.append((_engine, _time_module.time() - _t0,
                          float(np.asarray(agent.DiscFac).reshape(-1)[0]),
                          int(np.asarray(agent.MrkvArray[0]).shape[0])))
        self._report_solve_wall(_acct)

    @staticmethod
    def _report_solve_wall(acct):
        """P0 of the policy-store plan: one summary line per economy solve (and
        per-agent lines under HAFISCAL_SOLVE_WALL_VERBOSE=1) so the cold-solve
        share of a run can be read off its log; the totals also land in the
        provenance REUSE ledger (kind 'solve_wall')."""
        if not acct:
            return
        by = {}
        for eng, wall, _b, _s in acct:
            n, w = by.get(eng, (0, 0.0))
            by[eng] = (n + 1, w + wall)
        total = sum(w for _e, w, _b, _s in acct)
        S = acct[0][3]
        parts = " ".join(f"{eng}={n}({w:.0f}s)" for eng, (n, w) in sorted(by.items()))
        print(f"[solve-wall] S={S} agents={len(acct)} total={total:.0f}s {parts}",
              flush=True)
        if os.environ.get('HAFISCAL_SOLVE_WALL_VERBOSE', '') == '1':
            for eng, wall, b, s in acct:
                print(f"[solve-wall]   beta={b:.6f} S={s} {eng} {wall:.1f}s", flush=True)
        try:
            from solution_cache import record_reuse_event as _rre
            _rre('solve_wall', 'measured', 'none', S=S, agents=len(acct),
                 total_s=round(total, 2),
                 by_engine={k: [n, round(w, 2)] for k, (n, w) in by.items()})
        except Exception:
            pass
    ...
    def _solve_accel_method():
        ...
        if _v in ('aitken', 'anderson', 'newton2d', 'anderson2d'):
            return _v
        if _v not in ('', '0', 'off', 'plain'):
            warnings.warn(f"HAFISCAL_SOLVE_ACCEL={_v!r} not recognized "
                          "(use aitken|anderson|newton2d|anderson2d|off); "
                          "running plain EGM.")
        return None
    ...
    def _try_solve_accel(self, agent, from_solution, method):
        ...
        try:
            ...
            if method in ('newton2d', 'anderson2d'):
                ...
                solution, info = accel_solve_agent(
                    agent, from_solution=from_solution, method=method)
            if (os.environ.get('HAFISCAL_SOLVE_ACCEL_CENSUS', '') == '1'
                    or os.environ.get('HAFISCAL_SOLVE_ACCEL_VERBOSE', '') == '1'):
                print(f"[accel-warm] method={method} "
                      f"warm_started={info.get('warm_started')} "
                      f"iters={info.get('steps', info.get('iters'))} "
                      f"stop={info.get('stop')}", flush=True)
    ...
    def _try_solve_namg_base(self, agent, warm_start):
        ...
        try:
            ...
            c_init = None
            prev = warm_start_seed(agent, warm_start)   # BUG-080 gate: None when the seed was solved under a less patient beta
            if prev is not None:
    ...
    @staticmethod
    def _ati_pf_line_violation(sol, info, rel_tol=1e-9):
        """BUG-088 acceptance test for a converged ConsumedATI-Markov solution.

        Returns None when the policy is admissible, else a short reason string.
        Two checks, both consequences of Carroll-Kimball concavity (the
        stationary policy lies strictly BELOW the PF line MPCmin*(m+h_s) at
        every finite m, and its gap to the line decays):

        1. the solver's own tail-validity verdict: ``info['tail_form']`` is
           ``'powerlaw(exp-fallback)'`` / any non-finite ``info['tail_Q'][s]``
           when a state's top knots cannot carry a power-law decay toward the
           line (above it, or slope below MPCmin) — the solver only WARNS;
        2. the top knot of every state's unconstrained slice against that
           state's line, with the BUG-062 guard's tolerance
           ``rel_tol*max(1, |line|)`` so the two gates agree.

        Anything that prevents the check from being evaluated is itself a
        reason to reject (an unverifiable policy must not be installed).
        """
        try:
            tail_form = str(info.get('tail_form', 'powerlaw'))
            if tail_form != 'powerlaw':
                bad = []
                tq = info.get('tail_Q')
                if tq is not None:
                    bad = np.where(~np.isfinite(np.asarray(tq, dtype=float)))[0].tolist()
                return (f"tail_form={tail_form!r} (power-law tail invalid in "
                        f"{len(bad)} state(s){': ' + str(bad[:6]) if bad else ''})")
            MPCmin = float(sol.MPCmin)
            hNrm = np.asarray(sol.hNrm, dtype=float).reshape(-1)
            worst = None
            for s, cf in enumerate(sol.cFunc):
                g = cf
                for _ in range(4):
                    if hasattr(g, 'x_list'):
                        break
                    g = g.functions[0] if hasattr(g, 'functions') else g.func
                x_top = float(np.asarray(g.x_list, dtype=float)[-1])
                c_top = float(np.asarray(g.y_list, dtype=float)[-1])
                line = MPCmin * (x_top + hNrm[s])
                excess = c_top - line
                if excess > rel_tol * max(1.0, abs(line)) and (
                        worst is None or excess / line > worst[1]):
                    worst = (s, excess / line, c_top, line, x_top)
            if worst is not None:
                s, rel, c_top, line, x_top = worst
                return (f"top knot c={c_top:.6g} EXCEEDS the PF line {line:.6g} by "
                        f"{100.0 * rel:.1f}% (state {s}, m_top={x_top:.6g}, "
                        f"h={hNrm[s]:.6g}, MPCmin={MPCmin:.4e})")
            return None
        except Exception as _e:  # noqa: BLE001 — unverifiable => reject
            return f"PF-line check could not be evaluated ({_e!r})"
    ...
    def _try_solve_ati_markov(self, agent, from_solution):
        ...
        * ``BoroCnstArt == 0`` (the solver's shared ``mNrmMin=0`` grid), uniform
        ...
        try:
            ...
            if beta < AggregateDemandEconomy._step5_ati_min_discfac():
                _log(f"skip (patience routing): {_who} < threshold "
                     f"{AggregateDemandEconomy._step5_ati_min_discfac():.4f} -> EGM")
                return False
            if not permgrofac_fix_on():
            ...
            if len(IncShkDstn) != S:
                ...
                return False
            Rfree = AggregateDemandEconomy._per_state_param(agent.Rfree, S)
            PermGroFac = AggregateDemandEconomy._per_state_param(agent.PermGroFac, S)
            LivPrb = AggregateDemandEconomy._per_state_param(agent.LivPrb, S)
            ...
            # BUG-089 (2026-08-24): the ATI's tail-exponent estimator must be the
            # production attach's. Under the measured-Q attach the slope-derived
            # chain-Q overstated the exponent on a short grid (1.47 vs 0.63 at
            # the certified College top atom, K=1 top) — and because the tail
            # feeds back through the near-unit contraction of a very patient
            # type, it lifted that atom's WHOLE policy ~1.2% above the EGM+attach
            # solution (E1-E4 in conclusions_private/2026-08-24_bug088-*.md).
            # The old comment's premise ("differing from the measured-Q attach
            # only above the solve top") was false. So: 'lagged' mode with
            # local_q_tail's two-secant estimator as the per-iterate tail_q_fn
            # (ONE implementation; diag=False keeps the ATI's transient
            # iterates out of the attach's drift advisory). Under the slope
            # form the chain-Q IS the attach's estimator and stays.
            from grid_sizing import powerlaw_measured_active as _pl_measured
            if _pl_measured(os.environ):
                from local_q_tail import measured_q_for_solver as _tail_q_fn
                # 'lagged' converges LINEARLY in its terminal phase, so the
                # solver's default 1e-9 step/EE gates leave ~1e-8 to the limit
                # (measured 1.1e-8 at the HS base atom vs the P4a 1e-8
                # same-fixed-point certificate); one decade tighter costs a few
                # outer iterations and restores machine-class parity.
                _tail_kw = dict(tail_q_mode='lagged', tail_q_fn=_tail_q_fn,
                                tol_delta=1e-10, tol_EE=1e-10)
                _tail_src = 'measured'
            else:
                _tail_kw = dict(tail_q_mode='chain')
                _tail_src = 'chain'
            _t0 = _time_module.time()
            sol, info = solve_stationary_ConsumedATI_markov(
                ...
                # solver defaults), powerlaw tail. maxit=200 is the P4a bench
                # cap (observed 11-22 outer iterations under chain-Q; the
                # measured 'lagged' mode is linearly convergent in its terminal
                # phase, up to ~7x more outer iterations at the GIC edge);
                # hitting it -> clean EGM fallback.
                inner='gmres', maxit=200,
                tail_form='powerlaw', **_tail_kw,
            )
            ...
            # BUG-088 (2026-08-24): a CONVERGED block-Newton fixed point can be
            # SPURIOUS. At a GIC-capped College atom (S=252) ATI reported
            # fnorm 6e-13 on a policy whose top knot sat 34.5% ABOVE the PF
            # line MPCmin*(m+h) (above it from m~37 outward, power-law tail
            # attach invalid -> exponential fallback tail 25x the line by
            # m=1000), while EGM from the constrained-PF terminal converges
            # 9.4% BELOW the line. Installing that policy is what tripped the
            # BUG-062 guard at the AD-phase EGM warm start. By Carroll-Kimball
            # concavity a correct stationary policy is below the line at every
            # knot, so an above-line top knot (or a tail the solver itself
            # flagged invalid) is an in-family NON-CONVERGENCE event: fall back
            # to EGM exactly like the line-search branch above. (This is not
            # the availability fallback the 2026-08-22 ruling forbids.)
            _viol = AggregateDemandEconomy._ati_pf_line_violation(sol, info)
            if _viol is not None:
                _log(f"FALLBACK (spurious fixed point, BUG-088): {_who} {_viol} "
                     f"fnorm={info.get('fnorm', float('nan')):.2e} "
                     f"wall={_wall:.2f}s -> EGM")
                return False
            ...
            agent._step5_ati_info = dict(
                ...
                # BUG-089: the attached per-state exponent + its source, so a
                # same-fixed-point check can pin its Picard reference at the
                # exponent this solve actually used.
                tail_Q=(None if info.get('tail_Q') is None
                        else np.asarray(info['tail_Q'], dtype=float)),
                tail_q_source=_tail_src)
            _log(f"ROUTED: {_who} iters={info['iters']} "
                 ...
                 f"tail={_tail_src} wall={_wall:.2f}s")
            return True
        except Exception as _e:
            # OWNER RULING 2026-08-22: once ATI routing is attempted, ANY failure
            # is FATAL — never a silent fall-through to EGM (a soft-failed run on
            # one machine diverges invisibly from a routed run on another; the
            # Step-2 smoke ran EGM for exactly this reason when a worktree lacked
            # the FTI repo). The deliberate way to run EGM is the explicit opt-out
            # HAFISCAL_STEP5_ATI=0. (hark_fti's own per-slice
            # fell_back_to_direct remains — that is the solver's certified
            # in-family numerical robustness, not an availability fallback.)
            _log(f"FATAL (routing failure, no EGM fallback): {_e!r}")
            raise RuntimeError(
                "Step-5 ATI routing failed and the EGM soft-fallback is disabled "
                "(owner ruling 2026-08-22). Fix the FTI availability (see "
                "_hark_fti_path.py: $HAFISCAL_FTI_REPO / sibling / canonical "
                "locations) or opt out deliberately with HAFISCAL_STEP5_ATI=0."
            ) from _e

    def _macro_of_state(self, j):
        """Macro block of a combined belief index (// 2J under the earnings phase, // J otherwise)."""
        return _ep.macro_of(j, self.num_base_MrkvStates)
    ...
    def Macro_2_Micro_CFunc(self, MacroCFunc):
        ...
        '''
        dim = len(MacroCFunc)
        _Jb = _ep.j_full(self.num_base_MrkvStates)   # every micro state of a macro block (2J under the phase) shares the block's rule
        MicroCFunc = [[CRule(1.0,0.0) for i in range(dim*_Jb)] for j in range(dim*_Jb)]
        for i in range(dim*_Jb):
            for j in range(dim*_Jb):
                MicroCFunc[i][j] = MacroCFunc[int(np.floor(i/_Jb))][int(np.floor(j/_Jb))]
        return MicroCFunc
    ...
    def Compare_CFunc_Convergence(self,Old_Cfunc,New_Cfunc):
        ...
        FromMrkState_Slopes_Largest_Diff    = int(self._macro_of_state(i))
        ToMrkState_Slopes_Largest_Diff      = int(self._macro_of_state(j))
        
        Intercept_Diff                      = np.linalg.norm(DiffIntercepts)
        [i,j]                               = np.unravel_index(DiffIntercepts.argmax(),DiffIntercepts.shape)
        FromMrkState_Intercept_Largest_Diff = int(self._macro_of_state(i))
        ToMrkState_Intercept_Largest_Diff   = int(self._macro_of_state(j))
    ...
    def solve_ad_recession(self, num_max_iterations, convergence_cutoff=1E-3, name = None, shock_type = "recession"):
        ...
        dim = int(len(self.CFunc)/_ep.j_full(self.num_base_MrkvStates))   # macro blocks of 2J micro states under the phase
...
def try_solve_ati_markov(agent, from_solution):
    """Module-level entry to the ConsumedATI-Markov router for callers that hold an
    AGENT but no economy — the Step-5a fork worker (parallel_solve) and the welfare
    battery's spawn worker (welfare6_scenario). The router uses only static helpers
    of AggregateDemandEconomy (patience threshold, per-state params, the BUG-088
    PF-line gate), so the instance slot is unused. Same contract as the method:
    installs ``agent.solution`` and returns True on a verified ATI solve; False =
    take the EGM rungs (unqualified atom, non-convergence, spurious fixed point).
    Added 2026-08-24 (P5 of the shared-policy-store plan) so BOTH entry points
    solve every household with the same engine and the store's entries are
    identical whichever entry produced them."""
    return AggregateDemandEconomy._try_solve_ati_markov(None, agent, from_solution)


def step5_ati_enabled():
    """Env gate of the ATI router (``HAFISCAL_STEP5_ATI=1``), module-level twin."""
    return AggregateDemandEconomy._step5_ati_enabled()


# Pin the solver-source hash to the module AS IMPORTED (2026-08-28). The policy/equilibrium stores key their entries by a
# hash of a few solver function bodies read through inspect.getsource, i.e. from the file ON DISK at the time of the first
# key computation -- for a multiplier program that is its first SAVE, up to an hour after import. An edit to this file in
# between (even to unrelated code) shifts the line numbers, the hash is computed from the wrong text, and the program's
# equilibria are published under a key no consumer can ever produce (the 2026-08-28 Rfree_1015 appendix arm: four
# equilibria keyed 22424c.. while every clean import hashes d98273..). Computing it here makes the key a property of the
# imported code, not of the disk at save time. Any failure leaves the lazy path in place.
try:
    from solution_cache import policy_store as _ps_pin  # noqa: E402
    _ps_pin.solver_source_hash()
except Exception:  # never let the pin break an import (e.g. a stripped tree without solution_cache)
    pass
