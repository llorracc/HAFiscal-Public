...
def pooled_lorenz_shares(pieces, percentiles=None):
    """Lorenz shares of a distribution POOLED over several (values, weights) pieces.

    The level of aggregation matters and was wrong here until 2026-09-06. Step 2 fits the Lorenz
    curve of an EDUCATION GROUP -- it concatenates the group's discount-factor atoms into one
    weighted array and takes the Lorenz of that (`estim_phase2_tm_a.py`, `a_array =
    np.concatenate(a_vals_list)`). The drift diagnostic borrowed Step 2's PERCENTILES but not its
    population, and compared each atom's own Lorenz curve instead.

    That is not a noisier version of the right test; it is a different quantity. Most of the
    within-group wealth inequality this model produces comes FROM the spread of discount factors
    across atoms, so an atom-by-atom Lorenz curve discards exactly the dimension the estimation
    targets, and no threshold on it means anything. (Owner, 2026-09-06: "Any Lorenz comparison
    must be to the relevant GROUP.")

    ``pieces`` is an iterable of (values, weights); weights need not be normalised.
    """
    if percentiles is None:
        percentiles = LORENZ_PERCENTILES
    vals, wts = [], []
    for v, w in pieces:
    ...
    if not np.isfinite(total) or total <= 0:
        return None
    order = np.argsort(v)
    return lorenz_shares_from_weighted_grid(v[order], w[order] / total, percentiles)


# Standard Lorenz percentile cutoffs — match the Step-2 targets convention
# (data_LorenzPts in EstimParameters.py uses [0.2, 0.4, 0.6, 0.8]). NOTE that matching Step 2's
# percentiles is not the same as matching its POPULATION: see pooled_lorenz_shares above.
