...
def _import_machine_profile():
    """Import machine_profile (Code/HA-Models is one dir up from HERE)."""
    _ha = os.path.abspath(os.path.join(HERE, ".."))
    if _ha not in sys.path:
        sys.path.insert(0, _ha)
...
def _venv_nvjitlink_path():
    """venv libnvJitLink.so.12 for JAX-CUDA LD_PRELOAD (CUDA 12.6 system /
    12.9 venv ABI mismatch — see Code/HA-Models/jax_mc_speedup/GPU_SETUP.md).
    SST owner since 2026-08-06: machine_profile.nvjitlink_preload_path (the
    GPU-stack probe and this env builder must agree on the same paths)."""
    try:
        return _import_machine_profile().nvjitlink_preload_path()
    except Exception:
        return None  # no venv nvidia stack -> GPU children unsupported anyway


def _venv_nvidia_lib_dirs():
    """Colon-joined venv ``nvidia/*/lib`` dirs for LD_LIBRARY_PATH so
    JAX-CUDA can dlopen cuBLAS/cuSPARSE/cuDNN at runtime (without them jax
    fails "Unable to load cuSPARSE" and SILENTLY falls back to CPU;
    diagnosed 2026-06-23). SST owner since 2026-08-06:
    machine_profile.nvidia_venv_lib_dirs. Returns '' if none."""
    try:
        return _import_machine_profile().nvidia_venv_lib_dirs()
    except Exception:
        return ""


def _oom_deprioritize():
    """G2, plan 20260806-1211h (OOM record 20260806-0101h): make battery
    children WORSE OOM victims than the desktop. The incident's kernel dump
    showed workers at oom_score_adj=+100 vs the GNOME session's +200, so a
    global OOM ate the user's session (gsd-xsettings -> session teardown ->
    user@1000.service stop) and left the 46 GiB job standing. Raising one's
    own score needs no privilege; multiprocessing pool workers inherit it.
    Runs as Popen preexec_fn in the forked child: must never raise (a raise
    would abort the launch), and must touch nothing but /proc/self.
    HAFISCAL_OOM_SCORE_ADJ: integer score (default 900); '0'/'off' disables.
    """
    try:
        val = os.environ.get("HAFISCAL_OOM_SCORE_ADJ", "900").strip().lower()
        if val in ("", "0", "off"):
            return
        with open("/proc/self/oom_score_adj", "w") as f:
            f.write(str(int(val)))
    except Exception:
        pass  # non-Linux / unwritable: children keep the inherited score
...
def _build_child_env(hw_slot, ad_cache=None):
    ...
    # Owner ruling 2026-08-25 18:20 (BUG-092): under AD-equilibrium SHARING the battery
    # measures the spending program's equilibrium, and its panel must carry that
    # equilibrium's income LEVEL — the weighted-tail sampler is the default (K=200 tail
    # strata per atom, capped at N/4; HAFISCAL_MC_WEIGHTED_TAIL=0 turns it off, any explicit
    # value wins). Off when the battery runs its own AD loop (the loop does not carry
    # per-household weights — welfare6_scenario refuses) or under a non-analytic_markov seed.
    _share_env = env.get("HAFISCAL_AD_EQUILIBRIUM_SHARE")
    if _share_env is None:
        # Owner ruling 2026-08-25: sharing is an IMPROVEMENT resolved from the WORLD (on in
        # `default`, off in `as-corrected`); mirror config.catalog so the parent's weighted-tail
        # default agrees with what the children will resolve.
        try:
            sys.path.insert(0, os.path.dirname(HERE))
            from config.catalog import resolve_world as _rw
            _w = env.get("HAFISCAL_WORLD", "default").strip().lower()
            _share_env = _rw(_w, include_estimation_only=False).get("HAFISCAL_AD_EQUILIBRIUM_SHARE", "0")
        except Exception:
            _share_env = "0"
    _share_on = str(_share_env).strip().lower() in ("1", "on", "true", "yes")
    if _share_on and env.get("HAFISCAL_MC_PLVL_INIT", "analytic_markov").strip().lower() == "analytic_markov":
        if "HAFISCAL_MC_WEIGHTED_TAIL" not in env:
            env["HAFISCAL_MC_WEIGHTED_TAIL"] = "200"
            print("[run_welfare6_parallel] sharing on -> weighted-tail sampler default "
                  "(HAFISCAL_MC_WEIGHTED_TAIL=200; set 0 to disable)", flush=True)
    # Owner rulings R1-R3 (2026-08-07): welfare6 children default to
    # newton2d + numba kernel + warm-start payload (worlds/explicit env
    # excluded inside the helper; FTI-absent falls back to plain EGM).
    try:
        from solver_accel import apply_newton2d_defaults as _n2d_defaults
        _n2d_defaults("welfare6", env)
    except Exception as _n2d_e:  # pragma: no cover - fail-soft
        print(f"[run_welfare6_parallel] newton2d defaults unavailable "
              f"({_n2d_e}); plain EGM", flush=True)
...
def welfare6_mc(pol_r, none_r, base_r, periods, Rfree, CRRA,
    ...
    if abs(NPV_AddInc) < 1e-10:
        return float("nan")
    # BUG-092 weighted-tail sampler: per-household population weights (mean 1) on the sums
    _wgt = pol_r.get("agent_weights", None)
    _wgt = 1.0 if _wgt is None else np.asarray(_wgt, dtype=np.float64)[None, :]
    ...
    if use_per_dur:
        ...
        for dur in range(n_dur):
            ...
            sum_per_t += rec_probs[dur] * np.sum(
                (pol_w_d - none_w_d) / base_MU_d * _wgt, 1)
        w6 = (
        ...
        w6 = (
            np.sum(
                np.sum((pol_w - none_w) / base_MU * _wgt, 1)
                / NPV_AddInc
...
def _auto_parallel_plan(cpu_count, max_parallel, scenarios,
    ...
    hold ~1.5 GB in total — measured post-BUG-064 in a WARM, single-scenario,
    in-process duration-width sweep, where fork workers share the parent's
    solutions copy-on-write. It is NOT a battery-scale anchor: cold battery
    children measure 1.8-2.7 GiB RSS per process (OOM record 20260806-0101h,
    38 processes / 46.6 GiB). Memory budgeting lives in
    machine_profile.plan_welfare6_slots (slots x _MEM_GB_PER_CHILD, with the
    G1 memory reserve), not in this cpu-count divider.
    ...
    # Memory-consistency cap (guards v2, 2026-08-06 13:41 contained-OOM
    # lesson): slot count and per-child worker budgets are INVERSELY
    # coupled — at 5 slots this divider handed each child 6 workers and the
    # G5 census measured AD children at 10.8-12.5 GiB (vs 6.66 at sw=4),
    # so FEWER slots demanded MORE total memory and the 7 GB/child envelope
    # silently stopped meaning anything. Cap workers so the child TREE fits
    # the envelope BY CONSTRUCTION — restoring the certified sw=4/dw=4
    # shape at every slot count instead of inflating with freed cores.
    # Model tree_gb = base + workers*w, recalibrated 2026-08-06 15:30 on
    # the v2 G5 censuses (sw=4 trees measured 7.6-10.0 GiB steady, 6
    # procs): base 1.5 + 4*1.7 = 8.3 = the measured median; the inversion
    # lands the cap at 4 for the re-certified 9 GB envelope. (The earlier
    # 1.3/proc calibration came from the incident's kernel dump, which
    # averaged over mixed phases and undercounted the steady solve state.)
    _child_base_gb, _worker_gb = 1.5, 1.7
    try:
        _env_gb = float(_import_machine_profile()._MEM_GB_PER_CHILD)
    except Exception:
        _env_gb = 18.0
    # The halving below dates from the 18.5 GiB both-pools spike (env 18:
    # (18-1.5)/1.7/2 -> 4 = the measured shape). Post trim/R1/A1 the
    # full-cell cold peak is 9.4 GiB measured AT dw=sw=4 with the
    # both-pools phase inside it (plan 20260807-0919h C1), so the halving
    # is superseded by measurement; the ceiling stays 4 because no wider
    # per-child shape has ever been certified.
    mem_worker_cap = min(4, max(1, int((_env_gb - _child_base_gb) / _worker_gb)))
    budget = min(budget, mem_worker_cap)
...
def launch_scenarios(parametrization, scenarios, max_parallel, out_dir,
    ...
    if max_gpu_slots is None or max_cpu_slots is None:
        try:
            ...
            # The children setdefault HAFISCAL_POLICY_STORE_REQUIRE=1 (welfare6_scenario.py:54,
            # owner 2026-08-24: the welfare battery must not solve). Apply the same default HERE
            # so the planner sees the run's EFFECTIVE value: under require-hit a store MISS is an
            # error, so no child can build the cold 12 GB buffers and the warm 6 GB envelope is
            # safe by construction (machine_profile.child_envelope_gb, the slot fix 2026-09-07).
            # A driver that exports =0 (a fresh calibration) keeps the cold plan.
            os.environ.setdefault("HAFISCAL_POLICY_STORE_REQUIRE", "1")
            # The warm envelope (6 GB) is the 10,000-household footprint. A child's memory grows with the
            # panel (13-15.5 GiB measured at 100,000 on 2026-08-27 and again 2026-09-08), so a larger
            # --agent-count-total scales the envelope unless the caller pinned HAFISCAL_WELFARE_CHILD_GB:
            # 6 GB + 0.1 GB per 1,000 households above 10,000 (100k -> 15 GB), never below the warm value.
            _act = agent_count_total
            if _act and int(_act) > 10000 and not os.environ.get("HAFISCAL_WELFARE_CHILD_GB", "").strip():
                os.environ["HAFISCAL_WELFARE_CHILD_GB"] = f"{6.0 + 0.1 * (int(_act) - 10000) / 1000.0:.1f}"
                print(f"[machine-profile] agent-count-total {_act}: per-child envelope scaled to "
                      f"{os.environ['HAFISCAL_WELFARE_CHILD_GB']} GB", flush=True)
            _plan = _mp_plan()
    ...
    # G5 (plan 20260806-1211h): the incident's 6.66 GiB/child was invisible
    # until a kernel OOM dump revealed it — memory_samples sat empty. Print a
    # per-child process-TREE RSS census into the driver log every ~60s
    # (stdlib /proc walk; silent no-op where /proc is absent).
    _rss_last = [0.0]

    def _log_child_rss():
        now = time()
        if now - _rss_last[0] < 60 or not os.path.isdir("/proc"):
            return
        _rss_last[0] = now
        try:
            trees = {}  # scenario -> (nproc, rss_gb) over the child's session
            sids = {}
            for scenario, (proc, _f, _t0, _hw) in in_flight.items():
                sids[proc.pid] = scenario  # start_new_session: sid == pid
                trees[scenario] = [0, 0.0]
            for pid_dir in os.listdir("/proc"):
                ...
                try:
                    ...
                    sid = int(st[3])  # field 6 (session id), post-comm
                    if sid not in sids:
                        continue
                    ...
                    continue
            if trees:
                total = sum(t[1] for t in trees.values())
                parts = "  ".join(f"{s}={t[1]:.1f}G/{t[0]}p"
                                  for s, t in sorted(trees.items()))
                print(f"  [mem] children total {total:.1f} GiB: {parts}",
                      flush=True)
        except Exception:
            pass  # never let telemetry take down the battery
...
def _scenario_lifetime_utility(r, Rfree, CRRA):
    """Social-planner lifetime utility of a scenario pickle: sum_t R^-t sum_i u(c_it),
    duration-probability-weighted when the scenario carries per-duration panels (the
    BUG-046 ordering: felicity per duration, THEN weight)."""
    import sys as _sys
    _ha = os.path.abspath(os.path.join(HERE, ".."))
    if _ha not in _sys.path:
        _sys.path.insert(0, _ha)
    from welfare_ce import lifetime_utility
    _w = r.get("agent_weights", None)           # BUG-092 weighted-tail sampler (mean-1 weights)
    _w = 1.0 if _w is None else np.asarray(_w, dtype=np.float64)[None, :]
    if "per_dur_cLvl_all_splurge" in r:
        return float(sum(
            p * lifetime_utility(felicity(panel, CRRA) * _w, Rfree)
            for p, panel in zip(r["rec_probs"], r["per_dur_cLvl_all_splurge"])))
    return lifetime_utility(felicity(r["cLvl_all_splurge"], CRRA) * _w, Rfree)


def compute_welfare_ce_table(loaded, periods, Rfree, CRRA):
    """The appendix's C measure (Welfare.py METHOD 3 / welfare4.tex) from the battery's
    pickles, with the BUG-082 fix: the exact CRRA consumption equivalent of each cell's
    utility change relative to that cell's own no-policy path, minus the policy's fiscal
    cost as a share of the PDV of baseline consumption; reported as recession-cell minus
    normal-times cell. AD=1 cells use the AD=0 pair's NPV cost (as welfare6 does)."""
    import sys as _sys
    _ha = os.path.abspath(os.path.join(HERE, ".."))
    if _ha not in _sys.path:
        _sys.path.insert(0, _ha)
    from welfare_ce import consumption_equivalent, cost_share
    base = loaded["base"]
    agg_cons0 = float(np.asarray(base["AggCons"])[0])
    base_panel = np.asarray(base["cLvl_all_splurge"])
    n_agents = int(base_panel.shape[1])
    # lifetime horizon (default; HAFISCAL_WELFARE_CE_HORIZON): the stationary baseline's
    # per-period felicity sum completes the reference path beyond the simulated window.
    _wb = base.get("agent_weights", None)      # BUG-092 weighted-tail sampler (mean-1 weights)
    _wb = 1.0 if _wb is None else np.asarray(_wb, dtype=np.float64)[None, :]
    u_cont = float(np.mean(np.sum(felicity(base_panel, CRRA) * _wb, axis=1)))

    def U(key):
        return _scenario_lifetime_utility(loaded[key], Rfree, CRRA)

    def npv_add_inc(pol_key, none_key):
        return (calculate_NPV(np.array(loaded[pol_key]["AggIncome"]), periods, Rfree)[-1]
                - calculate_NPV(np.array(loaded[none_key]["AggIncome"]), periods, Rfree)[-1])

    def cell(pol_key, none_key, cost_pol_key, cost_none_key):
        x = consumption_equivalent(U(pol_key), U(none_key), CRRA, Rfree, periods, n_agents,
                                   u_cont_per_period=u_cont)
        return x - cost_share(npv_add_inc(cost_pol_key, cost_none_key), agg_cons0, Rfree)

    out = {}
    for pol, tag in (("Check", "check"), ("UI", "ui"), ("TaxCut", "taxcut")):
        normal = cell(pol, "base", pol, "base")
        rec = cell(f"recession{pol}", "recession", f"recession{pol}", "recession")
        rec_ad = cell(f"recession{pol}_AD", "recession_AD", f"recession{pol}", "recession")
        out[f"{tag}_C_rec"] = rec - normal
        out[f"{tag}_C_rec_AD"] = rec_ad - normal
    return out


def write_welfare4_tex(ce, path):
    """welfare4.tex in Welfare.py's format (basis points, 3 decimals); candidate-routed."""
    import sys as _sys
    _ha = os.path.abspath(os.path.join(HERE, ".."))
    if _ha not in _sys.path:
        _sys.path.insert(0, _ha)
    from welfare_ce import format_bp
    from generated_output import open_generated
    with open_generated(path) as f:
        f.write("\\begin{tabular}{@{}lccc@{}} \n")
        f.write("\\toprule \n")
        f.write("                          & Stimulus check      & UI extension    & Tax cut    \\\\  \\midrule \n")
        f.write(f"$\\mathcal{{C}}(Rec,\\text{{policy}})$ & {format_bp(ce['check_C_rec'])}  & {format_bp(ce['ui_C_rec'])}  & {format_bp(ce['taxcut_C_rec'])}     \\\\ \n")
        f.write(f"$\\mathcal{{C}}(Rec, AD,\\text{{policy}})$ & {format_bp(ce['check_C_rec_AD'])}  & {format_bp(ce['ui_C_rec_AD'])}  & {format_bp(ce['taxcut_C_rec_AD'])}     \\\\ \n")
        f.write("\\end{tabular}  \n")


def default_run_dirs(parametrization, seed_offset=0, out_dir=None, table_dir=None):
    """The run's (out_dir, table_dir, log_dir, note) — PER-SEED by default.

    The scenario pickles are named <scenario>.pkl and the tables welfare6_candidate.tex /
    welfare4_candidate.tex with NO seed in the name, so two seed offsets writing into the same
    directories keep only the LAST one. The 2026-08-20 spine batteries ran --seed-offset 0,1,2
    into the defaults and the packet's "S=3" welfare table was seed 2 alone (seeds 0 and 1
    survived only as printed tables in the driver log). Since then a non-zero seed offset that
    names no directory gets its own: welfare6_scenario_results_<param>_seed<K> and
    Tables/<param>_parallel_seed<K>; seed 0 keeps the production paths byte-for-byte, and
    explicit --out-dir / --table-dir always win. The multi-seed band is an explicit
    aggregation over the per-seed tables (Code/HA-Models/welfare6_seedband.py), never "the
    last table".
    """
    k = int(seed_offset or 0)
    sfx = f"_seed{k}" if k else ""
    derived = []
    if out_dir is None:
        out_dir = os.path.join(HERE, f"welfare6_scenario_results_{parametrization}{sfx}")
        derived.append("out_dir")
    if table_dir is None:
        table_dir = os.path.join(HERE, "Tables", f"{parametrization}_parallel{sfx}")
        derived.append("table_dir")
    note = None
    if k and derived:
        note = (f"[seed-dirs] seed_offset={k}: per-seed default {'/'.join(derived)} -> "
                f"out_dir={out_dir} table_dir={table_dir} (seed 0 = the production paths; "
                f"aggregate the seeds with Code/HA-Models/welfare6_seedband.py)")
    # C12 (2026-08-28): an explicit --table-dir names the RUN, so its scenario logs go under that name -- two arms of a gate
    # sharing a parametrization and seed no longer overwrite each other's logs (the Econ-1 plain seed 6 failure was
    # unrecoverable for that reason). The default path (no explicit table_dir) is unchanged.
    log_dir = os.path.join(HERE, "welfare6_parallel_logs",
                           os.path.basename(os.path.normpath(table_dir)) if "table_dir" not in derived and table_dir else parametrization + sfx)
    return out_dir, table_dir, log_dir, note
...
def main():
    ...
    p.add_argument("--seed-offset", type=int, default=0,
                   help="Forwarded to each scenario subprocess. Non-zero "
                        ...
                        "A non-zero offset derives its own default dirs "
                        "(..._seed<K>) unless --out-dir/--table-dir are "
                        "given; seed 0 keeps the production paths. "
                        "Aggregate per-seed tables with "
                        "welfare6_seedband.py (adopted 2026-08-21).")
    ...
    if args.max_gpu_slots is None or args.max_cpu_slots is None:
        try:
            ...
            # --agent-count-total scales the envelope unless the caller pinned HAFISCAL_WELFARE_CHILD_GB:
            # 6 GB + 0.1 GB per 1,000 households above 10,000 (100k -> 15 GB), never below the warm value.
            _act = getattr(args, "agent_count_total", None)
            if _act and int(_act) > 10000 and not os.environ.get("HAFISCAL_WELFARE_CHILD_GB", "").strip():
                os.environ["HAFISCAL_WELFARE_CHILD_GB"] = f"{6.0 + 0.1 * (int(_act) - 10000) / 1000.0:.1f}"
                print(f"[machine-profile] agent-count-total {_act}: per-child envelope scaled to "
                      f"{os.environ['HAFISCAL_WELFARE_CHILD_GB']} GB", flush=True)
    ...
    out_dir, table_dir, log_dir, _seed_note = default_run_dirs(
        parametrization, args.seed_offset, args.out_dir, args.table_dir)
    if _seed_note:
        print(_seed_note)
    ...
    results, _loaded = compute_welfare6_table(out_dir)
    tex_path = os.path.join(table_dir, "welfare6.tex")
    write_welfare6_tex(results, tex_path)
    # BUG-082: the appendix's consumption-equivalent (basis-point) measure from the same
    # pickles, on the corrected CRRA formula -- welfare4.tex next to welfare6.tex.
    try:
        _ce = compute_welfare_ce_table(
            _loaded, _loaded["base"]["act_T"], _loaded["base"]["Rfree"], _loaded["base"]["CRRA"])
        write_welfare4_tex(_ce, os.path.join(table_dir, "welfare4.tex"))
        print("[welfare4] C (bp): " + "  ".join(f"{k}={v*1e4:.3f}" for k, v in _ce.items()))
    except Exception as _ce_err:  # never let the appendix measure abort the main table
        print(f"[welfare4] WARNING: consumption-equivalent table not written ({_ce_err!r})")
    ...
    _emit_provenance_sidecar(
        ...
        output_roots=[os.path.join(table_dir, "welfare6.tex"),
                      os.path.join(table_dir, "welfare4.tex"),
