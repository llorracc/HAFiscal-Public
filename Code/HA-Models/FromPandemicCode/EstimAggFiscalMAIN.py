...
import sys
import os
# R-e item-2 bonded pair: estimator bootstrap — see estim_phase2_tm_a.py.
...
# _calib_suffix = interp_suffix() + world_suffix(): the DiscFacEstim/AllResults
# betas are world-specific (the as-corrected counterfactual re-estimates them),
# so they carry the world tag too (e.g. '_ESC_ascorrected'). default world ->
# world_suffix()=='' -> byte-for-byte legacy. See _interpretation.py / the
# as-corrected calibration run spec (plans/20260614_*).
...
# HAFISCAL_STEP2_WEALTH_LEGACY=1 (diagnostic ONLY; NOT a reproduction switch -- corrected 2026-08-30): raw
# aLvl/aNrm with no splurge adjustment. It was added 2026-08-29 by analogy with HAFISCAL_STEP1_WEALTH_LEGACY as
# "the published Step-2 read-out", but the QE code's Step 2 already read wealth as (1-Splurge)*aLvl at every
# moment (HAFiscal-QE EstimAggFiscalMAIN.py:83,97,105,135,141); only the QE Step 1 was raw (BUG-031's Step-1
# face). Under ESC the default branch below IS the published read-out. The switch reproduces a bug the paper
# never had (HS beta 0.929 -> 0.903 on the original model; run esc4, archived orig_reest_mc_esc4_step2rawreadout).
_STEP2_WEALTH_LEGACY = os.environ.get('HAFISCAL_STEP2_WEALTH_LEGACY', '').strip() == '1'


def _aLvl_for(ThisType):
    """Return aLvl scaled per ThisType.interpretation. CDC default."""
    if _STEP2_WEALTH_LEGACY or getattr(ThisType, 'interpretation', 'CDC') == 'CDC':
...
# -----------------------------------------------------------------------------
def _lottery_rng():
    """Seeded generators for the lottery-win quarter draws (BUG-127, 2026-09-07).

    calc_mpc_by_wealth_q_simple and calc_mpc_by_wealth_q assign each household the quarter in
    which its lottery win lands by drawing from the GLOBAL np.random / random generators, which
    nothing seeds -- the `random.seed(1234)` that would have is commented out further down, in
    this file and in the published QE copy alike. So every regeneration of the lottery-MPC
    block ("Average lottery-win-year MPCs by Wealth/Education", "IMPCs over time") and of the
    nonTargetedMoments exhibit built from it was a fresh random realization: run-to-run
    ~1e-3 on the averages and up to 1.6e-2 on the smallest wealth-quartile cells, measured
    2026-09-07 by two identical-code runs of the --hot regression check. Everything else in
    the fit-table pass (Lorenz points, median wealth, the betas) is byte-stable.

    Local generators, not a reseed of the globals, so no other consumer of np.random /
    random in this process is touched. HAFISCAL_LOTTERY_SEED='' restores the published
    unseeded behaviour; any integer fixes the realization. 1234 honours the commented intent.
    """
    v = os.environ.get('HAFISCAL_LOTTERY_SEED', '1234').strip()
    if v == '':
        return None, None
    return np.random.RandomState(int(v)), random.Random(int(v))


def calc_mpc_by_wealth_q_simple(Agents):
    _np_rng, _py_rng = _lottery_rng()   # BUG-127: one seeded stream per call
    '''
    ...
    for ThisType in Agents: 
        SpendQs = (_np_rng.randint if _np_rng is not None else np.random.randint)(0,4,ThisType.AgentCount)   # BUG-127
...
#    multi_thread_commands_fake(Agents, ['solve()', 'initialize_sim()', 'simulate()', 'unpack_cFunc()'])
    ...
    for type_num, ThisType in zip(range(TypeCount), Agents):
        ...
        for i in range(ThisType.AgentCount):
            LotteryWin[i,(_py_rng.randint if _py_rng is not None else random.randint)(0,3)] = 1   # BUG-127
...
#
# perm_during_unemp WIRING FIX (2026-06-14): the unemployed-state income MUST be
# built via build_unemployed_inc_shk_dstn so it honors perm_shocks_during_unemployment
# / tran_shocks_during_unemployment — exactly like the simulation path (Simulate.py:215+).
# Previously these were hardcoded to PermShk=1.0 (perm-during-unemp OFF) regardless of
# the flag, so the ESTIMATION objective (and the estimated betas) was INVARIANT to
# HAFISCAL_PERM_DURING_UNEMP: the as-corrected (perm-off) re-estimation was a no-op, and
# the default world's betas were estimated under perm-OFF while it SIMULATES under perm-ON.
# Blast radius of this fix: with tran_shocks_during_unemployment always False, the helper
# returns the SAME single-atom DiscreteDistribution([1.0],[[1.0],[IncUnemp]]) when
# perm_shocks_during_unemployment is False — so the as-corrected/perm-OFF world (and the
# legacy QE world) is BYTE-IDENTICAL to the old hardcode. Only the default/perm-ON world's
# estimation changes (unemployed states now carry the employed permanent-shock atoms).
# => RE-ESTIMATE the default world after this fix; its committed betas were perm-OFF MC estimates.
from income_process_sst import build_unemployed_inc_shk_dstn as _build_unemp_inc
for ThisType in BaseTypeList:
    _emp0 = ThisType.IncShkDstn[0]
    _p_on = getattr(ThisType, 'perm_shocks_during_unemployment', False)
    _t_on = getattr(ThisType, 'tran_shocks_during_unemployment', False)
    ...
    IncomeDstn_unemp.seed = 763607780
    IncomeDstn_unemp.reset()
    IncomeDstn_unemp_nobenefits.seed = 763607780
    IncomeDstn_unemp_nobenefits.reset()
...
# NOTE on classification: although historically described as "variance reduction
# / opt-in", stratified MC_SHUFFLE is a BUG-FIX, not a discretionary choice —
# reliable UI welfare is a correctness requirement (without it the UI welfare
# cells swing 27-29% between replicates; BUG-044/HARK PR #1776 makes the per-state
# counts exact). It is therefore ON in BOTH the canonical `default` AND the
# bug-fixed `as-corrected` world. See
# conclusions_private/20260613_config-worlds-definition-default-legacy.md.
# POLARITY (since Plan A, 2026-06-10): MC_SHUFFLE defaults ON via the EstimParameters
# canonical block — os.environ.setdefault HAFISCAL_MC_SHUFFLE=1 (plus
# SHUFFLE_MRKV_TRANSITION=stratified, SHUFFLE_NEWBORN_FIX=transition) before this
# read fires; OFF only under HAFISCAL_QE_FIDELITY=1 (legacy QE world) or an explicit
# HAFISCAL_MC_SHUFFLE=0. INCOME_SHUFFLE remains genuinely opt-in (default off).
# See plans/20260408-1024h_minimum-replicates-for-shuffle.md for minimum-N
# requirements and commit a3ec91af for the implementation.
...
def _baseline_commands():
    """Per-agent post-solve simulation commands for one Step-2 objective eval.

    Each objective evaluation has already called ``AggDemandEconomy.solve()``
    (warm-started, converged) immediately before running these commands, so the
    leading ``'solve()'`` is a redundant COLD re-solve of the same fixed point
    (HARK's ``AgentType.solve()`` restarts from the terminal solution; ~21 cold
    solves per eval at production N). Dropping it is results-preserving: the
    warm ``economy.solve()`` solution and the cold re-solve both converge to
    HARK's distance tolerance, so simulated moments differ only at O(tol) — far
    below the Nelder-Mead resolution (``xtol``). Set
    ``HAFISCAL_STEP2_KEEP_REDUNDANT_SOLVE=1`` to restore the old behavior (for
    bisection / exact-legacy reproduction)."""
    if os.environ.get('HAFISCAL_STEP2_KEEP_REDUNDANT_SOLVE', '') == '1':
        return ['solve()', 'initialize_sim()', 'simulate()', 'save_state()']
    return ['initialize_sim()', 'simulate()', 'save_state()']
...
#    sumSquares = np.sum((Stats.avgLWPI[educ_type]-data_avgLWPI[educ_type])**2)
    ...
    if _traj_path:
        ...
        _rec = {
            ...
            # Default '1' to MATCH the actual behavior default at the read site
            # above (~:1080). Was '0', so an unset env var logged in_place=false
            # while behavior was in-place=true (owner-review resolved 2026-06-13).
            'in_place': os.environ.get('HAFISCAL_NM_IN_PLACE', '1') == '1',
        }
...
if estimateDiscFacs:
    ...
    # ESC-MOD-PHASE3-write: tag output filename with interpretation + world suffix.
    # CDC + default world stays unsuffixed (preserves byte-identity with legacy
    # reads); ESC gets _ESC; the as-corrected world adds _ascorrected so its
    # re-estimated betas never clobber the default/headline betas. Cross-
    # interpretation+world registry isolation — see _interpretation.calib_suffix.
    # _calib_suffix imported at module level (not here) so the calcAllResults
    # block sees it even when this estimation block is skipped via
    # HAFISCAL_SKIP_ESTIMATION_OPTIMIZE.
    df_resFileStr = df_resFileStr + _calib_suffix() + '.txt'
    ...
    if _single_edtype:
        # Insert _edType{N} BEFORE the trailing _ESC[_ascorrected].txt
        # (CDC+default: trailing is just .txt). Preserves the interp+world
        # suffix at the end of the filename.
        _trailing = _calib_suffix() + '.txt'
    ...
    for edType in edtypes_to_run:
        ...
        if os.environ.get('HAFISCAL_NM_START_FROM_SAVED', '1') == '1':
            ...
            try:
                ...
                import re as _re
                # Each numeric field may be a bare float (`0.93`) OR a numpy-2.x
                # repr wrapper (`np.float64(0.93)`) — the latter leaks in when the
                # writer repr()'d a dict of np.float64 values on a numpy>=2 box
                # (e.g. the Mac venvs wrote edType1/2 that way). Tolerate both so
                # warm-start doesn't silently fall back to legacy starts.
        ...
        if _PIN_START_INDEX is not None:
            # Insert _start{K} BEFORE the trailing _ESC[_ascorrected].txt
            # (CDC+default: just .txt).
            _trailing_pin = _calib_suffix() + '.txt'
            _output_file = df_resFileStr[:-len(_trailing_pin)] + f'_start{_PIN_START_INDEX}' + _trailing_pin
            _output_dict = {'EducationGroup': edType, 'beta': float(opt_params[0]), 'nabla': float(opt_params[1]), 'GICx': float(opt_params[2]), 'distance': float(_best_dist)}
        else:
            _output_file = df_resFileStr
            # float() casts so repr() always writes bare floats, never the
            # numpy-2.x `np.float64(...)` wrapper that the warm-start parser had
            # to be taught to tolerate (see BUG-039 Phase E regex above).
            _output_dict = {'EducationGroup' : edType, 'beta' : float(opt_params[0]), 'nabla' : float(opt_params[1]), 'GICx' : float(opt_params[2])}
        # HAFISCAL_RESULTS_OUT_DIR: redirect ONLY the per-edType OUTPUT write to a
        # scratch dir (basename preserved). Warm-start READS above still come from
        # ../Results, so a cross-machine / parallel run reads the committed
        # calibration but never clobbers those git-tracked files (the orchestrator
        # gathers from the scratch dir instead). Default unset → ../Results, byte-
        # for-byte unchanged. See reproduce/cross_machine_step2.py.
        ...
        if _out_dir_override:
            ...
            _output_file = os.path.join(_out_dir_override, os.path.basename(_output_file))
            if mode == 'a' and not os.path.exists(_output_file):
                mode = 'w'  # first write into a fresh scratch dir
...
if calcAllResults:
    ...
    # ESC-MOD-PHASE3-write: tag both files with interpretation + world suffix.
    # CDC + default stays unsuffixed; ESC writes/reads its own _ESC files; the
    # as-corrected world adds _ascorrected so its results never clobber default.
    df_resFileStr = df_resFileStr + _calib_suffix() + '.txt'
    ar_resFileStr = ar_resFileStr + _calib_suffix() + '.txt'
    print('Loading estimates from ' + df_resFileStr + '\n')

    # Candidate-routing (QE-baseline freeze): the ENTIRE AllResults file — the
    # header written here AND the per-group/population body appended by the
    # betas_obj_func_educ / check_disc_fac_distribution / betas_obj_func calls
    # below — must land on the SAME routed path (the `_candidate` sibling unless
    # HAFISCAL_PROMOTE=1). Previously only the header was routed (open_generated)
    # while the body writers received the raw canonical `ar_resFileStr` and
    # appended to it via open(filename,'a') — polluting the frozen canonical
    # with fresh blocks and leaving the candidate header-only. Pass `ar_routed`
    # as their `filename=` so header+body stay together. See generated_output.py.
    from generated_output import open_generated, output_path, input_path
    ar_routed = output_path(ar_resFileStr)

    if printResToFile:
        with open_generated(ar_resFileStr) as resFile: 
            print('Saving all model results in ' + ar_routed + '\n')
    ...
    # pipeline run over the frozen canonical file (input_path imported above).
    ...
    while readStr != '' :
        ...
        betas_obj_func_educ(beta, nabla, GICx, educ_type = edType, print_mode=True, print_file=printResToFile, filename=ar_routed)
        check_disc_fac_distribution(beta, nabla, GICfactor, edType, print_mode=True, print_file=printResToFile, filename=ar_routed)
        readStr = betFile.readline().strip()
    betFile.close()
    ...
    betas_obj_func([myEstim[0][0], myEstim[1][0], myEstim[2][0]], \
                 ...
                 target_option = 1, print_mode=True, print_file=printResToFile, filename=ar_routed)
    ...
    try:
        ...
        # Register the outputs that actually hold THIS run's content: the
        # candidate-routed paths (input_path/ar_routed), not the frozen canonical
        # (which under candidate-routing is untouched by a non-promote run).
        _df_routed = input_path(df_resFileStr)
        if os.path.exists(_df_routed):
            _reg.register_output(_run_id, "step2_cal", _df_routed)
        if os.path.exists(ar_routed):
            _reg.register_output(_run_id, "step5_allresults", ar_routed)
        # Compute GoF + record metrics
        if os.path.exists(ar_routed):
            _metrics = _gof_mod.compute_all_gof(ar_routed)
