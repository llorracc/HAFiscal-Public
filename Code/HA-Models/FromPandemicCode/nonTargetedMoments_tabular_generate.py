    ...
    for i, line in enumerate(lines):
        if 'Population calculations:' in line:
            ...
            for j in range(i, min(i + 20, len(lines))):
                ...
                # This is ordered from poorest to richest (verified via EstimAggFiscalMAIN.py:626
                # where MPCbyWQ[qq] = mean MPC of agents with WealthQsAll==qq, and qq=0 is the
                # bottom quartile per the cutoff-incrementing logic at line 311-316).
                # The earlier comment claiming "richest to poorest" was wrong, and the reversal
                # below was producing a flipped table where WQ4 (poor) showed the rich's MPC.
                if 'Average lottery-win-year MPCs by Wealth (incl. splurge)' in current_line and 'simple' not in current_line:
                    match = re.search(r'\[([\d., ]+)\]', current_line)
                    if match:
                        values = [float(x.strip()) for x in match.group(1).split(',')]
                        # First 4 are MPCs by WQ from poorest to richest; last is population.
                        # Table wants [WQ4, WQ3, WQ2, WQ1] = [poorest, ..., richest] — no reversal needed.
                        results['mpc_by_wq'] = values
...
def generate_nonTargetedMoments_ltx(results, output_path):
    ...
    # Write to file (candidate-routing: `_candidate` sibling unless
    # HAFISCAL_PROMOTE=1; see generated_output.py)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    from generated_output import open_generated, output_path as routed_path
    with open_generated(output_path) as f:
        f.write(output)
    
    print(f"✅ Generated {routed_path(output_path)}")
...
def main():
    ...
    # Input: AllResults file. Route through _interpretation.resolve_path so
    # ESC reads its own AllResults_..._ESC.txt; CDC reads bare file.
    import sys as _sys_es
    _ha_root_es = os.path.abspath(os.path.join(script_dir, '..'))
    if _ha_root_es not in _sys_es.path:
        _sys_es.path.insert(0, _ha_root_es)
    # The repo's interpretation default (ESC) lives in EstimParameters
    # (`os.environ.setdefault('HAFISCAL_INTERPRETATION', 'ESC')`); the _interpretation
    # library's own literal is CDC. Import EstimParameters BEFORE resolving, or this script
    # silently reads the bare CDC AllResults (the 2026-07-28 promoted table was generated
    # that way). Found 2026-08-20, spine4 prep.
    import EstimParameters  # noqa: F401
    from _interpretation import resolve_path as _resolve_path
    # Read side of the candidate routing (as CreateLPfig/CreateIMPCfig already do): prefer the
    # `_candidate` AllResults a pipeline run just produced over the frozen canonical, so this
    # table can never be regenerated from a stale AllResults (trap found 2026-08-20, spine4 prep).
    from generated_output import input_path as _input_path
    results_file = _input_path(_resolve_path(os.path.join(script_dir, '..', 'Results', 'AllResults_CRRA_2.0_R_1.01.txt')))
