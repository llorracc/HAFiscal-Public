...
def experiment_2_tm_grid_convergence():
    ...
    ref_tm = run_experiment_tm(eco, shock_type="base", mCount=300, neutral_measure=True)
    ...
    for mc in m_counts:
        t0 = time.time()
        r = run_experiment_tm(eco, shock_type="base", mCount=mc, neutral_measure=True)
...
def experiment_3_mc_convergence(ergodic_stats, tm_ref_pc):
    ...
    for N in agent_counts:
        ...
        for s in range(n_seeds):
            ...
            tm_d = compute_baseline_tm_data(eco, mCount=100, neutral_measure=True, verbose=False)
...
def experiment_4_warmup_sweep(ergodic_stats, tm_ref_pc):
    ...
    tm_d = compute_baseline_tm_data(eco_base, mCount=100, neutral_measure=True)

    for wu in warmups:
        t0 = time.time()
        eco, bd, agent = setup_single_hs(AgentCount=N, seed=42)
        tm_d_w = compute_baseline_tm_data(eco, mCount=100, neutral_measure=True, verbose=False)
...
def experiment_5_correlated_vs_independent(ergodic_stats, tm_ref_pc):
    ...
    tm_B = compute_baseline_tm_data(eco_B, mCount=100, neutral_measure=True, verbose=False)
