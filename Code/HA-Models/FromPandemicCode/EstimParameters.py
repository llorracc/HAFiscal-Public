...
# Earnings-phase SST (owner ruling 2026-08-28): with HAFISCAL_EARNINGS_PHASE_HAZARD > 0 every macro block's
# micro chain is {growing, matured} x the employment/UI states (layout Mrkv = 2J*macro + J*phase + emp) and
# the per-state arrays are wrapped block-wise below; at 0 (the default) every wrapper is the identity.
...
# (shuffle OFF, plain 'shuffle' transition, dist_aGrid_max=500, m-indexed TM), approximating the
...
if os.environ.get('HAFISCAL_QE_FIDELITY', '') == '1':
    ...
    # UI_STATE_ENCODING=calendar + UI_EXTENSION_POLICY=window (since 2026-08-26; bug_fix
    # before). Applied via setdefault (explicit env still wins).
    #   dist_aGrid_max=1300 (the TM distribution-grid top; HAFISCAL_TM_AMAX) is NOT a magic number:
    #   the TM grid must cover the MOST-PATIENT College discount-factor atom (the GIC-cap atom,
    #   GPF=theGICfactor=0.9995 under BUG-053) — its ergodic aNrm tail is the binding constraint;
    #   1300 = production_dist_aGrid_max(). The mc engine keeps 1300 (NOT QE 500) because it seeds
    #   the MC cross-section from the TM ergodic dist (Simulate.py mc_use_tm_init), and a
    #   500-truncated seed grid cannot be rebuilt by the 24-period warmup. Re-derive via
    #   production_dist_aGrid_max() if the calibration/theGICfactor changes. (See catalog tm_amax.)
    #
    ...
    _WORLD_APPLY = (
        ...
        # 2026-08-26 (Improvements A/B): the extension policy travels with the world
        # (DISCRETIONARY row; inert unless the encoding is 'calendar').
        'HAFISCAL_UI_EXTENSION_POLICY',
        # 2026-08-27: t_age_cap is a BUG_FIX (owner ruling; the published code estimated uncapped and
        # simulated capped) -- BOTH worlds are uncapped now; the key stays in the list so the catalog
        # value (none) is applied uniformly.
        'HAFISCAL_T_AGE',
        # OWNER RULING 2026-08-25: AD-equilibrium sharing (Step 5b consumes Step 5a's
        # equilibrium) is an IMPROVEMENT — on in `default`, off in `as-corrected` unless
        # opted in by an explicit HAFISCAL_AD_EQUILIBRIUM_SHARE=1 (explicit env always wins).
        'HAFISCAL_AD_EQUILIBRIUM_SHARE',
        # OWNER RULING 2026-08-28: the earnings-phase hazard (matured-earner Markov phase) travels
        # with the world (DISCRETIONARY row; canonical 0 until the wiring's gates pass, then 1/120).
        'HAFISCAL_EARNINGS_PHASE_HAZARD',
        # OWNER RULING 2026-08-29: the uniform downscaling of the education growth rates (BUG_FIX bundled
        # with t_age_cap; both worlds; 1 until the P4 install, then 0.44) travels with the world.
        'HAFISCAL_PERM_GROWTH_SCALE',
        # OWNER RULING 2026-09-06: the onset-spike t=0 exemption (BUG-122 Step 0) -- the spiked are held
        # in their first benefit quarter and the spike is resized so the unemployment path is unchanged.
        # BUG_FIX, so both worlds; the reproduction arm pins '0' EXPLICITLY (unsetting no longer means off).
        'HAFISCAL_ONSET_SPIKE_T0_EXEMPT',
        # OWNER RULING 2026-09-07 ("OK, I accept this analysis. Implement"): income-quintile strata inside the
        # stratified Markov shuffle (p:5) and the Madow quota rounding it forces -- IMPROVEMENT rows, on in
        # `default`, the paper values (unset / hamilton) in `as-corrected` until the asymptotic certification.
        # From here on UNSET no longer means the plain engine: a driver whose arm means OFF pins
        # HAFISCAL_SHUFFLE_MRKV_STRATA= (empty) and HAFISCAL_SHUFFLE_MRKV_ROUNDING=hamilton explicitly.
        'HAFISCAL_SHUFFLE_MRKV_STRATA', 'HAFISCAL_SHUFFLE_MRKV_ROUNDING',
    )
    ...
    if _world == 'as-corrected':
        import warnings as _warnings
        # Since U1b (2026-08-23) a matched as-corrected calibration exists and the
        # loader PREFERS `_ascorrected` DiscFacEstim files whenever present (see
        # _interpretation.resolve_calib_path + the vintage guard), so the old
        # unconditional "RUNTIME-ONLY counterfactual" warning is wrong when the
        # matched set is installed. Warn only when NO as-corrected calibration
        # can be found; otherwise note the matched load quietly.
        import glob as _glob
        _res_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                '..', 'Results')
        _ac_calibs = _glob.glob(os.path.join(_res_dir, 'DiscFacEstim_*_ascorrected*.txt'))
        if _ac_calibs:
            print("[world] as-corrected: matched re-estimated calibration present in "
                  "Results/ (loader prefers _ascorrected files) — full counterfactual.")
        else:
            _warnings.warn(
                "HAFISCAL_WORLD=as-corrected applies the paper's discretionary RUNTIME choices on top of "
                "all bug-fixes (e.g. PERM_DURING_UNEMP=off), but NO matched as-corrected discount-factor "
                "calibration was found in Results/ — the CANONICAL set will load. A fully-matched "
                "as-corrected counterfactual requires the RE-ESTIMATED (beta,nabla) under these settings "
                "(PERM_DURING_UNEMP affects the estimate); until installed this is RUNTIME-ONLY. See "
    ...
    # bug-fix, stratified-MC welfare, dist_aGrid_max=1300, the TM-ergodic MC seed); 'mc' is a clean same-model
    ...
    if _engine == 'mc':
        ...
        # changes vs tm; the model, bug-fixes, dist_aGrid_max, and TM-ergodic seed are identical.
...
# HAFISCAL_SPLURGE_FILE must win over the default resolution here exactly as it
# does in Parameters.return_parameters — this module-level Splurge is what
# step4/ge.py and the Step-2 engine consume, so without the override the
# cold-rerun chain's hops would silently read the installed default (caught by
# the S0.5 wiring pilot, 2026-08-13).
_SPLURGE_RESULT = os.environ.get('HAFISCAL_SPLURGE_FILE') or _resolve_path(str(_ha_root / "Target_AggMPCX_LiquWealth" / "Result_AllTarget.txt"))
...
# R-e item-2 BONDED PAIR (owner "go" 2026-08-09): the income-convention axis.
# The SCF targets above are wealth/GROSS-income ratios (make_liquid_wealth.py
# normalizes by SCF logPermIncQ — before-tax "normal income"; no tax
# adjustment anywhere). A model whose household income is net-of-tax — the
# HANK-SAM block's (1-tau)*w*N — must be disciplined by wealth/DISPOSABLE
# income, i.e. the same ratios / (1-tau). Under HAFISCAL_CALIB_TARGET_INCOME
# =net the three wealth-to-income RATIO targets are restated in disposable
# units; the Lorenz points and the share targets are distributional SHAPES
# (scale-free) and stay untouched. tau default 0.3 = the HANK fiscal block's
# tau_ss (model-consistency, NOT an SCF-2004 tax-rate estimate); override via
# HAFISCAL_CALIB_TARGET_TAU. The paired calibration files carry the
# '_netinc' suffix (calib_suffix()); resolve_calib_path HALTS rather than
# fall back across this axis. Default 'gross' is byte-inert.
_calib_target_income = os.environ.get('HAFISCAL_CALIB_TARGET_INCOME', 'gross').strip().lower()
if _calib_target_income in ('net', 'taustar'):
    if _calib_target_income == 'taustar':
        # BUG-077 stage C: the shared derived tau* (single-sourced from
        # fiscal_tau_star; ignores HAFISCAL_CALIB_TARGET_TAU).
        import sys as _sys_cti
        _ha_models_cti = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if _ha_models_cti not in _sys_cti.path:
            _sys_cti.path.insert(0, _ha_models_cti)
        from fiscal_tau_star import TAU_STAR as _calib_target_tau
        _calib_target_tau = float(_calib_target_tau)
    else:
        _calib_target_tau = float(os.environ.get('HAFISCAL_CALIB_TARGET_TAU', '0.3'))
    data_avgLWPI = data_avgLWPI / (1.0 - _calib_target_tau)
    data_LWoPI = data_LWoPI / (1.0 - _calib_target_tau)
    data_medianLWPI = data_medianLWPI / (1.0 - _calib_target_tau)
    print(f"[calib-target-income] {_calib_target_income.upper()} convention: wealth/income "
          f"targets restated in the model's net-income units (x 1/(1-{_calib_target_tau:g})); "
          f"medianLWPI -> [{', '.join(f'{x:.2f}' for x in data_medianLWPI)}] "
          f"(Lorenz points and share targets unchanged — scale-free)")
elif _calib_target_income != 'gross':
    raise ValueError(f"HAFISCAL_CALIB_TARGET_INCOME must be 'gross' or 'net', "
                     f"got {_calib_target_income!r}")
...
# Scalar override, mirroring return_parameters' precedence (OVERRIDE > FILE >
# default). Placed before the argv[5]/MC-determinism blocks so their existing
# wins are unchanged.
_splurge_env_override = os.environ.get('HAFISCAL_SPLURGE_OVERRIDE')
if _splurge_env_override:
    Splurge = float(_splurge_env_override)
if len(sys.argv) >= 6:
...
# CRRA from argv[2] -- REINSTATED 2026-08-21 (BUG-085). The old block here was commented
# out together with the retired per-CRRA splurge-file autoload it contained, which
# silently disconnected CRRA from the positional-argument convention every Step-2 caller
# uses (argv = Rfree CRRA IncUnemp IncUnempNoBenefits Splurge; Rfree flows at ~line 528,
# IncUnemp/NoBenefits below, Splurge above -- CRRA alone stayed at the 2.0 default).
# Invisible while every caller passed 2.0; the first gamma!=2 caller (the 2026-08-20
# gamma-chain G3) estimated at gamma=2 while writing gamma=2-named files over main-spec
# intermediates. The splurge autoload is deliberately NOT restored: Splurge arrives via
# argv[5] / HAFISCAL_SPLURGE_OVERRIDE exactly like the other overrides.
if len(sys.argv) >= 3:
    CRRA = float(sys.argv[2])
    if CRRA not in (1.0, 2.0, 3.0):
        print(f'[EstimParameters] WARNING: CRRA={CRRA} -- the splurge was only estimated for 1.0/2.0/3.0')
    if CRRA != 2.0:
        # printed so a wrong-gamma run can never again be silent in its own log
        print(f'[EstimParameters] CRRA={CRRA} (argv override of the 2.0 default)')
...
# UI-extension ENCODING (Improvement A, 2026-08-26; rule SST: Code/HA-Models/ui_extension_rule.py)
#
# HAFISCAL_UI_STATE_ENCODING:
#   'legacy':    the published QE code -- 4 micro states {e, u1Q, u2Q, noBen}; the extension is the
#                transition_ub=False FREEZE of the benefit chain on transitions into experiment blocks
#                1..ExtraUBperiods (Parameters.make_cond_mrkv_arrays_recession_ui). The as-corrected
#                world's value (the paper's encoding AND the paper's policy).
#   'calendar':  (default) 2 + UBspell_normal + n_extension micro states, the PLAIN chain in every
#                scenario; the extension is INCOME at the extension states, paid on a calendar window
#                of experiment periods (t_enact..t_end). The paper's window reproduces the legacy
#                freeze EXACTLY as a state relabeling (derivation in ui_extension_rule.py; it needs
#                ExtraUBperiods = 3 extension states, i.e. 7 micro states, because the freeze holds
#                the onset cohort for three transitions). Identical Markov dynamics across the
#                policy/no-policy pair restore per-agent CRN coupling for the stratified-shuffle
#                engine (~1/15 the UI-cell variance at fixed N): the speedup the improvement is for.
#   'bug_fix':   the 2026-05-16 six-state rule (u3Q/u4Q paid iff the CURRENT macro state is the
#                recession). A DIFFERENT policy from the paper's, mislabeled a bug fix; BUG-043 is
#                WITHDRAWN (RECONCILED_private/RECONCILED-005_*). Kept to reproduce 05-16..08-26 numbers.
# Policy (Improvement B): HAFISCAL_UI_EXTENSION_POLICY = 'window' (the paper's) | 'history'
# (enactment lag 1, open entry, in force t=1..8, +2 quarters per spell) -- resolved by
# ui_extension_rule.resolve_window; per-parameter overrides HAFISCAL_UI_EXT_{ENACT_LAG,END,ENTRY,QUARTERS}.
# Only read under 'calendar'. Plan: plans/20260826-0800h_ui-extension-calendar-window-encoding_plan.md
HAFISCAL_UI_STATE_ENCODING = os.environ.get('HAFISCAL_UI_STATE_ENCODING', 'calendar').strip()
assert HAFISCAL_UI_STATE_ENCODING in ('legacy', 'calendar', 'bug_fix'), \
    f"HAFISCAL_UI_STATE_ENCODING must be 'legacy', 'calendar' or 'bug_fix', got {HAFISCAL_UI_STATE_ENCODING!r}"

# UBspell_extended: the QE code's extended-benefit duration constant ("average duration of
# unemployment benefits when extended and assuming policy remains in place"; Parameters.py imports
# it from here). ExtraUBperiods = UBspell_extended - UBspell_normal = 3 is the number of frozen
# transitions in the published freeze -- hence both the paper window's t_end and its extension-state
# count. NOT the paper text's "four quarters": the freeze also restarts the onset cohort's clock.
UBspell_extended = 5

from ui_extension_rule import (resolve_window as _resolve_ui_window,
                               num_extension_states as _num_ext_states,
                               describe as _describe_ui_encoding)
HAFISCAL_UI_EXTENSION_POLICY = os.environ.get('HAFISCAL_UI_EXTENSION_POLICY', 'window').strip().lower()
UI_EXT_WINDOW = _resolve_ui_window(HAFISCAL_UI_EXTENSION_POLICY, UBspell_normal, UBspell_extended)

# Policy_ExtraBenefitQuarters: the number of EXTENSION micro states = the extra benefit quarters per
# spell the chain can carry: 0 under legacy (no such states -- the 4-state list everywhere), 2 under
# bug_fix (fixed), the policy's n_extension under calendar (3 for the paper's window, 2 for 'history').
Policy_ExtraBenefitQuarters = _num_ext_states(HAFISCAL_UI_STATE_ENCODING, UI_EXT_WINDOW)
...
if HAFISCAL_UI_STATE_ENCODING != 'legacy':
    print(_describe_ui_encoding(HAFISCAL_UI_STATE_ENCODING, UI_EXT_WINDOW, num_base_MrkvStates))
...
def make_full_mrkv_array(MacroMrkvArray, CondMrkvArrays):
    ...
    # Earnings phase: every macro pair's employment block M_ik becomes [[(1-h) M_ik, h M_ik], [0, M_ik]]
    # (the phase transition is independent of the employment/macro transition of the same period);
    # MacroMrkvArray and CondMrkvArrays stay UNWRAPPED (J x J). Identity when the phase is off.
    return [_ep.wrap_chain(FullMrkv, CondMrkvArrays[0].shape[0])]
...
PermGroFac_base =   [1.0]
# Uniform downscaling lambda of the education growth rates (owner ruling 2026-08-29; catalog `perm_growth_scale`, a
# BUG_FIX bundled with the cap removal, plan 20260829-0845h): PermGroFac_e = 1 + lambda * g_e/4 with the Pandemic
# paper's annual g_e unchanged below. lambda = 1 reproduces the paper's factors bit for bit (1.0 * x == x exactly);
# the world loop above setdefaults the catalog value and an explicit HAFISCAL_PERM_GROWTH_SCALE wins. This is the
# SINGLE application point: the GIC cap, build_PermGroFac_micro, Parameters/Simulate, the TM p-mixture, the welfare
# battery and Step 4 (through the PE calibration) all read these three factors.
_perm_growth_scale = float((os.environ.get('HAFISCAL_PERM_GROWTH_SCALE') or '1').strip() or 1.0)
if not (0.0 <= _perm_growth_scale <= 1.0):
    raise ValueError(f'HAFISCAL_PERM_GROWTH_SCALE={_perm_growth_scale!r}: expected a scale in [0, 1]')
PermGroFac_base_d = [1.0 + _perm_growth_scale * (0.01421/4)]  # From Pandemic paper: avg growth rates during
PermGroFac_base_h = [1.0 + _perm_growth_scale * (0.01812/4)]  # working life for each education group
PermGroFac_base_c = [1.0 + _perm_growth_scale * (0.01958/4)]
...
#      distribution exists, the eigenvector solve piles mass at the distribution-grid
#      top (dist_aGrid_max), and only finite-horizon MC is meaningful.)
#
...
#      FAT (cut off only by mortality), which forces a tall `dist_aGrid_max` — the
#      GIC-edge atom is the binding case for the distribution-grid top (the (1−1e-4)
#      ergodic aNrm quantile ≈ 1224 → dist_aGrid_max = 1300; see
#      `adaptive_grid_tm.production_dist_aGrid_max`).
#
# Full write-up:
...
# ---------------------------------------------------------------------------
# Owner ruling 2026-08-14 (BUG-078 arc; REVISED later same day: FVAC dropped):
# the cap CRITERION is the mortality-adjusted AGGREGATE cusp alone. Precondition
# imposed in practice (owner note 2026-08-14):
# FHWC (Γ < R; with mortality Γ·L < R) — without finite human wealth the PF
# solution is c(w)=∞ for EVERY w (c = κ·(w+h), h infinite), degenerate before
# any other condition can bite; holds identically here (Γ_e ≤ 1.0049 < R),
# β-independent ⟹ no β-cusp. The criterion: finiteness of the
# AGGREGATE wealth–income ratio uses the
# RAW GPF (BST eq:GICLivMod, Modigliani estate destruction; the E[1/ψ] Jensen
# term belongs to the mean individual ratio and LLN-cancels in ΣM/ΣP):
#     beta_AGG = (Γ/L)^ρ / R
# Nondegeneracy of the consumption function is guarded by the FHWC precondition
# and RIC (R^(ρ-1)/L, slack above β_AGG here) — NOT by FVAC: the FVAC cusp
# (β_FVAC = Γ^(ρ-1)/(L·E[ψ^(1-ρ)]), E[ψ^(1-ρ)] = exp(σ²ρ(ρ-1)/2)) was briefly
# in a min() criterion on 2026-08-14 and then DROPPED by owner ruling the same
# day — FVAC divergence is a value-LEVEL statement invisible to Euler iteration;
# the optimal value stays finite in the (β_FVAC, β_AGG) window (income floor +
# RIC fund PF-growth consumption from savings) and an EGM probe solves
# finite/monotone/continuous straight through the FVAC frontier. β_FVAC stays
# computed below as a diagnostic only.
# DEFAULT since 2026-08-19 (owner ruling, reaffirmed the same evening after repeated
# earlier adoptions: "what matters is whether the GPF that results in finite aggregate
# wealth is the ultimate upper limit"): GICmaxBetas = the mortality-adjusted AGGREGATE
# cusp beta_AGG = (Gamma_e/L)^rho / R — the beta at which aggregate wealth diverges.
# HAFISCAL_STEP2_GIC_AGGCAP=0 is the rollback to the GPF-Mod cap (the pre-ruling value
# and the cap under which the currently-INSTALLED betas were estimated).
# TRANSITIONAL NOTE (until S7 promotes the matched re-estimated calibrations): the
# installed College calibration was estimated under the GPF-Mod cap, so under this
# default its top atom un-clips at load (the higher cusp no longer binds it) — a real,
# owner-accepted behavior change on the default path; every chain candidate since the
# cold-rerun S2 stage was already estimated under this cap, so the S7 promotion
# restores the matched pair. The theGICfactor shave below applies to the selected cap
# unchanged (β-space margin ×shave^ρ — BUG-053 machinery). Step-1's sibling
# (Estimation_BetaNablaSplurge) has defaulted to the aggregate cusp since 08-14.
# Derivation + numbers + the FVAC-drop probe:
# conclusions_private/2026-08-14_gic-cap-min-fvac-aggregate.md (REVISED);
# scope + history: BUGS_private/HAFiscal_BUG-078_*.md.
_Ex_psi_one_minus_rho = np.exp(PermShkStd[0]**2 * CRRA * (CRRA - 1.0) / 2.0)  # E[ψ^(1-ρ)]
_PermGroFacs_by_edu = np.array([PermGroFac_base_d[0], PermGroFac_base_h[0], PermGroFac_base_c[0]])
# Earnings-phase state (owner ruling 2026-08-28; earnings_phase.py): when the matured-earner phase is
# on, asymptotically every old household is matured and grows at Gamma = 1, so the growth that
# governs the stationarity of AGGREGATE wealth -- the binding GIC -- is the matured phase's, not the
# education group's. The cap therefore uses Gamma = 1 in every group (the Step-1 aggregate cap
# beta_AGG = (1/L)^rho / R replaces the education caps: -0.2 / -0.4 / -0.5 % for D / HS / C).
_cap_growth_by_edu = np.ones(3) if _ep.enabled() else _PermGroFacs_by_edu
_beta_FVAC_by_edu = _cap_growth_by_edu**(CRRA - 1.0) / (LivPrb_base[0] * _Ex_psi_one_minus_rho)
_beta_AGG_by_edu = (_cap_growth_by_edu / LivPrb_base[0])**CRRA / Rfree_base[0]
if os.environ.get('HAFISCAL_STEP2_GIC_AGGCAP', '1') == '1':
    GICmaxBetas = _beta_AGG_by_edu.copy()
...
# HAFISCAL_GIC_FACTOR (2026-08-29): the GIC cap factor as an env; default 0.9995 (BUG-053's re-estimation input, byte-identical),
# `0.999` = the published value (column A of the ergodic-conversion chain runs the paper's own estimation procedure; catalog row
# `the_gic_factor`, class design-or-withdrawn: reported nowhere as a correction, restored only to reproduce the paper).
theGICfactor = float((os.environ.get('HAFISCAL_GIC_FACTOR') or '0.9995').strip() or 0.9995)
minBeta = 0.01
...
if _ep.enabled():
    # Earnings phase: the wrapped base chain's stationary vector is degenerate (matured is absorbing, so
    # all its mass is matured); newborns are born GROWING at the employment chain's ergodic mix.
    # (J = this chain's OWN employment count: the estimation surface's base chain is the UBspell+2 chain
    # without extension states, shorter than num_base_MrkvStates -- the per-state arrays are longer and
    # get first-StateCount sliced by the solver, as before.)
    init_mrkv_dist_d = _ep.growing_block_ergodic(MrkvArray_base_d[0], CondMrkvArrays_base_d[0].shape[0])
    init_mrkv_dist_h = _ep.growing_block_ergodic(MrkvArray_base_h[0], CondMrkvArrays_base_h[0].shape[0])
    init_mrkv_dist_c = _ep.growing_block_ergodic(MrkvArray_base_c[0], CondMrkvArrays_base_c[0].shape[0])
else:
...
# HAFISCAL_T_AGE: maximum-age-cap override — same semantics as Parameters.py
# (owner decision 2026-07-26: the undocumented QE-era 200-quarter cap executes
# 28.5% of each cohort at the wall; 'none' = no cap, flips at the tail-state
# epoch; unset = status quo). Kept in sync with Parameters.py by
# test_t_age_flag.py.
_t_age_env = os.environ.get('HAFISCAL_T_AGE', '').strip().lower()
if _t_age_env in ('', None):
    _T_age_estim = None         # EPOCH 2026-07-27: uncapped (belief-consistent) default
elif _t_age_env in ('none', 'off', '0'):
    _T_age_estim = None
else:
    _T_age_estim = int(_t_age_env)
    if _T_age_estim <= 0:
        raise ValueError(f"HAFISCAL_T_AGE integer must be positive; got {_T_age_estim}")
...
init_dropout = {"cycles": 0, # This will be overwritten at type construction
                ...
                'T_age': _T_age_estim,  # BUG-038 QE-era cap (200) unless HAFISCAL_T_AGE overrides
                'AgentCount': 10000,
                ...
                # per-state arrays over the base micro block: J entries, or [growing J | matured J] under the
                # earnings phase (Rfree/LivPrb identical across phases; PermGroFac = 1 in every matured state)
                "Rfree" : _ep.wrap_states(np.array(num_base_MrkvStates*Rfree_base)),
                "PermGroFac": [np.array(_ep.wrap_permgrofac(build_PermGroFac_micro(PermGroFac_base_d[0], num_base_MrkvStates, PermGroFac_base_d[0] if unemp_pLvl_grows_like_employed else PermGroFac_unemp)))],
                "LivPrb": [_ep.wrap_states(np.array(LivPrb_base*num_base_MrkvStates))],
                ...
                # The ESTIMATION surface's own employment count (the base chain built above from
                # UBspell_normal alone: 2 + UBspell_normal = 4), NOT the production count with the
                # UI-extension states (num_base_MrkvStates = 7, exported for Parameters.py). Until
                # 2026-08-29 this dict carried 7 while its chain had 4 states -- harmless pre-phase
                # (states 0..3 decode identically under % 7) but under the earnings phase the 8-state
                # chain's matured states decoded wrongly (post2's MC pass crashed on NaN controls).
                'num_base_MrkvStates' : int(CondMrkvArrays_base_d[0].shape[0]),
...
adj_highschool = {
    ...
    "PermGroFac": [np.array(_ep.wrap_permgrofac(build_PermGroFac_micro(PermGroFac_base_h[0], num_base_MrkvStates, PermGroFac_base_h[0] if unemp_pLvl_grows_like_employed else PermGroFac_unemp)))],
...
adj_college = {
    ...
    "PermGroFac": [np.array(_ep.wrap_permgrofac(build_PermGroFac_micro(PermGroFac_base_c[0], num_base_MrkvStates, PermGroFac_base_c[0] if unemp_pLvl_grows_like_employed else PermGroFac_unemp)))],
...
# the legacy 40/48). The COUNT scaling is ESSENTIAL: extending aXtraMax at a fixed count thins the
# moment region (37→33 gridpoints ≤ m=6) and shifts HS beta ~0.31% — the scaled count restores
# beta-neutrality (E4/E5 in the journal). Derivations validated in grid_sizing.py and
# prompts_local/2026-06-24_grid-sizing-experiments-journal.md. (The TM DISTRIBUTION grid is sized
# separately by adaptive_grid_tm.production_dist_aGrid_max()=1300, the measured Pareto-tail quantile;
# grid_sizing.tm_dist_aGrid_max is its analytic cross-check / GIC-Mod-failure fallback.)
# Precedence: explicit HAFISCAL_SOLVE_AMAX > endogenous (flag on) > measured-Q K·h̄ rule > legacy.
# DEFAULT (2026-07-23 owner flip): the PF-decay extrapolation and measured-Q are ON by default, so
# the K·h̄ branch FIRES on a bare run — the default solve top is K=3·h̄ per group (467/551/591).
# LEGACY (top-40): set HAFISCAL_PF_DECAY_EXTRAP=0 (naive-linear world) or =exp, or
# HAFISCAL_PF_DECAY_Q=slope — any of these keeps the per-group vars at the legacy 40/48.
# SST (owner ruling 2026-07-24): the full precedence — HAFISCAL_SOLVE_AMAX >
# endogenous (HAFISCAL_ENDOGENOUS_GRID, this site's opt-in) > K·h̄ under the
# measured-Q default > legacy — lives in grid_sizing.resolve_solve_grid (ONE
# implementation; FromPandemicCode/Parameters.py and the Step-1 setup consume
# the same function). This site only gathers ITS primitives per education
# group and assigns the results. Semantics unchanged (byte-parity gated by
# test_resolve_solve_grid.py + the 2026-07-24 golden-probe matrix): K·h̄ rule
# from plans/20260722_local-two-secant-tail-q_plan.md; count held to near-0
# density (E4/E5: fixed count thins the moment region, shifts HS beta ~0.31%)
# from the count-converged basis 192 unless HAFISCAL_AXTRA_COUNT / a code-level
# count override (aXtraCount != 48) wins; endogenous branch = solve_grid_aMax
# per group at its GIC-cap atom.
...
_grid_why = None
for _e_idx, _G_base in ((0, PermGroFac_base_d), (1, PermGroFac_base_h), (2, PermGroFac_base_c)):
    _aMax_e, _aCount_e, _grid_why = _gs.resolve_solve_grid(
        Rfree=Rfree_base[0], PermGroFac=_G_base[0],
        legacy_aMax=aXtraMax, legacy_count=aXtraCount, aXtraMin=aXtraMin,
        allow_endogenous=True,
        endogenous_kwargs=(lambda _e=_e_idx: dict(
            beta=gic_capped_beta(_e, theGICfactor), CRRA=CRRA, LivPrb=LivPrb_base[0])),
        count_basis_anchor=48, tag='[grid_sizing]')
    if _e_idx == 0:
        aXtraMax_d, aXtraCount_d = _aMax_e, _aCount_e
    elif _e_idx == 1:
        aXtraMax_h, aXtraCount_h = _aMax_e, _aCount_e
    else:
        aXtraMax_c, aXtraCount_c = _aMax_e, _aCount_e
if _grid_why is not None:
...
# sized to the BASE state count (the whole micro block: 2J under the earnings phase) -- the pre-switch base solve reads them
intercept_prev = np.ones((_ep.n_states(num_base_MrkvStates, 1), _ep.n_states(num_base_MrkvStates, 1)))    # Intercept of aggregate savings function
slope_prev = np.zeros((_ep.n_states(num_base_MrkvStates, 1), _ep.n_states(num_base_MrkvStates, 1)))       # Slope of aggregate savings function
...
init_ADEconomy = {'intercept_prev': intercept_prev,
                     ...
                     'MrkvArray' : MrkvArray_base_h,
                     'num_base_MrkvStates' : int(CondMrkvArrays_base_d[0].shape[0]),   # the estimation economy's own count (see the agent dict)
                'PermGroFac_unemp': PermGroFac_unemp,
                'perm_shocks_during_unemployment': perm_shocks_during_unemployment,
                'tran_shocks_during_unemployment': tran_shocks_during_unemployment,
                     "MrkvArray_base" : MrkvArray_base_h, 
