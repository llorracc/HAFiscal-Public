...
def lock_axes_for(canonical_name, fig=None):
    """Owner ruling 2026-08-26: a regenerated figure keeps the PUBLISHED figure's axes.
    Applies the limits recorded in Code/HA-Models/fig_axes_published.json (recovered from
    the ../HAFiscal-QE PDFs by fig_axes.py) to `fig` (Figure or pyplot; default: the current
    pyplot figure) when `canonical_name` (basename, extension optional) is in the registry.
    HAFISCAL_FIG_AXES_LOCK=0 disables. Returns the lock report or None."""
    try:
        _ha = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        if _ha not in sys.path:
            sys.path.insert(0, _ha)
        import fig_axes
    except Exception as exc:  # never break a figure save over the lock
        print(f"[fig-axes] unavailable ({exc}); axes left unlocked", flush=True)
        return None
    if fig is None:
        import matplotlib.pyplot as plt
        fig = plt
    try:
        return fig_axes.lock_axes(fig, os.path.basename(str(canonical_name)))
    except Exception as exc:
        print(f"[fig-axes] {canonical_name}: lock failed ({exc}); axes left unlocked", flush=True)
        return None
...
def save_generated_figure(canonical_path, fig=None, **savefig_kwargs):
    ...
    if fig is None:
        import matplotlib.pyplot as plt
        fig = plt
    lock_axes_for(canonical_path, fig)
...
def make_figs_generated(figure_name, saveFigs, drawFigs, target_dir):
    ...
    (which writes <name>.{pdf,png,jpg,svg} into target_dir). The published-axes
    lock is applied to the current pyplot figure first (lock_axes_for)."""
    from HARK.utilities import make_figs
    _track_dir(os.path.join(str(target_dir), "x"))  # provenance blanket
    lock_axes_for(figure_name)
