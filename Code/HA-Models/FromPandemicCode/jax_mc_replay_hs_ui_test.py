"""HS_Only recessionUI replay v2 sanity — validates UI extension scenario."""
...
def main():
    print("=== HS_Only recessionUI replay v2 test ===", flush=True)
    t0 = time.time()
    ...
    print(f"HARK AD-UI: {time.time()-t0:.1f}s ({len(iter_logs)} iters)", flush=True)
    ...
    for c_idx, agent in enumerate(eco.agents):
        ...
        print(f"  cohort {c_idx} N={agent.AgentCount} JAX-replay-v2 AggCons[0]={per_cohort_aggcons[-1][0]:.4f}", flush=True)

    jax_total = np.sum(per_cohort_aggcons, axis=0)
    jax_cratio = jax_total / base_AggCons
    print(f"\n=== HS_Only recessionUI replay-v2 Summary ===")
