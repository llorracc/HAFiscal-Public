...
def Output_Results(saved_results_dir,fig_dir,table_dir,Parametrization='Baseline'):
    ...
    if Parametrization=='Baseline':
        ...
        try:
            ...
            import warnings
            warnings.warn(
                "[HANK overlay] Step-4 artifact not found (%s); skipping the "
                "Cumulative_multipliers_withHank comparison figure. It is a Step-4 "
                "(HANK/SAM) product, absent on Step-5-standalone runs; downstream "
                "outputs such as Multiplier.tex are unaffected." % HANK_results_dir
            )

    # Short-circuit guard: if Parametrization != 'Baseline' the first operand is
    # False and HANK_results (bound only inside the block above) is never read.
    if Parametrization=='Baseline' and HANK_results is not None:
        fig, ax = plt.subplots()
