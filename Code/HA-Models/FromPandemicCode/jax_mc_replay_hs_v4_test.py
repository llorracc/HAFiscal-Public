"""
HS_Only full-alignment test: replay v2 + v4 ref (with sim_birth capture).

If this closes JAX-HARK bias to <0.1% at HS_Only, we've proven that full
RNG alignment (shocks + newborn replacement) reaches FP precision.
"""
...
def main():
    print("=== HS_Only full-alignment test (replay v2 + v4 ref) ===", flush=True)
    t0 = time.time()
    ...
    t0 = time.time()
    eco.solve()
    print(f"Re-solve: {time.time()-t0:.1f}s", flush=True)
    ...
    # NOTE on initial state: HARK's history[0] is END of period 0.
    # The aNrm_init_perperiod[0] is the START of period 0 (post-burn-in / sim_birth).
    # Use that for true alignment.
    for c_idx, agent in enumerate(eco.agents):
        ...
        # Use aNrm_init_perperiod[0] as the actual START state (not history[0])
        ...
        print(f"  cohort {c_idx} N={agent.AgentCount} JAX-replay-v2 AggCons[0]={ac[0]:.6f} wall={time.time()-t_c:.1f}s",
              flush=True)

    jax_total = np.sum(per_cohort_aggcons, axis=0)
    jax_cratio = jax_total / base_AggCons
    print(f"\n=== HS_Only Replay-v2 Summary ===")
    print(f"JAX-replay-v2 AggCons[0]={jax_total[0]:.6f}, HARK={hark_aggcons[0]:.6f}, ratio={jax_total[0]/hark_aggcons[0]:.6f}")
