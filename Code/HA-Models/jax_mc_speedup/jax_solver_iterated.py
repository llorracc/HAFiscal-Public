...
def required_m_top(agent):
    """Largest next-period market-resources value the kernel can query.

    The kernel evaluates the belief tables at mNrmNext = R*a/(G*psi) + theta*AD
    for every (a in the agent's grid, shock atom, C slice); the tables are
    tabulated on ``m_eval``, and queries ABOVE its top are linearly
    extrapolated — for a convex decreasing vP that extrapolation eventually
    goes NEGATIVE, and vP**(-1/CRRA) then produces NaN policy knots. The
    historical fixed ``_DEFAULT_M_EVAL`` (top 50) silently relied on the
    legacy aXtraMax=40-class grids; under the 2026-07-23 measured-Q K*h-bar
    default grids (tops ~470-590) it under-spans and the 2B solve NaNs out
    (F1.1 finding). Compute the actual reachable top from the agent's own
    grid + shocks, with 10% headroom.
    """
    S = agent.MrkvArray[0].shape[0]

    def _ps(v):
        arr = np.asarray(v, dtype=float).reshape(-1)
        return np.full(S, float(arr[0])) if arr.size == 1 else arr[:S]

    R = _ps(agent.Rfree)
    G = _ps(agent.PermGroFac[0] if isinstance(agent.PermGroFac, list)
            else agent.PermGroFac)
    a_top = float(np.max(np.asarray(agent.aXtraGrid)))
    inc = agent.IncShkDstn[0]
    perm_min = min(float(np.min(np.asarray(inc[j].atoms[0]))) for j in range(S))
    tran_max = max(float(np.max(np.asarray(inc[j].atoms[1]))) for j in range(S))
    ADel = float(getattr(agent, 'ADelasticity', 0.0) or 0.0)
    AD_max = float(np.max(np.asarray(agent.Cgrid, dtype=float))) ** abs(ADel) \
        if ADel else 1.0
    top = float(np.max(R)) * a_top / (float(np.min(G)) * perm_min) \
        + tran_max * max(1.0, AD_max)
    return 1.10 * top


def default_m_eval_for(agent):
    """``_DEFAULT_M_EVAL`` extended (geometrically, 200 pts) to cover
    :func:`required_m_top` for this agent. No-op when the legacy span suffices."""
    from jax_solver_drop_in import _DEFAULT_M_EVAL
    base = np.asarray(_DEFAULT_M_EVAL)
    top_need = required_m_top(agent)
    if top_need <= base[-1]:
        return base
    return np.unique(np.concatenate(
        [base, np.geomspace(base[-1], top_need, 200)]))
...
def extract_solve_inputs(agent, solution_initial=None, m_eval=None):
    ...
    if m_eval is None:
        m_eval = default_m_eval_for(agent)
    ...
    # Per-state parameter mapping — the PRODUCTION slicing convention
    # (AggregateDemandEconomy._per_state_param / the stock solver's [:StateCount]):
    # HAFiscal sizes Rfree/PermGroFac/LivPrb for the recession UI encoding, which
    # can be LONGER than this solve's StateCount (e.g. the base estimation
    # economy: MrkvArray (4,4) with 6-sized params). The kernel broadcasts these
    # against (StateCount, M, C) tables, so unsliced arrays raise shape errors
    # (F1.1 finding, 2026-07-23); slice exactly as the stock solver does.
    def _per_state(v):
        arr = np.asarray(v, dtype=float).reshape(-1)
        if arr.size == 1:
            return np.full(StateCount, float(arr[0]))
        if arr.size < StateCount:
            raise ValueError(
                f"per-state param has {arr.size} entries < StateCount={StateCount}")
        return arr[:StateCount]

    Rfree = _per_state(agent.Rfree)
    PermGroFac = _per_state(
        agent.PermGroFac[0] if isinstance(agent.PermGroFac, list)
        else agent.PermGroFac)
    LivPrb = _per_state(
        agent.LivPrb[0] if isinstance(agent.LivPrb, list) else agent.LivPrb)
