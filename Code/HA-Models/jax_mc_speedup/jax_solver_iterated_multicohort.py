...
def extract_solve_inputs_stacked(agents, from_solutions=None, m_eval=None):
    ...
    if m_eval is None:
        # SHARED adaptive m_eval (F1.1, 2026-07-23): the per-agent default
        # (default_m_eval_for) sizes the table span to the agent's own grid —
        # required under the K*h-bar default grids (the fixed top-50 grid NaNs
        # the kernel there) — but vmap stacking needs ONE shared grid, so take
        # the widest requirement across cohorts. (Heterogeneous per-cohort
        # aXtraGrids remain unsupported by this stacked path — the stack below
        # raises loudly — exactly as before.)
        from jax_solver_iterated import required_m_top
        from jax_solver_drop_in import _DEFAULT_M_EVAL
        base = np.asarray(_DEFAULT_M_EVAL)
        top_need = max(required_m_top(a) for a in agents)
        m_eval = base if top_need <= base[-1] else np.unique(
            np.concatenate([base, np.geomspace(base[-1], top_need, 200)]))
...
def solve_all_cohorts_to_convergence_consumer_solutions(
    ...
    # F1.1 (2026-07-23): same power-law tail attach as the serial 2B drop-in
    # (see jax2b_powerlaw_tail; gated by the power-law default + the
    # HAFISCAL_JAX2B_ALLOW_NO_TAIL escape). Per-cohort limits differ (DiscFac,
    # IncShkDstn), so the attach is computed per agent.
    from jax2b_powerlaw_tail import build_tail_solution
    ...
    for c_idx in range(n_cohorts):
        ...
        mNrm = mNrm_all[c_idx]
        bc = bc_all[c_idx]
        sol_tail = build_tail_solution(agents[c_idx], np.asarray(cNrm),
                                       np.asarray(mNrm), np.asarray(bc),
                                       Cgrid_np, BoroCnstArt, CRRA)
        if sol_tail is not None:
            per_cohort_solutions.append([sol_tail])
            continue
