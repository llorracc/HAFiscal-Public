...
def solve_to_convergence_consumer_solution(
    ...
    # F1.1 (2026-07-23): under the power-law PF-decay DEFAULT, attach the same
    # per-(state, C-slice) power-law measured-Q tails the stock solver attaches
    # (bounded post-solve wrap; in-sample bit-identical). Escape hatch:
    # HAFISCAL_JAX2B_ALLOW_NO_TAIL=1 (legacy naive-linear wrapper). Under
    # exp/0 the legacy wrapper is kept (2B never had an exp attach).
    from jax2b_powerlaw_tail import build_tail_solution
    sol_tail = build_tail_solution(agent, cNrm, mNrm, BoroCnstNat_per_i,
