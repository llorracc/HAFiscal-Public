...
# OMP_NUM_THREADS cap (would silently throttle to e.g. 3 on a 32-thread box
# launched from a shell that exported OMP_NUM_THREADS=3), so strip the OMP
# vars for the probe — but do NOT use `nproc --all`, which also ignores the
# cgroup cpuset: under SLURM (Rockfish) or taskset that reads the whole node
# instead of the allocation and oversubscribes the grant.
