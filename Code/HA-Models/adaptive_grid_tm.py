...
def production_dist_aGrid_max(interpretation="ESC", threshold=THRESHOLD,
    ...
    """
    # BUG-084 FIX (2026-08-21): probe with beta=2.0 — far above ANY cap regime — so the
    # clip-to-cap premise holds by construction (the old 1.01 probe silently measured an
    # arbitrary beta=1.01 agent once the aggregate-cusp College cap rose to 1.011419 > 1.01),
    # and SELF-CHECK that the clip actually landed.
    # BUG-084 FIX (criterion v2, 2026-08-21, after owner review): certify the quantile by
    # MEASURED CONVERGENCE, not by a pile-mass margin. The build's jump-to-grid clips
    # above-top mass into the top cell, but the truncated dynamics' OWN ergodic undercounts
    # the true tail ~3x (clipped agents recycle downward — the 2026-07-26 finding that
    # read-out corrections cannot rescue a low top), so any fixed pile margin is a fitted
    # knob: 10x wrongly refused a certifiable real config (installed College atom @12k,
    # pile=threshold/8.6), 2x missed a known-bad one (@8k, true quantile 12% past the top).
    # Instead: compute the quantile on the covering grid AND on a 1.5x taller one; a
    # converged quantile is top-invariant, a truncation-corrupted one moves with the top.
    # The pile check survives as a fast-path for colossal overruns (skips the second
    # build). Under the aggregate-cusp cap the cap atom (killed-Kesten alpha ~1.42) fails
    # certification at ANY practical covering grid — by design: the quantile rule is
    # RETIRED (sizing rule of record = onset AND support, onset_dist_top.required_top();
    # conclusions_private/2026-08-21_dist-top-onset-rule-ruling.md — which RATIFIES the
    # recorded production dist_aGrid_max=1300).
    def _one(cov_top, cov_n):
        g, m, bt = college_top_ergodic(probe_beta, 0.0, cov_top,
                                       interpretation=interpretation, aCount=int(cov_n))
        if probe_beta >= 1.5 and bt >= probe_beta - 1e-9:
            raise RuntimeError(
                "production_dist_aGrid_max: the probe atom did NOT clip to the GIC cap "
                f"(beta_top={bt}); the cap plumbing has changed — fix before trusting any "
                "quantile from this function.")
        m = np.asarray(m, dtype=float)
        m = m / m.sum()
        pile = float(m[-1])
        if pile > 0.5 * threshold:
            raise RuntimeError(
                f"production_dist_aGrid_max: top-cell pile mass {pile:.3e} exceeds "
                f"threshold/2 ({0.5 * threshold:.1e}) on the covering grid (top "
                f"{cov_top:.0f}) — the (1-{threshold:g}) quantile is NOT certifiable "
                "(BUG-084; the quantile rule is RETIRED — use "
                "onset_dist_top.required_top(); recorded production value: "
                "HAFISCAL_TM_AMAX=1300).")
        c = np.cumsum(m)
        return float(np.interp(1.0 - threshold, c / c[-1], g)), bt
    q1, beta_top = _one(covering_aMax, covering_aCount)
    q2, _ = _one(1.5 * covering_aMax, np.ceil(1.5 * covering_aCount))
    if abs(q1 - q2) / max(q2, 1.0) > 0.05:
        raise RuntimeError(
            f"production_dist_aGrid_max: quantile NOT converged in the covering top "
            f"(q={q1:.0f} at {covering_aMax:.0f} vs q={q2:.0f} at {1.5 * covering_aMax:.0f}, "
            f"disagreement {abs(q1 - q2) / max(q2, 1.0):.1%} > 5%) — the truncated dynamics "
            "are corrupting the tail (BUG-084; the quantile rule is RETIRED — use "
            "onset_dist_top.required_top(); recorded production value: "
            "HAFISCAL_TM_AMAX=1300).")
    q = q2
