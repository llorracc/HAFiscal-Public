#!/usr/bin/env python
"""check_rec via BUCKETED-5D — DIAGNOSTIC (superseded as the Plan-A closure).

OUTCOME STAMP (2026-06-10 decision): check_* welfare cells are MC (unified-MC
canonical method); bucketed-5D retains the structural bucket-mean φ(pLvl)
Jensen limit (~+0.9% check_rec vs MC, does not converge away), so it is NOT
the closure it was built to be. A provable TM check fix would need the 6-D
(pLvl-tracking) kernel — deferred. Kept as a diagnostic cross-check.
...
def _work(arg):
    c, b, d_idx = arg
    J = __import__('earnings_phase').j_full(_POL[c].num_base_MrkvStates)   # the micro block (2J under the earnings phase)
...
def main():
    ...
    bd = compute_baseline_tm_data(base, dist_aGrid_count=aCount, neutral_measure=True)
