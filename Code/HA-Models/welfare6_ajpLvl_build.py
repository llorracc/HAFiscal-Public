...
def compute_ajpLvl_joint(agent, bd, n_plvl=120, n_iter=2500, tol=1e-12, verbose=True):
    ...
    A = len(aGrid); J = __import__('earnings_phase').j_full(int(agent.num_base_MrkvStates))   # the micro block (2J under the earnings phase)
    ...
    _spl = getattr(agent, 'Splurge', None)
    if _spl is None:
        raise AttributeError(
            "welfare6_ajpLvl_build: agent has no Splurge attribute — refusing the stale "
            "hardcoded fallback (dual-path sweep 2026-09-02; the old default 0.2571 was an "
            "April-era CDC vintage and produced silently wrong welfare at any other vintage)")
    Splurge = float(_spl)
