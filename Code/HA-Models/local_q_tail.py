...
def local_q_from_knots(m, c, h, mpc_min, min_span=MIN_SPAN, targets=SELECT_TARGETS,
    ...
    diag : update the process/round diagnostics and arm the one-shot fallback
           warning (the attach-layer call). ``diag=False`` is the pure
           estimator — used per iterate by the ConsumedATI solver's measured-Q
           tail (BUG-088, 2026-08-24) so its transient iterates never feed the
           attach's drift advisory.
    ...
    if sel is None:
        return _fallback("fewer than three usable knots", diag)
    idx = np.array(sel)
    x3 = x[idx]
    ...
    if span < min_span:
        return _fallback(
            ...
            f"for local-Q identifiability)", diag)
    gap3 = mpc_min * x3 - c[idx]
    c3 = c[idx]
    if np.any(gap3 <= 0.0) or np.any(gap3 / np.maximum(c3, 1e-300) <= GAP_FLOOR):
        return _fallback("non-positive or sub-floor gap at a selected knot", diag)
    Q1, Q2, drift_top = two_secant_q(x3, gap3)
    if Q2 <= 0.0:
        return _fallback(f"non-positive upper-secant exponent Q2={Q2:.4f}", diag)
...
def measured_q_for_solver(m, c, mpc_min, h):
    """The attach layer's measured exponent as a solver hook (BUG-089, 2026-08-24):
    ``tail_q_fn(m_knots, c_knots, MPCmin, h_s) -> Q`` for the ConsumedATI-Markov
    'lagged' mode (fast-time-iteration ``lagged_tail_q``) and for the in-formulation
    Picard reference that certifies it — the SAME estimator object on both sides,
    diagnostics off. NaN where the local power law is not identifiable (the solver
    then uses that state's exp tail for the iterate)."""
    r = local_q_from_knots(m, c, h, mpc_min, diag=False)
    return float(r["Q"]) if r["ok"] else float("nan")


def _fallback(reason, diag=True):
    if not diag:
