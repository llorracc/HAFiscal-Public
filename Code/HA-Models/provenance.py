...
# Cross-process reuse fragments (schema v2): battery CHILDREN do not emit
# sidecars (HAFISCAL_PROV_CHILD=1 — the driver emits one aggregate), so a
# child dumps its reuse ledger as a dotfile fragment the driver ingests.
# Dotfiles are already excluded from _hash_outputs.
REUSE_FRAGMENT_PREFIX = ".reuse_"
REUSE_FRAGMENT_SUFFIX = ".json"
...
# ---------------------------------------------------------------------------
# Reuse ledger (schema v2): what this RUN actually REUSED, not just requested
# ---------------------------------------------------------------------------
# The gap this closes (plan 20260803-0900h): a run's bytes can legitimately
# depend on cache state (solve caches, ad_full guarded reuse, belief seeds,
# presolve warm starts), and none of that reached the sidecar — two runs with
# IDENTICAL sidecars could differ in bytes with no recorded reason. Events
# are appended in-process at each reuse decision point (best-effort, never
# raising) and included by gather()/emit(). byte_effect per event:
#   "exact"     — reuse occurred and the result is byte-identical to the
#                 no-cache path (policy_full/base_aggcons HITs; ad_full
#                 guard_pass AND guard_fail — its fallback is a full reset);
#   "tolerance" — reuse occurred with a documented tolerance-class deviation
#                 (knot-reconstruct hark_solve_only HITs ~1e-5 warm-start
#                 class; cached_eco_solve HIT ~3e-7; ad_cache HITs;
#                 ad_belief_seed consumed);
#   "none"      — no byte effect on THIS run (saves, publishes, platform/
#                 scheduling observations).
# determinism_class = "cache-history-dependent" iff any tolerance event,
# else "pure-config". Platform events never flip the class.

_REUSE_EVENTS: list = []
_STAMPED_DIRS: set = set()   # abs dirs emit() stamped in THIS process


def record_reuse_event(kind, outcome, byte_effect="none", **fields):
    """Append one reuse event to the in-process ledger. NEVER raises."""
    try:
        ev = {"kind": str(kind), "outcome": str(outcome),
              "byte_effect": str(byte_effect), "t_utc": _now_iso()}
        for k, v in fields.items():
            ...
            try:
                ...
                v = repr(v)
            ev[str(k)] = v
        _REUSE_EVENTS.append(ev)
    except Exception:
        pass


def reuse_summary() -> dict:
    """The sidecar's `reuse` block: derived determinism class + the events."""
    try:
        tol = any(e.get("byte_effect") == "tolerance" for e in _REUSE_EVENTS)
        return {"determinism_class":
                ("cache-history-dependent" if tol else "pure-config"),
                "events": list(_REUSE_EVENTS)}
    except Exception:  # pragma: no cover - defensive
        return {"determinism_class": "pure-config", "events": []}


def already_stamped(output_dir) -> bool:
    """True if emit() already wrote a sidecar into this dir THIS process
    (the generated_output atexit blanket uses this to avoid double-stamping)."""
    try:
        return os.path.abspath(str(output_dir)) in _STAMPED_DIRS
    except Exception:
        return False


def dump_reuse_fragment(output_dir, label=None):
    """Child-side: write this process's reuse ledger as a dotfile fragment
    (battery children are sidecar-suppressed via HAFISCAL_PROV_CHILD; the
    driver ingests fragments into its aggregate sidecar). Best-effort."""
    try:
        if not _REUSE_EVENTS:
            return None
        d = Path(output_dir)
        d.mkdir(parents=True, exist_ok=True)
        p = d / f"{REUSE_FRAGMENT_PREFIX}{os.getpid()}{REUSE_FRAGMENT_SUFFIX}"
        payload = {"schema_version": SCHEMA_VERSION, "pid": os.getpid(),
                   "label": label, "argv": sys.argv,
                   "events": list(_REUSE_EVENTS)}
        tmp = d / f".{p.name}.tmp.{os.getpid()}"
...
def ingest_reuse_fragments(dirs, cleanup=True):
    """Driver-side: fold children's fragments into THIS process's ledger
    (each event tagged with its source child). Returns fragments ingested."""
    n = 0
    try:
        for d in dirs or ():
            dp = Path(d)
            if not dp.is_dir():
                continue
            for p in sorted(dp.glob(f"{REUSE_FRAGMENT_PREFIX}*{REUSE_FRAGMENT_SUFFIX}")):
                try:
                    frag = json.loads(p.read_text())
                    src = frag.get("label") or f"pid{frag.get('pid')}"
                    for ev in frag.get("events", []):
                        ev = dict(ev)
                        ev.setdefault("source_child", src)
                        _REUSE_EVENTS.append(ev)
                    n += 1
                    if cleanup:
                        p.unlink()
    ...
    try:
        ...
        for d in output_dirs:
            try:
                ...
                _STAMPED_DIRS.add(os.path.abspath(str(d)))
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    for sc_path in sidecars:
        ...
        if ru is not None:
            evs = ru.get("events", [])
            print(f"reuse   : determinism_class={ru.get('determinism_class')}  "
                  f"({len(evs)} event{'s' if len(evs) != 1 else ''})")
            shown = [e for e in evs
                     if e.get("outcome") not in ("saved", "published",
                                                 "miss_solved_saved")][:12]
            for e in shown:
                extra_bits = {k: v for k, v in e.items()
                              if k not in ("kind", "outcome", "byte_effect",
                                           "t_utc")}
                brief = ", ".join(f"{k}={v}" for k, v in list(extra_bits.items())[:4])
                print(f"    [{e.get('byte_effect'):>9}] {e.get('kind')}:"
                      f"{e.get('outcome')}  {brief}")
            if len(evs) > len(shown):
                print(f"    ... and {len(evs) - len(shown)} more "
                      f"(saves/publishes included in the sidecar)")
    print("=" * 72)
    return 0
