#!/bin/bash
# P28 (dell, 2026-08-27 20:45; owner ruling 3): the ORIGINAL model with the paper's erroneous tax-cut experiment reproduced on the
# new machinery (HAFISCAL_LEGACY_TAXCUT_ATOM=1 now reaches the exact TM -- run-time transitory rescaling + the paper's simulated
# 21:45: HAFISCAL_T_AGE=200 added -- the as-corrected world dropped the cap this afternoon (t_age_cap -> BUG_FIX); the original model simulated WITH the cap.
# process in kernel and aggregator -- and the welfare battery): 5a + five seeds -> Tables/Baseline_orig_typo*, which the chain's start
# column and the BUG-023 breakdown row read. The pre-mimic outputs were renamed *_pre_mimic_20260827 (their tax-cut cells were the fix's).
...
# exponent is moot. 5a + seeds 0..4 + band each. Ends with "P28 DELL DONE" (P21 waits for it).
set -u
...
ORIG=(HAFISCAL_WORLD=as-corrected HAFISCAL_T_AGE=200 HAFISCAL_INTERPRETATION=CDC HAFISCAL_PERMGROFAC_FIX=0 HAFISCAL_PF_DECAY_EXTRAP=0 HAFISCAL_PF_DECAY_Q=slope HAFISCAL_GIC_SHAVE_ON_GPF=0 HAFISCAL_DISCFAC_FILE=$Q/Results/DiscFacEstim_CRRA_2.0_R_1.01.txt HAFISCAL_SPLURGE_FILE=$Q/Target_AggMPCX_LiquWealth/Result_AllTarget.txt)
