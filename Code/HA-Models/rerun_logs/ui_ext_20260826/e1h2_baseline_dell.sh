...
LOG=$REPO/Code/HA-Models/rerun_logs/ui_ext_20260826; Q=$LOG/e1h2_queue.txt; cd $FPC; source $HA/launch_helpers.sh
...
n=0
# gate (2026-08-28 10:35): wait for the tmux-scope plainH arm AND for the Econ-2 same-seed triple (owner 10:20: P32 cancelled) to finish -- one
# Baseline battery at a time on the box (systemd unit, MemoryMax=24G)
until grep -q "E1H BASELINE plainH DONE" $LOG/e1h_baseline.out 2>/dev/null; do sleep 60; done   # 12:40: the triple moved to m5; gate on the plainH reruns only
stamp "gate open"
while :; do
  ...
  echo "E1H2 BASELINE $MODE DONE $(date +%H:%M:%S)"
done
echo "E1H2 BASELINE QUEUE DONE $(date +%H:%M:%S)"
