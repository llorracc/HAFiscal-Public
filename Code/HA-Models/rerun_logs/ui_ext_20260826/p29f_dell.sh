...
# the thread pins (launch_helpers.sh). Then P32 (own-loop 40x) follows via its gate on "P29F DELL DONE".
set -u
...
export HAFISCAL_WORLD=default HAFISCAL_UI_EXTENSION_POLICY=window HAFISCAL_PARALLEL_SOLVE=10   # fewer forked cohort solves: the 5a's memory spike is what tripped systemd-oomd
...
seeds_only() { NAME=$1
  for K in 0 1 2; do T1=$(date +%s); stamp "w6[$NAME] seed $K start (BUG-098 rerun)"
    runw6 --parametrization $NAME --seed-offset $K --out-dir welfare6_scenario_results_${NAME}_seed$K --table-dir Tables/${NAME}_seed$K > $LOG/w6_${NAME}_seed$K.log 2>&1; rc=$?
    stamp "w6[$NAME] seed $K end rc=$rc wall=$(( ($(date +%s)-T1)/60 ))min tables=$(ls Tables/${NAME}_seed$K/*.tex 2>/dev/null | wc -l)"; done
  stamp "band[$NAME]"; $PY compute_welfare6_se_table.py --summaries Tables/${NAME}_seed{0,1,2}/welfare6_parallel_summary.json --out $LOG/welfare6_${NAME}_S3_seed_band.tex 2>&1 | tail -n 4; }
echo "P29f (P29e remainder after BUG-098: the Rspell_4 5a is done and re-keyed; its seeds + ADElas) starting at $(git -C $REPO rev-parse --short HEAD) $(date +%H:%M:%S)"
seeds_only Rspell_4; cfg ADElas
echo "P29F DELL DONE $(date +%H:%M:%S)"
