#!/bin/bash
# Econ-1 gate at HS_Only, part 3 (2026-08-28 09:20): the REVISED lever — quotas stratified by initial income quantiles within each state (HAFISCAL_SHUFFLE_MRKV_STRATA=p:5) and by income x wealth (p:5,a:4); 10 seeds each, plain baseline from parts 1+2. Waits for E1B.
...
until grep -q "E1B HSONLY GATE DONE" $LOG/e1b_gate.out 2>/dev/null; do sleep 30; done
for MODE in pstrat pastrat; do
  unset HAFISCAL_MC_STRATIFY_UNEMP; [ $MODE = pstrat ] && export HAFISCAL_SHUFFLE_MRKV_STRATA=p:5 || export HAFISCAL_SHUFFLE_MRKV_STRATA=p:5,a:4
...
echo "E1C HSONLY GATE DONE $(date +%H:%M:%S)"
