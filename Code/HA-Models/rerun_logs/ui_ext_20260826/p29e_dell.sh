...
# the thread pins (launch_helpers.sh). Then P32 (own-loop 40x) follows via its gate on "P29E DELL DONE".
set -u
...
echo "P29e (relaunch of the P29c remainder after the systemd-oomd kill at 11:17) starting at $(git -C $REPO rev-parse --short HEAD) $(date +%H:%M:%S)"
cfg Rspell_4; cfg ADElas
echo "P29E DELL DONE $(date +%H:%M:%S)"
