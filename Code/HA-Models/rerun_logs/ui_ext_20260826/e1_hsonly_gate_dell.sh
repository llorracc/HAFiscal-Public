#!/bin/bash
# Econ-1 gate at HS_Only (2026-08-28): (1) publish the HS_Only equilibria in the default world (paper's UI window) — the battery
...
T0=$(date +%s); stamp "5a[HS_Only uiA_gate] start"; HAFISCAL_FIGS_SUFFIX=_uiA_gate run5a --hs-only > $LOG/mult_HS_Only_uiA_gate.log 2>&1; rc=$?
stamp "5a[HS_Only uiA_gate] end rc=$rc wall=$(( ($(date +%s)-T0)/60 ))min"; grep "AD effect)" Tables/HS_Only_uiA_gate/Multiplier_candidate.tex 2>/dev/null; [ $rc -eq 0 ] || exit 1
for MODE in plain strat; do
  [ $MODE = strat ] && export HAFISCAL_MC_STRATIFY_UNEMP=0.3 || unset HAFISCAL_MC_STRATIFY_UNEMP
  for K in 0 1 2; do T1=$(date +%s); stamp "w6[HS_Only $MODE] seed $K start"
...
echo "E1 HSONLY GATE DONE $(date +%H:%M:%S)"
