...
until grep -q "IDLE TESTS M5 DONE" $HOME/idle_tests_m5.out 2>/dev/null; do sleep 120; done; stamp "gate open (idle tests done; MOVED TO m5 12:40)"
...
echo "E3 5A DIAGNOSTICS DONE $(date +%H:%M:%S) on $(hostname)"
