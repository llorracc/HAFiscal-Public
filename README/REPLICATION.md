...
The publication-default reproduction. Dispatches to `reproduce/reproduce_computed.sh`, which runs `python Code/HA-Models/do_all.py` with Steps 1, 2, 4, and 5 enabled (Step 3, the Splurge=0 robustness, is OFF by default — use `--comp max` to enable it):

- Solves heterogeneous agent models
- Generates all main paper figures and tables
- Performs Monte Carlo simulations
- Replicates the quantitative results in the paper
