\input{./relpath-to-latexroot.ltx}
\documentclass[\latexroot/\projectname]{subfiles}
\input{\latexroot/@resources/subfile-setup.ltx}

\begin{document}

\begin{figure}[H]
  \centering
  \caption{\pregenmark Model validation for nontargeted spending patterns}
  \whenintegrated{\label{fig:untargetedMoments}} % integrated doc: SST for labels
  \noindent\begin{minipage}{\textwidth}
    \centering
    \begin{subfigure}[b]{.48\linewidth}
      \centering
      \frozenORcandidate[width=\linewidth]{\latexroot/Code/HA-Models/FromPandemicCode/IMPCs_wSplEstimated.pdf}
      \caption{Dynamic spending after lottery win}
      \whenintegrated{\label{fig:USaggmpclotterywin}} % integrated doc: SST for labels
    \end{subfigure}
    %
    \begin{subfigure}[b]{.48\linewidth}
      \centering
      % Original path: \latexroot/Code/HA-Models/FromPandemicCode/Figures/UnempSpell_Dynamics
      \frozenORcandidate[width=\linewidth]{\latexroot/Code/HA-Models/FromPandemicCode/Figures/UnempSpell_Dynamics.pdf}
      \caption{Spending upon UI benefit expiry}
      \whenintegrated{\label{fig:expiryUI}} % integrated doc: SST for labels
    \end{subfigure}
  \end{minipage}
\end{figure}
\noindent\parbox{\textwidth}{\footnotesize
  \textbf{Note}: This figure demonstrates model performance on nontargeted validation moments (Section~\ref{sec:nonTargetedMoments}).
  Subfigure~(a) shows the model's dynamic consumption response compared to~\cite{fagereng-mpc-2021} estimates
  using the discount factor distributions estimated separately for each education group.
  Subfigure~(b) validates the model against~\cite{ganongConsumer2019}, who find that nondurable spending
  drops by 12\% the month when UI benefits expire; our quarterly model predicts an 18\% drop
  the quarter after benefit expiry,   demonstrating broad consistency with this empirical pattern.
}

\vspace{1em}  % Add space after figure

% Smart bibliography: Only include bibliography if standalone AND has citations
\smartbib

\end{document}
