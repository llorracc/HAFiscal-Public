...
ensure_uv_environment() {
    ...
    # A HEALTHY .venv symlink is the repo convention (`make sync` creates it and
    # tests/tools resolve the interpreter through it) — leave it alone. This
    # script uses platform-specific paths directly, so it only cleans up a
    # DANGLING symlink (target gone), which would otherwise shadow the venv.
    # (Caught 2026-08-24: the old unconditional removal here deleted the live
    # symlink mid-pytest whenever a test exercised reproduce.sh's parse surface,
    # making every later subprocess-spawning test fail unpredictably.)
    if [[ -L "$script_dir/.venv" ]] && [[ ! -e "$script_dir/.venv" ]]; then
        local symlink_target=$(readlink "$script_dir/.venv" 2>/dev/null || echo "unknown")
        log INFO "Removing DANGLING .venv symlink (pointed to: $symlink_target)"
        rm -f "$script_dir/.venv"
        log SUCCESS "Dangling symlink removed"
    fi
