...
If you have the main project environment (UV-based):

```bash
# From project root, activate architecture-specific venv:
# Apple Silicon Mac:
...
# Dashboard dependencies (voila, ipywidgets) should already be installed
# If not: pip install voila ipywidgets nbconvert
jupyter nbextension enable --py widgetsnbextension --sys-prefix
```

**Note**: The main project uses UV with architecture-specific virtual environments (e.g., `.venv-darwin-arm64`). If you prefer Conda, use Method 1 instead.
...
# First activate your architecture-specific venv (examples below)
source .venv-darwin-arm64/bin/activate  # Adjust for your platform/arch
