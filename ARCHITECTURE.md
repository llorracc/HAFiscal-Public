...
**Last Updated**: 2026-06-13 (econ-mw integration merge: `--mode`, drift/pLvl flags)  
...
> **Inspecting the MODEL rather than the code layout?** Start at
> **[`HAFiscal-math-to-code-map.md`](HAFiscal-math-to-code-map.md)** — the
> complete mathematical statement of the model with per-object code anchors
> (household Bellman, the C-conditional solver, beliefs/AD fixed point,
> experiment structure, outputs) and the register of which performance
> layers are byte-exact vs documented-residual. It sequences the deeper
> spec docs (`HAFiscal-bellman-for-matsya.md`,
> `HAFiscal-doloplus-orchestrator.md`, `MODEL_INTERPRETATIONS.md`,
> `HAFiscal-doloplus-draft.yaml`).
...
Flags (see ./reproduce.sh --help for the full, current list):
  ...
  --world NAME        WORLD axis (config/ SoT): default = paper headline,
                      as-corrected = paper + all-and-only bug-fixes (runtime-only for now)
  --multiplier-engine X  METHOD axis: tm = TM multipliers, mc = reliable MC
                      multiplier cross-check (--mode default|legacy = deprecated alias)
```
