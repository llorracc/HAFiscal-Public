...
% DEBUGGED: 20250907-1112h Removed duplicate label - kept the one before the main section (line 63)
\pdfonly{
...
Additional robustness exercises show that the model can fit the liquid wealth distribution for alternative interest rates of $0.5\%$ and $1.5\%$ per quarter. In both cases, the estimation exactly matches the median liquid wealth to permanent income ratios for each education group listed in Panel~B of Table~\ref{tab:estimBetas}.
...
The drop in spending when unemployment benefits expire is virtually the same in the model without splurge consumption (17\% versus 18\% in the baseline).
...
This is shown in the solid and dashed orange lines in Figure~\ref{fig:expiryUI_wSplZero}, and is due to the wider distribution of discount factors that is needed to match the wealth distributions in the model without the splurge. This leads to a greater number of agents being close to the borrowing constraint.
...
The policy ranking in terms of the multiplier shifts slightly.
