...
Table~\ref{tab:robustness_gamma} displays the values we obtain for the splurge and the discount factor distributions when we change $\gamma$ to $3$.
...
The second row of table~\ref{tab:robustness_gamma} shows results when we increase risk aversion from $\gamma=2.0$ to $\gamma=3.0$.
...
Table~\ref{tab:robustness_gamma_mlwpi} confirms that the re-estimated model continues to match the median liquid-wealth-to-permanent-income ratio for each education group at $\gamma=3.0$, and Figure~\ref{fig:LorenzPts_robustness_CRRA} shows that it continues to match the distribution of liquid wealth within each group.

% gamma=1 row DROPPED (owner ruling 2026-08-21, citing BUG-086): the published row was a
% finite-burn-in artifact -- the estimation objective is horizon-dependent in the
% GIC-violating region (College beta = 1.05\textdagger, downstream replacement value
% unreported). Internal record:
% BUGS_private/HAFiscal_BUG-086_crra1_row_finite_burnin_nonstationary_estimate.md;
% cold-rerun evidence: Results/cold_rerun_2026-08/crra1_parked/ (8/8 cold starts pinned at
% the rho=1 GIC cap, objective 0.441 vs 0.0017 at rho=2).
We do not report estimates for $\gamma=1.0$.
At log utility there is no parameter configuration of the model that both satisfies the GIC and matches the liquid-wealth targets: re-estimation using a method that computes the model's exact stationary distribution drives the most patient discount-factor type to the GIC bound, while the estimation objective remains two orders of magnitude worse than at $\gamma=2.0$.
Estimates for $\gamma=1.0$ reported in an earlier version of this appendix were obtained from finite simulations; in the region where the GIC fails, simulated wealth moments depend on the length of the simulation rather than on a stationary distribution of the model, and the College-group estimate of $\beta$ exceeded the GIC bound for every type in the discrete approximation.
The incompatibility of log utility with the observed concentration of liquid wealth is itself a robustness result: values of risk aversion materially below the baseline are not merely a worse fit, but are rejected by the wealth data.
...
We do not conduct these calculations for $\gamma=1.0$: as explained above, at log utility the model has no GIC-satisfying configuration that matches the level and distribution of liquid wealth.
