    ...
    if [[ "$REPRODUCTION_MODE" == "quick" ]]; then
        ...
        if [[ $compile_result -eq 0 ]]; then
            ...
            # DEFERRED:                 cleanup_auxiliary_files "$doc_path" "$doc_name"
                ...
                # Check a FRESH PDF was generated despite the error. Testing mere
                # existence (as this did before 2026-08-04) lets a stale PDF from an
                # earlier build mask a total compile failure -- see the fuller note on
                # the full-mode branch below.
                local pdf_output="${doc_path%.tex}.pdf"
                local pdf_output_mtime=0
                if [[ -f "$pdf_output" ]]; then
                    pdf_output_mtime=$(stat -f %m "$pdf_output" 2>/dev/null || stat -c %Y "$pdf_output" 2>/dev/null || echo 0)
                fi
                if [[ -f "$pdf_output" ]] && [[ "$pdf_output_mtime" -ge "$start_time" ]]; then
                    local end_time
                    end_time=$(date +%s)
                    log_warning "$doc_name: latexmk reported error but a fresh PDF was generated"
        ...
        # Check the PDF was actually (re)generated by THIS run.
        #
        # BUGFIX 2026-08-04: this tested only that SOME pdf existed, so a stale file
        # from an earlier build turned a failed compile into a reported success --
        # and compile_result was captured on the lines above and then never consulted
        # at all. That is how a Public tree which could not compile was reported as
        # building cleanly: HAFiscal.pdf was shipped in the tree, latexmk died on a
        # missing \input, produced nothing, and the mere presence of the old file
        # satisfied the test. The wrapper then printed "completed successfully" and
        # exited 0, so nothing downstream could tell either.
        #
        # A non-zero exit is still tolerated when a FRESH pdf appeared, since latexmk
        # returns non-zero for some warnings -- but a stale pdf is now fatal.
        ...
        local pdf_mtime=0
        if [[ -f "$pdf_check" ]]; then
            pdf_mtime=$(stat -f %m "$pdf_check" 2>/dev/null || stat -c %Y "$pdf_check" 2>/dev/null || echo 0)
        fi

        if [[ ! -f "$pdf_check" ]] || [[ "$pdf_mtime" -lt "$start_time" ]]; then
            if [[ ! -f "$pdf_check" ]]; then
                log_error "$doc_name compilation failed (no PDF produced; latexmk exit $compile_result)"
            else
                log_error "$doc_name compilation failed (PDF not rewritten by this run -- stale; latexmk exit $compile_result)"
            fi
            local log_file="${doc_path%.tex}.log"
            if [[ -f "$log_file" ]]; then
                parse_latex_error "$log_file" "$doc_name"
        ...
        if [[ $compile_result -ne 0 ]]; then
            log_warning "$doc_name: latexmk exited $compile_result but a fresh PDF was produced"
        fi
...
main() {
    ...
    if [[ -f "$PREGENERATED_FLAG" ]]; then
        ...
        echo ""
        echo "The full reproduction takes 4-5 days on a high-end 2025 laptop (migration-era figure; Step-5a measured 9.45h forked-AD at Baseline, 2026-06-11 — see Code/HA-Models/README.md) but provides complete verification"
