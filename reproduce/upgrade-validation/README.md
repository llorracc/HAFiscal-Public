...
> **ARCHIVED IN PLACE — DO NOT RUN (owner ruling 2026-06-12; COMMENT_AUDIT_FINDINGS
> finding o / orchestration rows 22-23).** The 0.14.1 → 0.17.0 migration this
> harness was built to validate is COMPLETE, and the harness itself is partly
> fictional relative to the final pipeline: `step_runner.py` expects result
> files (`EstimResults.txt`, `AggFiscalResults.txt`, `SimulationResults.txt`,
> `AllResults.txt`) that never existed and uses step labels that diverge from
> `do_all.py`'s canonical steps; `interactive_validation.py` would build a
> Python 3.9 + PyPI `econ-ark==0.17.0` environment, while this branch requires
> Python 3.10/3.11 + the pinned git HARK ref (see CLAUDE.md). Running it
> produces guaranteed false failures. Kept as the historical record of the
> upgrade campaign; the authoritative cross-version conclusion is
> `conclusions_private/2026-04-20_hark-017-can-reproduce-hafiscal-qe-bit-identical-when-properly-configured.md`.
