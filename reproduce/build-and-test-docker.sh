...
container_python_env_ready() {
    docker exec "$DOCKER_CONTAINER" bash -c '
        ...
        arch=$(uname -m)
        venv="/workspace/.venv-${platform}-${arch}"
...
# Wait for ENTRYPOINT venv setup before docker exec (avoids concurrent uv venv creation)
log_info "Waiting for entrypoint Python environment setup..."
venv_ready=false
for i in $(seq 1 300); do
    if container_python_env_ready; then
        venv_ready=true
        log_success "Entrypoint venv ready (${i} poll(s))"
        break
    fi
    sleep 2
done
if [[ "$venv_ready" != "true" ]]; then
    log_error "Timed out waiting for entrypoint venv (600s)"
    echo ""
    echo "Container logs (last 40 lines):"
    docker logs "$DOCKER_CONTAINER" 2>&1 | tail -40
    docker rm -f "$DOCKER_CONTAINER" 2>/dev/null || true
    exit 1
fi
