...
# Pull allowlisted @resources files from econ-ark/econ-ark-tools before each build
# (owner 2026-08-30: "whenever the doc is recompiled, download the latest version and incorporate it
# locally"). DEFAULT-DENY via @local/econark-sync.allow, and PROTECTED files are refused even if
# listed -- this is deliberately NOT @resources/scripts/@resources-update-from-remote.sh, which
# rewrites ~30 files wholesale and would revert local fixes (CLAUDE.md). Fails soft: a missing clone
# or no network warns and lets the build proceed. Disable with ECONARK_SYNC=0.
if (-x "$latexmkroot/\@local/econark-sync.sh") {
    system("\"$latexmkroot/\@local/econark-sync.sh\"");
}
