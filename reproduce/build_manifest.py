...
def _hafiscal_env_vars() -> dict:
    """Snapshot a curated subset of HAFISCAL_* env vars (None if unset).

    Kept for backward compatibility (emit-recipe consumes this). The COMPLETE
    picture is in `all_hafiscal_env_at_start` + `resolved_config`.
    """
    keys = [
        "HAFISCAL_RUN_STEP_1",
        "HAFISCAL_RUN_STEP_2",
...
def _all_hafiscal_env() -> dict:
    """Snapshot EVERY HAFISCAL_* env var present, plus BLAS thread counts.

    Captures what argv + the 9-var curated snapshot miss: env exported in the
    launching shell (e.g. HAFISCAL_NUM_STARTS, OMP_NUM_THREADS)."""
...
def _resolved_config() -> dict | None:
    """The resolved effective config (world/method/interpretation + env map).

    Best-effort: imports the config package from Code/HA-Models; returns None if
    unavailable so the manifest still builds on a minimal checkout.
    """
    try:
        ha_root = REPO_ROOT / "Code" / "HA-Models"
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    manifest = {
        ...
        "invocation": {
            ...
            "all_hafiscal_env_at_start": _all_hafiscal_env(),
        },
