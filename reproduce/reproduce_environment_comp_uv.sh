...
# Python for uv must satisfy pyproject requires-python and numba (currently <3.12).
# Keep this aligned with repo .python-version so `uv sync` and this script match.
# Legacy HARK 0.14.1 reproduction: use Python 3.10 — put "3.10" in .python-version on that checkout.
if [[ -f "$PROJECT_ROOT/.python-version" ]]; then
    UV_PYTHON_REQUEST=$(tr -d '[:space:]' < "$PROJECT_ROOT/.python-version")
else
    UV_PYTHON_REQUEST="3.11"
fi
...
if [[ -d "$VENV_PATH" ]] && [[ -f "$VENV_PATH/bin/python" ]]; then
    ...
    if "$VENV_PATH/bin/python" -c "import HARK" 2>/dev/null; then
        ...
        # Wrong micro-lineage (e.g. 3.13 or stale 3.10 after .python-version → 3.11): rebuild, do not exit early
        if [[ "$PYTHON_VERSION" != "${UV_PYTHON_REQUEST}" ]] && [[ "$PYTHON_VERSION" != "${UV_PYTHON_REQUEST}".* ]]; then
            echo "⚠️  Venv uses Python $PYTHON_VERSION but this repo requests ${UV_PYTHON_REQUEST}.x (see .python-version / pyproject)."
            echo "   Removing $VENV_NAME and continuing to recreate with Python ${UV_PYTHON_REQUEST}..."
            echo ""
            rm -rf "$VENV_PATH"
        else
            ...
            (
                ...
                ln -sfn "$VENV_NAME" .venv
                echo "   Linked .venv -> $VENV_NAME (plain uv sync uses this path)"
            )
            echo ""
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# STEP 3: Ensure requested Python is available (see UV_PYTHON_REQUEST)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

echo "Checking Python ${UV_PYTHON_REQUEST} availability (from .python-version or default 3.11)..."
if uv python list 2>/dev/null | grep -q "cpython-${UV_PYTHON_REQUEST}"; then
    echo "✅ Python ${UV_PYTHON_REQUEST} is available"
else
    echo "Installing Python ${UV_PYTHON_REQUEST}..."
    uv python install "${UV_PYTHON_REQUEST}"
    echo "✅ Python ${UV_PYTHON_REQUEST} installed"
fi
echo ""
...
# Drop only a *wrong* .venv symlink (uv follows a correct .venv -> $VENV_NAME when syncing)
if [[ -L ".venv" ]]; then
    if [[ "$(readlink .venv)" != "$VENV_NAME" ]]; then
        echo "⚠️  Removing stale .venv symlink (expected -> $VENV_NAME)..."
        rm -f .venv
    fi
fi
...
# Create platform-specific venv if python executable is missing
if [[ ! -f "$VENV_PATH/bin/python" ]]; then
    ...
    # Install requested Python using UV's managed Python installer
    # This ensures we have a standalone Python that doesn't depend on Xcode/system Python
    echo "Ensuring Python ${UV_PYTHON_REQUEST} is available..."
    if [[ "$(uname -m)" == "arm64" ]]; then
        arch -arm64 uv python install "${UV_PYTHON_REQUEST}" >/dev/null 2>&1 || true
    else
        uv python install "${UV_PYTHON_REQUEST}" >/dev/null 2>&1 || true
    fi
    echo "✓ Python ${UV_PYTHON_REQUEST} ready"
    echo ""
    
    # Force arm64 on Apple Silicon; --clear replaces partial dirs from races
    if [[ "$(uname -m)" == "arm64" ]]; then
        echo "Detected Apple Silicon - creating arm64 environment"
        arch -arm64 uv venv --clear --python "${UV_PYTHON_REQUEST}" "$VENV_PATH"
    else
        uv venv --clear --python "${UV_PYTHON_REQUEST}" "$VENV_PATH"
    fi
...
# Do not let an unrelated activated venv override discovery (avoids 3.13/conda surprises)
unset VIRTUAL_ENV

# Force arm64 on Apple Silicon
if [[ "$(uname -m)" == "arm64" ]]; then
    if arch -arm64 uv sync --all-groups --python "${UV_PYTHON_REQUEST}"; then
        echo ""
    ...
    if uv sync --all-groups --python "${UV_PYTHON_REQUEST}"; then
        echo ""
...
# Make plain `uv sync` (no env vars) use the same env as this script: .venv -> platform venv
(
    cd "$PROJECT_ROOT" || exit 0
    ln -sfn "$VENV_NAME" .venv
    echo ""
    echo "   Linked .venv -> $VENV_NAME so default uv/Cursor path matches this environment."
)
...
echo "========================================"
echo "Virtual environment: $VENV_NAME/"
echo "Python version: ${UV_PYTHON_REQUEST} (uv-managed; see .python-version)"
